...
El bridge mantiene la interfaz estable y versiona la aplicación visual. `STABLE` usa `current.json`, `PREVIEW` carga una release explícita y `DEV` prueba el Vite del PC; si DEV no responde en 4 segundos, vuelve a STABLE. La release activa actual es `0.1.6` e incluye el panel paralelo `Witmind Lobby` con suscripción persistente al pronóstico y navegación integrada con Home Assistant.

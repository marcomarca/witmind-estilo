...
`;class ce extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",t=>this._handleClick(t)),this.shadowRoot.addEventListener("scroll",t=>this._handleEnergyScroll(t),!0)}set hass(t){var o,n,d,l;const e=this._hass,i=this._config(),a=this._relevantHassChanged(e,t),r=!e||((o=e.states)==null?void 0:o[i.energySensor])!==((n=t==null?void 0:t.states)==null?void 0:n[i.energySensor]),s=!((d=e==null?void 0:e.states)!=null&&d[i.energySensor])&&!!((l=t==null?void 0:t.states)!=null&&l[i.energySensor]);this._hass=t,a&&this._syncStatesFromHass(),this.isConnected&&(this._started?r&&(s?this._loadEnergyStatistics():this._scheduleEnergyRefresh()):(this._started=!0,this._start()),a&&this._updateStatePresentation())}get hass(){return this._hass}set panel(t){const e=this._config().weather;this._panel=t;const i=this._config().weather;this._config().panelKind==="general"&&this._activeView==="home"&&(this._activeView="lights"),this._started&&e!==i&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._updateStatePresentation())}get panel(){return this._panel}set narrow(t){this._narrow=!!t,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._started||(this._started=!0,this._start()),this._updateStatePresentation(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const t of this._switchTimers.values())clearTimeout(t);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_updateStatePresentation(){var o;if(!((o=this.shadowRoot)!=null&&o.querySelector(".app-shell"))){this._requestRender();return}const t=n=>{var m;const d=n.dataset.entity;if(!d)return;const l=this._state(d),p=this._visibleSwitchState(d),h=p==="on",g=!l||["unknown","unavailable"].includes(l.state),u=this._pendingSwitches.has(d),f=this._switchErrors.get(d),_=f||(u?p==="on"?"Encendiendo…":"Apagando…":g?"No disponible":h?"Encendido":"Apagado");n.classList.toggle("is-on",h),n.classList.toggle("is-pending",u),n.classList.toggle("is-error",!!f),n.disabled=g,n.setAttribute("aria-pressed",String(h));const A=((m=n.querySelector(".device-copy strong"))==null?void 0:m.textContent)||d;n.setAttribute("aria-label",`${A}: ${_}`);const v=n.querySelector(".device-copy small");v&&(v.textContent=_)};this.shadowRoot.querySelectorAll('[data-action="toggle-switch"]').forEach(t),this.shadowRoot.querySelectorAll('[data-action="run-scene"]').forEach(n=>{const d=this._allScenes().find(u=>u.key===n.dataset.sceneKey);if(!d)return;const l=this._sceneStatus(d),p=this._pendingAction===d.key,h=p?"Aplicando...":l.active?"Activo":l.unavailable?"Sin datos":"Inactivo";n.classList.toggle("is-pending",p),n.classList.toggle("is-active",l.active),n.classList.toggle("is-unavailable",l.unavailable),n.disabled=!!(this._pendingAction&&!p),n.setAttribute("aria-pressed",String(l.active));const g=n.querySelector(".scene-state");g&&(g.textContent=h)});const e=this._config(),i=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],a=i.filter(n=>this._visibleSwitchState(n.entity)==="on").length,r=this.shadowRoot.querySelector("[data-lights-summary]");r&&(r.textContent=`${a} de ${i.length}`);const s=this.shadowRoot.querySelector("[data-energy-summary]");s&&(s.textContent=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`)}_handleClick(t){const e=t.target.closest("[data-action]");if(!e||!this._hass)return;const i=e.dataset.action;if(i==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(i==="toggle-theme"){this._toggleTheme();return}if(i==="set-view"){const a=e.dataset.view;["home","lights","energy","system"].includes(a)&&(this._activeView=a,this._requestRender());return}if(i==="toggle-switch"){this._toggleSwitch(e.dataset.entity);return}if(i==="run-scene"){this._runScene(e.dataset.sceneKey||e.dataset.entity,e.dataset.label);return}if(i==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(i==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(i==="general-off"){const a=this._config().generalOffScript;a?this._hass.callService("script","turn_on",{entity_id:a}).catch(r=>{this._notify("No se pudo ejecutar Apagar todo Witmind.","error"),console.error("Error ejecutando el apagado general de Witmind:",r)}):this._notify("El apagado general aún no tiene una entidad configurada.","error");return}if(i==="cancel-power-confirm"){const a=t.target.closest("[data-dialog-card]");if(e.classList.contains("dialog-backdrop")&&a)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(i==="confirm-power"){this._confirmGeneralPower();return}if(i==="clear-scene"){this._clearScene();return}if(i==="media"){this._mediaAction(e.dataset.service);return}if(i==="energy-range"){this._setEnergyRange(e.dataset.range);return}if(i==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(i==="energy-day-next"){this._shiftEnergyDay(1);return}if(i==="energy-day-today"){this._resetEnergyDay();return}i==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(t){console.warn("No se pudo guardar el tema del showroom:",t)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this.dispatchEvent(new CustomEvent("witmind-theme-change",{detail:{theme:this._theme},bubbles:!0,composed:!0})),this._requestRender()}set theme(t){t!=="dark"&&t!=="light"||(this._theme=t,this.setAttribute("data-theme",t),this._saveTheme())}get theme(){return this._theme}_config(){var u;const t=((u=this._panel)==null?void 0:u.config)||{},e=t.panel_kind==="lobby"||t.panel_kind==="general"||t.static_only===!0||t.staticOnly===!0,i=f=>Array.isArray(f)?[...new Set(f.filter(Boolean).map(_=>String(_)))]:[],a=(f,_)=>(Array.isArray(f)&&(f.length||e)?f:_).filter(v=>v==null?void 0:v.entity).map((v,m)=>({entity:String(v.entity),name:v.name||`Dispositivo ${m+1}`,subtitle:v.subtitle||"Iluminación",icon:v.icon||"bulb"})),r=a(t.spots,k.spots),s=a(t.samples||t.muestras,k.samples),o=[...r,...s].map(f=>f.entity),n=i(t.scene_control_entities||t.sceneControlEntities),d=n.length?n:o,l=(f,_,A)=>(Array.isArray(f)&&(f.length||e)?f:_).filter(m=>(m==null?void 0:m.entity)||(m==null?void 0:m.id)||(m==null?void 0:m.key)).map((m,b)=>{const w=m.entity?String(m.entity):"",T=String(m.id||m.key||w||`${A}-${b+1}`),x=_.find(L=>{const At=L.entity?String(L.entity):"",se=String(L.id||L.key||At||"");return w&&At===w||se===T}),z=w||(x!=null&&x.entity?String(x.entity):""),N=String(m.id||m.key||(x==null?void 0:x.id)||z||`${A}-${b+1}`),M=z||`${A}:${N}`,$=Array.isArray(m.on_entities)||Array.isArray(m.onEntities),it=Array.isArray(m.off_entities)||Array.isArray(m.offEntities),ot=i($?m.on_entities||m.onEntities:x==null?void 0:x.onEntities),ae=i(it?m.off_entities||m.offEntities:[]),re=it?ae.filter(L=>!ot.includes(L)):d.filter(L=>!ot.includes(L));return{key:M,id:N,entity:z,name:m.name||(x==null?void 0:x.name)||`Escena ${b+1}`,subtitle:m.subtitle||(x==null?void 0:x.subtitle)||"Escena del showroom",icon:m.icon||(x==null?void 0:x.icon)||"presentation",directOnly:m.direct_only??m.directOnly??(x==null?void 0:x.directOnly)??!1,onEntities:ot,offEntities:re}}),p=t.reflector||k.reflector,h=Number(t.history_hours??t.historyHours),g=Number(t.chart_hours??t.chartHours);return{title:t.title||k.title,subtitle:t.subtitle||k.subtitle,siteLabel:t.site_label||t.siteLabel||k.siteLabel,logo:t.logo||k.logo,weather:t.weather||k.weather,mediaPlayer:t.media_player||t.mediaPlayer||k.mediaPlayer,lightCountSensor:t.light_count_sensor||t.lightCountSensor||k.lightCountSensor,energySensor:t.energy_sensor||t.energySensor||k.energySensor,batteryLevel:t.battery_level||t.batteryLevel||k.batteryLevel,powerOnScript:t.power_on_script||t.powerOnScript||k.powerOnScript,powerOffScript:t.power_off_script||t.powerOffScript||k.powerOffScript,historyHours:Number.isFinite(h)&&h>0?Math.min(24,h):k.historyHours,chartHours:Number.isFinite(g)&&g>0?Math.min(72,g):k.chartHours,showForecast:t.show_forecast??t.showForecast??k.showForecast,panelKind:t.panel_kind||t.panelKind||"showroom",staticOnly:t.static_only??t.staticOnly??!1,generalOffScript:t.general_off_script||t.generalOffScript||"",spots:r,samples:s,sceneControlEntities:d,reflector:p!=null&&p.entity?{entity:String(p.entity),name:p.name||k.reflector.name,subtitle:p.subtitle||k.reflector.subtitle,icon:p.icon||k.reflector.icon}:null,scenes:l(t.scenes,k.scenes,"scene"),sampleScenes:l(t.sample_scenes||t.sampleScenes,k.sampleScenes,"sample")}}_allDevices(){const t=this._config();return[...t.spots,...t.samples,...t.reflector?[t.reflector]:[]]}_allScenes(t=this._config()){return[...t.scenes,...t.sampleScenes]}_trackedEntities(){const t=this._config();return new Set([...this._allDevices().map(e=>e.entity),...t.sceneControlEntities,...this._allScenes(t).flatMap(e=>[e.entity,...e.onEntities,...e.offEntities]),t.weather,t.mediaPlayer,t.lightCountSensor,t.energySensor,t.batteryLevel,t.powerOnScript,t.powerOffScript].filter(Boolean))}_relevantHassChanged(t,e){var i,a;if(!t||!e)return!0;for(const r of this._trackedEntities())if(((i=t.states)==null?void 0:i[r])!==((a=e.states)==null?void 0:a[r]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var t;if((t=this._hass)!=null&&t.states)for(const e of this._trackedEntities()){const i=this._hass.states[e];i&&this._applyLiveState(e,i)}}_applyLiveState(t,e){if(!e){this._liveStates.delete(t);return}this._liveStates.set(t,e);const i=this._pendingSwitches.get(t);if(i&&e.state===i.desired){this._pendingSwitches.delete(t),this._switchErrors.delete(t);const a=this._switchTimers.get(t);a&&clearTimeout(a),this._switchTimers.delete(t)}}async _fetchCurrentStates(){var t;if((t=this._hass)!=null&&t.callWS)try{const e=await this._hass.callWS({type:"get_states"}),i=this._trackedEntities();for(const a of e||[])i.has(a.entity_id)&&this._applyLiveState(a.entity_id,a);this._updateStatePresentation()}catch(e){console.error("No se pudieron sincronizar los estados del showroom:",e)}}async _subscribeStateChanges(){var t;if(!(!((t=this._hass)!=null&&t.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(e=>{var a;const i=(a=e==null?void 0:e.data)==null?void 0:a.entity_id;!i||!this._trackedEntities().has(i)||(this._applyLiveState(i,e.data.new_state),i===this._config().energySensor&&this._scheduleEnergyRefresh(),this._updateStatePresentation())},"state_changed")}catch(e){console.error("No se pudo suscribir a state_changed:",e)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var e;const t=this._config();if(!(!((e=this._hass)!=null&&e.connection)||!t.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===t.weather)){this._resetForecastSubscription();try{this._forecastEntity=t.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(i=>{this._forecast=Array.isArray(i==null?void 0:i.forecast)?i.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:t.weather})}catch(i){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",i)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(t=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const e=this._energyRefreshInterval(),i=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,a=t?0:this._energyLastLoadedAt?Math.max(1e3,e-i):e;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),a)}_energyRangeDefinition(t=this._energyRange){const e=new Date;let i,a=e,r,s,o,n=!1;if(t==="year")i=new Date(e.getFullYear(),0,1,0,0,0,0),r="month",s=`Año ${e.getFullYear()}`,o="mes";else if(t==="month")i=new Date(e.getFullYear(),e.getMonth(),1,0,0,0,0),r="day",s=e.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),o="día";else{const d=new Date(e.getFullYear(),e.getMonth(),e.getDate(),0,0,0,0);i=new Date(d.getFullYear(),d.getMonth(),d.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),n=i.getTime()===d.getTime(),a=n?e:new Date(i.getFullYear(),i.getMonth(),i.getDate()+1,0,0,0,0),r="hour",s=i.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),o="hora"}return{start:i,end:a,period:r,title:s,intervalLabel:o,isToday:n}}_energyValueToKWh(t,e){const i=Number(t);if(!Number.isFinite(i))return null;const a=String(e||"kWh").trim().toLowerCase().replaceAll(" ","");return a==="kwh"?i:a==="wh"?i/1e3:a==="mwh"?i*1e3:null}_deriveEnergyStateDeltas(t,e,i){const a=Number(e instanceof Date?e.getTime():e),r=Number(i instanceof Date?i.getTime():i),s=t.filter(d=>Number.isFinite(Number(d.start))&&Number.isFinite(Number(d.state))).sort((d,l)=>Number(d.start)-Number(l.start)),o=[];let n=null;for(const d of s){const l=Number(d.start),p=Number(d.state);if(l<a){n=p;continue}if(l>=r)break;let h=0;if(Number.isFinite(n)){const g=p-n;h=g>=0?g:Math.max(0,p)}o.push({...d,change:h}),n=p}return o}_aggregateEnergyDayRows(t,e,i){var d;const a=new Date(e.start),r=new Date(e.end),s=e.isToday?new Date(r.getFullYear(),r.getMonth(),r.getDate(),r.getHours(),0,0,0):new Date(a.getFullYear(),a.getMonth(),a.getDate(),23,0,0,0),o=new Map;for(let l=new Date(a);l<=s;l.setHours(l.getHours()+1)){const p=l.getTime();o.set(p,{start:p,end:new Date(l.getFullYear(),l.getMonth(),l.getDate(),l.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:e.isToday&&p===s.getTime(),live:!1})}let n=null;for(const l of t){const p=new Date(Number(l.start));if(!Number.isFinite(p.getTime()))continue;const h=new Date(p.getFullYear(),p.getMonth(),p.getDate(),p.getHours(),0,0,0).getTime(),g=o.get(h);g&&(g.change+=Math.max(0,Number(l.change)||0),g.samples+=1,Number.isFinite(l.state)&&(!n||Number(l.end||l.start)>Number(n.end||n.start))&&(n=l))}if(e.isToday){const l=this._energyValueToKWh(i==null?void 0:i.state,(d=i==null?void 0:i.attributes)==null?void 0:d.unit_of_measurement),p=n==null?void 0:n.state,h=Number((n==null?void 0:n.end)||(n==null?void 0:n.start)),g=o.get(s.getTime());if(g&&Number.isFinite(l)&&Number.isFinite(p)&&Number.isFinite(h)&&h>=s.getTime()&&r.getTime()-h>=0&&r.getTime()-h<=900*1e3){const u=l-p;Number.isFinite(u)&&u>=0&&(g.change+=u,g.live=u>0)}}return[...o.values()].filter(l=>l.samples>0||l.live)}_calculateCurrentMonthEnergy(t,e,i){var h;const a=new Date(e),r=new Date(a.getFullYear(),a.getMonth(),1,0,0,0,0).getTime(),s=t.filter(g=>Number.isFinite(Number(g.start))&&Number(g.start)>=r).sort((g,u)=>Number(g.start)-Number(u.start));let o=s.reduce((g,u)=>g+Math.max(0,Number(u.change)||0),0),n=null;for(const g of s)Number.isFinite(Number(g.state))&&(!n||Number(g.end||g.start)>Number(n.end||n.start))&&(n=g);const d=this._energyValueToKWh(i==null?void 0:i.state,(h=i==null?void 0:i.attributes)==null?void 0:h.unit_of_measurement),l=Number(n==null?void 0:n.state),p=Number((n==null?void 0:n.end)||(n==null?void 0:n.start));if(Number.isFinite(d)&&Number.isFinite(l)&&Number.isFinite(p)&&p>=r&&p<=a.getTime()&&a.getTime()-p<=7200*1e3){const g=d-l;Number.isFinite(g)&&g>=0&&(o+=g)}return s.length||Number.isFinite(l)?o:null}_aggregateEnergyCalendarRows(t,e,i,a){var p;if(!["month","year"].includes(a))return[];const r=new Map;let s=null;for(const h of t){const g=Number(h.start);if(!Number.isFinite(g))continue;const u=new Date(g),f=a==="month"?new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0).getTime():new Date(u.getFullYear(),u.getMonth(),1,0,0,0,0).getTime(),_=r.get(f)||{start:f,change:0,samples:0,partial:!1,live:!1};_.change+=Math.max(0,Number(h.change)||0),_.samples+=1,r.set(f,_),Number.isFinite(Number(h.state))&&(!s||g>Number(s.start))&&(s=h)}const o=this._energyValueToKWh(i==null?void 0:i.state,(p=i==null?void 0:i.attributes)==null?void 0:p.unit_of_measurement),n=Number(s==null?void 0:s.state),d=Number(s==null?void 0:s.start),l=new Date(e.end);if(Number.isFinite(o)&&Number.isFinite(n)&&Number.isFinite(d)&&d<=l.getTime()&&l.getTime()-d<=7200*1e3){const h=o-n;if(Number.isFinite(h)&&h>=0){const g=a==="month"?new Date(l.getFullYear(),l.getMonth(),l.getDate(),0,0,0,0).getTime():new Date(l.getFullYear(),l.getMonth(),1,0,0,0,0).getTime(),u=r.get(g)||{start:g,change:0,samples:0,partial:!1,live:!1};u.change+=h,u.live=h>0,r.set(g,u)}}return[...r.values()].sort((h,g)=>Number(h.start)-Number(g.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(t=>[Number(t.start),Number(t.change)||0,Number(t.samples)||0,!!t.partial,!!t.live])])}async _loadEnergyStatistics(){var g;if(!((g=this._hass)!=null&&g.connection))return;clearTimeout(this._energyRefreshTimer);const t=this._config(),e=this._state(t.energySensor),i=Number(e==null?void 0:e.state);if(!e){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${t.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(i)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${t.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const a=++this._energyRequestId,r=this._energyViewSignature(),s=this._energyData.length>0,o=this._energyRangeDefinition(),n=new Date,d=new Date(n.getFullYear(),n.getMonth(),1,0,0,0,0),l=new Date(o.start.getTime()-3600*1e3),p=this._energyRange==="year"?"hour":"5minute",h=new Date(d.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,s||this._requestRender();try{const u=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[t.energySensor]});if(a!==this._energyRequestId)return;const _=(Array.isArray(u)?u:[]).find(M=>(M==null?void 0:M.statistic_id)===t.energySensor)||null;if(!_||!_.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const A=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:l.toISOString(),end_time:o.end.toISOString(),statistic_ids:[t.energySensor],period:p,units:{energy:"kWh"},types:["state"]}),v=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:h.toISOString(),end_time:n.toISOString(),statistic_ids:[t.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[m,b]=await Promise.all([A,v]);if(a!==this._energyRequestId)return;const w=M=>(Array.isArray(M==null?void 0:M[t.energySensor])?M[t.energySensor]:[]).map($=>({start:Number($.start),end:Number($.end),change:$.change===void 0||$.change===null?null:Math.max(0,Number($.change)||0),state:$.state===void 0||$.state===null?null:Number($.state)})).filter($=>Number.isFinite($.start)&&(Number.isFinite($.state)||Number.isFinite($.change))),T=w(m),x=this._deriveEnergyStateDeltas(T,o.start,o.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows(x,o,e):this._aggregateEnergyCalendarRows(x,o,e,this._energyRange);const z=w(b),N=this._deriveEnergyStateDeltas(z,d,n);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(N,n,e),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((M,$)=>M+Math.max(0,Number($.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const M=new Date(n.getFullYear(),n.getMonth(),1,0,0,0,0).getTime(),$=this._energyData.find(it=>Number(it.start)===M);$&&($.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(u){if(a!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(u==null?void 0:u.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",u)}finally{if(a===this._energyRequestId){this._energyLoading=!1;const u=r!==this._energyViewSignature();!s||u&&this._activeView==="energy"?this._requestRender():this._updateStatePresentation(),this._scheduleEnergyRefresh()}}}_setEnergyRange(t){!["day","month","year"].includes(t)||t===this._energyRange||(this._energyRange=t,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(t){if(this._energyRange!=="day")return;const e=Number(t);if(!Number.isFinite(e)||e===0)return;const i=Math.min(0,this._energyDayOffset+e);i!==this._energyDayOffset&&(this._energyDayOffset=i,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(t,e=2){const i=Number(t);return Number.isFinite(i)?i.toLocaleString("es-BO",{minimumFractionDigits:e,maximumFractionDigits:e}):"--"}_energyLabel(t,e=this._energyRange){const i=new Date(Number(t));return Number.isFinite(i.getTime())?e==="year"?i.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):e==="month"?String(i.getDate()):i.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var i;const t=(i=this.shadowRoot)==null?void 0:i.querySelector("[data-energy-scroll]");if(!t)return;const e=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=e<=0||e-t.scrollLeft<=8}_handleEnergyScroll(t){var a,r;const e=(r=(a=t.target)==null?void 0:a.closest)==null?void 0:r.call(a,"[data-energy-scroll]");if(!e)return;const i=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=i<=0||i-e.scrollLeft<=8}_restoreEnergyChartScroll(){var i;const t=(i=this.shadowRoot)==null?void 0:i.querySelector("[data-energy-scroll]");if(!t)return;const e=Math.max(0,t.scrollWidth-t.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){t.scrollLeft=e,this._energyScrollLeft=t.scrollLeft;return}t.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,e))}_energyChart(){const t=this._energyData;if(this._energyLoading&&!t.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!t.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const e=280,i=14,a=16,r=22,s=42,o=this._energyRange==="day"?64:this._energyRange==="month"?36:58,n=Math.max(620,t.length*o+i+a),d=e-r-s,l=n-i-a,p=Math.max(...t.map(m=>m.change),.001),h=this._energyRange==="day"?10:t.length>24?4:7,g=Math.max(6,(l-h*Math.max(0,t.length-1))/t.length),u=this._energyRange==="month"?Math.max(1,Math.ceil(t.length/10)):this._energyRange==="day"?2:1,f=[0,.25,.5,.75,1],_=f.map(m=>{const b=r+d*(1-m);return`<line x1="${i}" y1="${b.toFixed(1)}" x2="${n-a}" y2="${b.toFixed(1)}" class="energy-grid-line"/>`}).join(""),A=f.map(m=>{const b=r+d*(1-m),w=this._formatEnergy(p*m,p<1?2:1);return`<span class="energy-y-tick" style="top:${b.toFixed(1)}px">${this._escape(w)}</span>`}).join(""),v=t.map((m,b)=>{const w=i+b*(g+h),T=m.change>0?Math.max(2,m.change/p*d):1,x=r+d-T,z=b%u===0||b===t.length-1,N=this._energyLabel(m.start),M=m.partial?" · en curso":"",$=m.partial?`<text x="${(w+g/2).toFixed(1)}" y="${(r+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${m.partial?"is-current":""}"><rect x="${w.toFixed(1)}" y="${x.toFixed(1)}" width="${g.toFixed(1)}" height="${T.toFixed(1)}" rx="${Math.min(4,g/2).toFixed(1)}" class="energy-bar ${m.partial?"is-partial":""}"><title>${this._escape(N)} · ${this._formatEnergy(m.change)} kWh${M}</title></rect>${$}${z?`<text x="${(w+g/2).toFixed(1)}" y="${e-14}" text-anchor="middle" class="energy-axis-text">${this._escape(N)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${A}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${n}" height="${e}" viewBox="0 0 ${n} ${e}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${_}${v}</svg></div></div>`}async _loadHistory(){var o;const t=this._config();if(!((o=this._hass)!=null&&o.callApi))return;const e=Math.max(t.historyHours,t.chartHours),i=new Date(Date.now()-e*60*60*1e3).toISOString(),a=new Date().toISOString(),r=[t.lightCountSensor,...t.spots.map(n=>n.entity),...t.samples.map(n=>n.entity)].filter(Boolean).join(",");if(!r)return;const s=`history/period/${encodeURIComponent(i)}?filter_entity_id=${encodeURIComponent(r)}&end_time=${encodeURIComponent(a)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",s),this._historyError=""}catch(n){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",n)}this._requestRender()}_state(t){var e,i;return this._liveStates.get(t)||((i=(e=this._hass)==null?void 0:e.states)==null?void 0:i[t])}_isUnavailable(t){var i;const e=(i=this._state(t))==null?void 0:i.state;return!e||e==="unknown"||e==="unavailable"}_visibleSwitchState(t){var e,i;return((e=this._pendingSwitches.get(t))==null?void 0:e.desired)||((i=this._state(t))==null?void 0:i.state)||"unavailable"}async _toggleSwitch(t){if(!t||!this._hass)return;const e=this._state(t);if(!e||["unknown","unavailable"].includes(e.state)){this._switchErrors.set(t,"No disponible"),this._updateStatePresentation();return}const a=this._visibleSwitchState(t)==="on"?"off":"on",r=a==="on"?"turn_on":"turn_off",s=t.split(".")[0]||"switch";this._pendingSwitches.set(t,{desired:a,startedAt:Date.now()}),this._switchErrors.delete(t),this._updateStatePresentation();try{await this._hass.callService(s,r,{entity_id:t});const o=this._switchTimers.get(t);o&&clearTimeout(o);const n=setTimeout(()=>this._verifySwitchState(t,a),6e3);this._switchTimers.set(t,n)}catch(o){this._pendingSwitches.delete(t),this._switchErrors.set(t,"La acción falló"),this._updateStatePresentation(),console.error(`Error ejecutando ${r} en ${t}:`,o)}}async _verifySwitchState(t,e){var a;await this._fetchCurrentStates();const i=((a=this._state(t))==null?void 0:a.state)===e;this._pendingSwitches.delete(t),this._switchTimers.delete(t),i?this._switchErrors.delete(t):this._switchErrors.set(t,"Sin confirmación"),this._updateStatePresentation()}_sceneStatus(t){const e=[...t.onEntities.map(r=>({entityId:r,desired:"on"})),...t.offEntities.map(r=>({entityId:r,desired:"off"}))];if(!e.length)return{active:!1,unavailable:!1,mismatches:[]};const i=e.some(({entityId:r})=>this._isUnavailable(r)),a=e.filter(({entityId:r,desired:s})=>{var o;return((o=this._state(r))==null?void 0:o.state)!==s});return{active:!i&&a.length===0,unavailable:i,mismatches:a}}_sceneExpectations(t){return[...t.offEntities.map(e=>({entityId:e,desired:"off"})),...t.onEntities.map(e=>({entityId:e,desired:"on"}))]}_markExpectedStates(t){const e=Date.now();for(const{entityId:i,desired:a}of t)this._pendingSwitches.set(i,{desired:a,startedAt:e}),this._switchErrors.delete(i)}_clearExpectedStates(t){for(const{entityId:e}of t){this._pendingSwitches.delete(e);const i=this._switchTimers.get(e);i&&clearTimeout(i),this._switchTimers.delete(e)}}async _setEntitiesState(t,e){const i=[...new Set((t||[]).filter(Boolean))];if(!i.length)return;const a=new Map;for(const s of i){const o=s.split(".")[0];o&&(a.has(o)||a.set(o,[]),a.get(o).push(s))}const r=e==="on"?"turn_on":"turn_off";for(const[s,o]of a)await this._hass.callService(s,r,{entity_id:o})}async _waitForExpectedStates(t,e=7e3){const i=Date.now()+e;let a=t;for(;Date.now()<i;){if(await this._fetchCurrentStates(),a=t.filter(({entityId:r,desired:s})=>{var o;return((o=this._state(r))==null?void 0:o.state)!==s}),!a.length)return{ok:!0,mismatches:[]};await new Promise(r=>setTimeout(r,450))}return{ok:!1,mismatches:a}}async _runScene(t,e){var o;if(!t||this._pendingAction)return;const i=this._config(),a=this._allScenes(i).find(n=>n.key===t||n.entity===t);if(!a){this._notify("La escena no está configurada.","error");return}const r=this._sceneExpectations(a);this._pendingAction=a.key,this._markExpectedStates(r),this._updateStatePresentation();let s=null;try{if(a.entity&&!a.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:a.entity})}catch(l){s=l,console.warn(`La escena ${a.entity} no respondió; se aplicará el perfil directo.`,l)}await this._setEntitiesState(a.offEntities,"off"),await this._setEntitiesState(a.onEntities,"on");const n=await this._waitForExpectedStates(r);if(!n.ok){const l=n.mismatches.map(p=>p.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${l}`)}const d=s?`${e||"Modo"} aplicado mediante control directo.`:`${e||"Modo"} activo.`;this._notify(d,"success")}catch(n){for(const{entityId:d,desired:l}of r)((o=this._state(d))==null?void 0:o.state)!==l&&this._switchErrors.set(d,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",n)}finally{this._clearExpectedStates(r),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _executeGeneralPower(t,e={}){var d;if(!this._hass||this._pendingAction||!["on","off"].includes(t))return!1;const i=this._config(),a=t==="on"?i.powerOnScript:i.powerOffScript,r=i.sceneControlEntities.map(l=>({entityId:l,desired:t})),s=e.successMessage||(t==="on"?"Iluminación general encendida.":"Iluminación general apagada."),o=e.errorMessage||(t==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=a||`direct-power-${t}`,this._markExpectedStates(r),this._updateStatePresentation();let n=null;try{if(a)try{await this._hass.callService("script","turn_on",{entity_id:a})}catch(p){n=p,console.warn(`El script ${a} no respondió; se aplicará el control directo.`,p)}await this._setEntitiesState(i.sceneControlEntities,t);const l=await this._waitForExpectedStates(r);if(!l.ok){const p=l.mismatches.map(h=>h.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${p}`)}return this._confirmAction="",this._notify(s,"success"),!0}catch(l){for(const{entityId:p,desired:h}of r)((d=this._state(p))==null?void 0:d.state)!==h&&this._switchErrors.set(p,"Sin confirmación");return this._notify(o,"error"),console.error("Error ejecutando el control general del showroom:",{error:l,scriptError:n,desired:t}),!1}finally{this._clearExpectedStates(r),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _confirmGeneralPower(){const t=this._confirmAction;["on","off"].includes(t)&&await this._executeGeneralPower(t,{successMessage:t==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(t){const e=this._config().mediaPlayer;if(!(!e||!t||this._isUnavailable(e)))try{await this._hass.callService("media_player",t,{entity_id:e})}catch(i){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${t}:`,i)}}_updateToastPresentation(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("[data-toast]");if(!this._toast){t==null||t.remove();return}if(t){t.className=`toast ${this._escape(this._toast.type)}`,t.textContent=this._toast.message;return}const e=document.createElement("div");e.dataset.toast="",e.className=`toast ${this._escape(this._toast.type)}`,e.setAttribute("role","status"),e.textContent=this._toast.message,this.shadowRoot.append(e)}_notify(t,e="success"){var i;clearTimeout(this._toastTimer),this._toast={message:t,type:e},(i=this.shadowRoot)!=null&&i.querySelector(".app-shell")?this._updateToastPresentation():this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._updateToastPresentation()},4200)}_historyMap(){var e;const t=new Map;for(const i of this._history||[]){const a=(e=i==null?void 0:i[0])==null?void 0:e.entity_id;a&&t.set(a,i)}return t}_historySegments(t,e,i){if(!Array.isArray(t)||!t.length)return[];const a=t.map(s=>({state:s.state,time:new Date(s.last_changed||s.last_updated).getTime()})).filter(s=>Number.isFinite(s.time)).sort((s,o)=>s.time-o.time);if(!a.length)return[];const r=[];for(let s=0;s<a.length;s+=1){const o=a[s],n=a[s+1],d=Math.max(e,o.time),l=Math.min(i,(n==null?void 0:n.time)??i);l<=d||r.push({state:o.state,left:(d-e)/(i-e)*100,width:(l-d)/(i-e)*100})}return r}_sparkline(t,e){var u;const i=this._historyMap().get(t)||[],a=Date.now(),r=a-e*60*60*1e3,s=i.map(f=>({value:Number(f.state),time:new Date(f.last_changed||f.last_updated).getTime()})).filter(f=>Number.isFinite(f.value)&&Number.isFinite(f.time)&&f.time>=r).sort((f,_)=>f.time-_.time),o=Number((u=this._state(t))==null?void 0:u.state);if(Number.isFinite(o)&&s.push({value:o,time:a}),!s.length)return{path:"",min:"--",max:"--",avg:"--"};const n=s.map(f=>f.value),d=Math.min(...n),l=Math.max(...n),p=n.reduce((f,_)=>f+_,0)/n.length,h=l-d||1;return{path:s.map((f,_)=>{const A=(f.time-r)/(a-r)*300,v=66-(f.value-d)/h*52;return`${_?"L":"M"}${Math.max(0,Math.min(300,A)).toFixed(1)},${v.toFixed(1)}`}).join(" "),min:this._formatNumber(d),max:this._formatNumber(l),avg:this._formatNumber(p)}}_formatNumber(t){return Number.isFinite(t)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(t):"--"}_updateClock(){var o,n,d;if(!this.shadowRoot)return;const t=new Date,e=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(t),i=((o=e.find(l=>l.type==="hour"))==null?void 0:o.value)||"--",a=((n=e.find(l=>l.type==="minute"))==null?void 0:n.value)||"--",r=(((d=e.find(l=>l.type==="dayPeriod"))==null?void 0:d.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const l of this.shadowRoot.querySelectorAll("[data-clock-time]"))l.textContent=`${i}:${a}`;for(const l of this.shadowRoot.querySelectorAll("[data-clock-period]"))l.textContent=r||"";const s=this.shadowRoot.querySelector("[data-current-time]");s&&(s.dateTime=t.toISOString(),s.setAttribute("aria-label",`${i}:${a} ${r}`.trim()))}_icon(t,e=""){const i=Mt[t]||Mt.bulb;return`<svg class="icon ${this._escape(e)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${i}</svg>`}_escape(t){return String(t??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(t){const e=this._state(t.entity),i=this._visibleSwitchState(t.entity),a=i==="on",r=!e||["unknown","unavailable"].includes(e.state),s=this._pendingSwitches.has(t.entity),o=this._switchErrors.get(t.entity),n=o||(s?i==="on"?"Encendiendo…":"Apagando…":r?"No disponible":a?"Encendido":"Apagado");return`
    ...
    `}_renderWeather(){const t=this._config(),e=this._state(t.weather);if(!e)return`
      ...
      <section class="surface weather-card">
        <div class="weather-main">
          <div class="weather-symbol">${this._escape(ct[a]||"·")}</div>
          <div class="weather-copy">
            <span class="eyebrow">Clima · Casa</span>
            <h2>${this._escape(Et[a]||a)}</h2>
          ...
          <div class="forecast-row">
            ${r.length?r.map(s=>{const o=new Date(s.datetime),n=new Intl.DateTimeFormat("es-BO",{weekday:"short"}).format(o);return`
                <div class="forecast-item">
                  ...
                  <b>${this._escape(ct[s.condition]||"·")}</b>
    ...
    `}_renderMedia(){const t=this._config(),e=this._state(t.mediaPlayer),i=!e||["unknown","unavailable"].includes(e.state),a=(e==null?void 0:e.attributes)||{},r=(e==null?void 0:e.state)==="playing",s=i?"No disponible":r?"Reproduciendo":(e==null?void 0:e.state)==="paused"?"En pausa":(e==null?void 0:e.state)==="idle"?"En espera":(e==null?void 0:e.state)||"Detenido",o=a.media_title||a.friendly_name||"Showroom 1",n=a.media_artist||a.source||"Música del showroom",d=Number(a.volume_level),l=(p,h,g,u=!1)=>`
      <button
      ...
      <section class="surface media-card ${i?"is-unavailable":""}">
        ...
        <div class="media-body">
          ...
          <div class="media-copy">
            ...
            <span>${this._escape(n)}</span>
            <small>${Number.isFinite(d)?`Volumen ${Math.round(d*100)}%`:"Volumen no informado"}</small>
          </div>
        </div>
        <div class="media-controls">
          ${l("volume_down","volumeDown","Bajar volumen")}
          ${l("media_previous_track","previous","Pista anterior")}
          ${l("media_play_pause",r?"pause":"play",r?"Pausar":"Reproducir",!0)}
          ${l("media_next_track","next","Pista siguiente")}
          ${l("volume_up","volumeUp","Subir volumen")}
        </div>
      </section>
    ...
    `}_renderSceneBlock({eyebrow:t,title:e,scenes:i,className:a}){const r=this._config(),s=i.find(l=>this._sceneStatus(l).active),o=this._pendingAction===r.powerOffScript,n=r.sceneControlEntities.some(l=>{var p;return((p=this._state(l))==null?void 0:p.state)==="on"}),d=!!this._pendingAction||!n;return`
      <section class="surface scenes-card ${this._escape(a)}">
        ...
        <div class="scene-grid">${i.map(l=>this._renderSceneButton(l)).join("")}</div>
      </section>
      ...
      <section class="surface activity-card energy-shell">
        ...
        <div class="energy-toolbar">
          <div class="energy-tabs" role="tablist" aria-label="Período de consumo energético">
            ${[["day","Día"],["month","Mes"],["year","Año"]].map(([n,d])=>`<button class="energy-tab ${this._energyRange===n?"is-active":""}" data-action="energy-range" data-range="${n}" role="tab" aria-selected="${this._energyRange===n}">${d}</button>`).join("")}
          </div>
      ...
      <div class="app-shell">
        <header class="topbar">
          ...
          ${e?"":`<div class="status-strip" aria-label="Resumen de ${this._escape(t.title)}">
            ...
            ${t.panelKind==="general"?"":`<button class="status-pill ${d==="playing"?"is-active":""}" data-action="set-view" data-view="home">
              <span class="status-pill-icon">${this._icon("music")}</span>
              <span><strong>${this._escape(l)}</strong><small>Multimedia</small></span>
            </button>`}
          ...
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(Et[r]||r||"Sin datos")}, ${this._escape(a.temperature??"Sin datos")}${this._escape(a.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(ct[r]||"·")}</span>
              <strong>${this._escape(a.temperature??"--")}${this._escape(a.temperature_unit??"°")}</strong>
            </div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar a tema ${s}" title="Cambiar a tema ${s}">${oe}</button>
          </div>
        </header>
    ...
    `,this._updateClock(),requestAnimationFrame(()=>this._restoreEnergyChartScroll())}}customElements.get("showroom-panel")||customElements.define("showroom-panel",ce);const le={"clear-night":"Noche despejada",cloudy:"Nublado",exceptional:"Condición excepcional",fog:"Niebla",hail:"Granizo",lightning:"Tormenta eléctrica","lightning-rainy":"Tormenta y lluvia",partlycloudy:"Parcialmente nublado",pouring:"Lluvia intensa",rainy:"Lluvia",snowy:"Nieve","snowy-rainy":"Aguanieve",sunny:"Soleado",windy:"Ventoso","windy-variant":"Viento y nubes"},de={"clear-night":"☾",cloudy:"☁",exceptional:"!",fog:"≋",hail:"◆",lightning:"ϟ","lightning-rainy":"ϟ",partlycloudy:"◒",pouring:"☂",rainy:"☂",snowy:"❄","snowy-rainy":"❄",sunny:"☀",windy:"≈","windy-variant":"≈"},he='<svg class="menu-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><line x1="3" x2="21" y1="6" y2="6"></line><line x1="3" x2="21" y1="12" y2="12"></line><line x1="3" x2="21" y1="18" y2="18"></line></svg>',pe='<svg class="theme-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><g class="theme-icon-sun"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"></path></g><path class="theme-icon-moon" d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"></path></svg>',Ct={bulb:'<path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5M9 18h6M10 22h4"/>',panel:'<rect width="18" height="18" x="3" y="3" rx="3"/><path d="M3 9h18M3 15h18M9 3v18M15 3v18"/>',corridor:'<path d="M4 3h16v18H4V3Zm2 2v14h5V5H6Zm7 0v14h5V5h-5Z"/>',workshop:'<path d="M3 7l9-5 9 5v14H3V7Zm2 1.2V19h4v-6h6v6h4V8.2l-7-3.9-7 3.9Z"/>',power:'<path d="M12 2v10M18.4 6.6a9 9 0 1 1-12.8 0"/>',energy:'<path d="M13 2 5 13h6l-1 9 8-11h-6l1-9Z"/>',shield:'<path d="M12 3 20 6v5c0 5-3.4 8.3-8 10-4.6-1.7-8-5-8-10V6l8-3Z"/>',office:'<path d="M3 4h18v16H3V4Zm4 4h4v3H7V8Zm6 0h4v3h-4V8ZM7 13h4v3H7v-3Zm6 0h4v3h-4v-3Z"/>'},W=c=>`${new Intl.NumberFormat("es-BO",{maximumFractionDigits:0}).format(c)} W`;class ge extends HTMLElement{constructor(){super(),this._hass=null,this._panel={},this._started=!1,this._renderQueued=!1,this._pending=new Map,this._errors=new Map,this._pendingAction="",this._confirmAction="",this._toast="",this._toastTimer=null,this._theme=this._loadTheme(),this.attachShadow({mode:"open"}),this.shadowRoot.addEventListener("click",t=>this._handleClick(t))}set hass(t){var e;this._hass=t,this._started||(this._started=!0),this.isConnected&&((e=this.shadowRoot)!=null&&e.querySelector(".operations-shell")?this._updatePresentation():this._render())}get hass(){return this._hass}set panel(t){this._panel=t&&typeof t=="object"?t:{},this.isConnected&&this._render()}get panel(){return this._panel}set theme(t){if(t==="dark"||t==="light"){const e=this._theme!==t;this._theme=t,this.setAttribute("data-theme",t),this._saveTheme(),e&&this.isConnected&&this._queueRender()}}get theme(){return this._theme}connectedCallback(){this.setAttribute("data-theme",this._theme),this._hass&&this._render()}disconnectedCallback(){this._toastTimer&&window.clearTimeout(this._toastTimer)}_kind(){return String(this._panel.panel_kind||this._panel.panelKind||"offices").toLowerCase()}_loadTheme(){try{return localStorage.getItem("witmind-showroom-panel-theme")==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem("witmind-showroom-panel-theme",this._theme)}catch{}}_toggleTheme(){this.theme=this._theme==="dark"?"light":"dark",this.dispatchEvent(new CustomEvent("witmind-theme-change",{detail:{theme:this._theme},bubbles:!0,composed:!0}))}_state(t){var e,i;return(i=(e=this._hass)==null?void 0:e.states)==null?void 0:i[t]}_isOn(t){var e;return((e=this._state(t))==null?void 0:e.state)==="on"||this._pending.get(t)==="on"}_unavailable(t){var i;const e=(i=this._state(t))==null?void 0:i.state;return!e||e==="unknown"||e==="unavailable"}_escape(t){return String(t??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_icon(t){return`<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${Ct[t]||Ct.bulb}</svg>`}_devices(){const t=this._kind(),e=this._panel.power_watts||this._panel.powerWatts||this._panel.device_watts||{},i=a=>({...a,watts:a.watts??(Number(e[a.entity]||0)||void 0)});return t==="offices"?(this._panel.areas||[]).flatMap(a=>(a.devices||[]).map(i)):t==="recording"?(this._panel.switches||[]).map(i):(this._panel.zones||[]).flatMap(a=>(a.devices||(a.entities||[]).map(r=>({entity:r,name:r}))).map(i))}_powerTotal(t=this._devices()){return t.reduce((e,i)=>e+Number(i.watts||0),0)}_powerActive(t=this._devices()){return t.filter(e=>this._isOn(e.entity)).reduce((e,i)=>e+Number(i.watts||0),0)}_weather(){const t=this._state(this._panel.weather||"weather.forecast_casa");return{temperature:((t==null?void 0:t.attributes)||{}).temperature??"—",condition:le[String((t==null?void 0:t.state)||"")]||String((t==null?void 0:t.state)||"Sin datos")}}_queueRender(){this._renderQueued||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this._render()}))}_render(){var h;if(!this.shadowRoot)return;const t=this._weather(),e=this._devices(),i=this._kind(),a=this._panel.title||(i==="recording"?"Sala de grabación":i==="control"?"Control general":"Oficinas"),r=this._panel.subtitle||"Control operativo",s=e.filter(g=>this._isOn(g.entity)).length,o=this._powerActive(e),n=this._powerTotal(e),d=new Date,l=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit"}).format(d);this.setAttribute("data-kind",this._kind()),this.setAttribute("data-theme",this._theme),this.shadowRoot.innerHTML=`
      ...
      <div class="operations-shell">
        <header class="topbar">
          <div class="topbar-start">
            <button class="menu-button" data-action="toggle-menu" aria-label="Abrir menú de navegación de Home Assistant" title="Abrir menú">${he}</button>
          ...
          <div class="topbar-meta">
            <time class="header-clock"><strong>${this._escape(l)}</strong></time>
            <div class="header-weather" aria-label="Clima actual: ${this._escape(t.condition)}, ${this._escape(t.temperature)}°"><span aria-hidden="true">${this._escape(de[String(((h=this._state(this._panel.weather||"weather.forecast_casa"))==null?void 0:h.state)||"")]||"·")}</span><strong>${this._escape(t.temperature)}°</strong></div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar tema" title="Cambiar tema">${pe}</button>
          </div>
        </header>
        <main class="dashboard">
          <section class="workspace-heading"><div><span class="section-kicker">${this._escape(r)}</span><h1>${this._escape(a)}</h1></div><span class="workspace-summary">${e.length} circuitos · ${W(n)}</span></section>
    ...
    </style><div class="energy-panel"><header class="topbar"><div class="topbar-start"><button class="menu-button" data-action="toggle-menu" aria-label="Abrir menú de Home Assistant">${ve}</button><div class="brand"><strong>WITMIND</strong><span>WTX · MDTC</span></div></div><div class="status-strip"><div class="status-pill">${at}<span><strong data-live-active>${this._activeCount()} de ${this._circuits().length}</strong><small>Circuitos activos</small></span></div><div class="status-pill">${at}<span><strong data-live-power>${this._formatPower(this._currentPower())}</strong><small>Potencia actual</small></span></div><div class="status-pill">${at}<span><strong>${this._formatPower(this._installedPower())}</strong><small>Potencia conocida</small></span></div></div><div class="topbar-end"><time class="clock">${this._escape(e)}</time><button class="theme-button" data-action="toggle-theme" aria-label="Cambiar tema">${ke}</button></div></header><main class="dashboard"><section class="workspace-heading"><div><span class="eyebrow">Analítica del edificio</span><h1>Gestión de energía</h1><p>Consumo estimado, horas de uso y escenarios de ahorro para Showroom, Lobby, Oficinas y Sala de grabación.</p></div><div class="toolbar"><div class="range-tabs" role="tablist" aria-label="Período de análisis">${["day","week","month"].map(a=>`<button data-action="range" data-range="${a}" class="${this._range===a?"is-active":""}" role="tab" aria-selected="${this._range===a}">${a==="day"?"24 h":a==="week"?"7 días":"30 días"}</button>`).join("")}</div><button class="icon-button" data-action="refresh" aria-label="Actualizar historial" title="Actualizar">${at}</button></div></section>${this._loading?`<section class="surface state-card"><div><div class="loader"></div><strong>Cargando historial energético</strong><span>Consultando ${this._circuits().length} circuitos para ${this._escape(t.title.toLowerCase())}.</span></div></section>`:this._error?`<section class="surface state-card"><div><strong>No se pudo cargar el historial</strong><span>${this._escape(this._error)}. Usa Actualizar para volver a intentarlo.</span></div></section>`:i?this._renderReport(i):'<section class="surface state-card"><div><strong>Sin datos disponibles</strong><span>Home Assistant todavía no devolvió historial para este período.</span></div></section>'}</main></div>`}}customElements.get("witmind-energy-panel")||customElements.define("witmind-energy-panel",$e);/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const rt=globalThis,vt=rt.ShadowRoot&&(rt.ShadyCSS===void 0||rt.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,kt=Symbol(),Nt=new WeakMap;let Kt=class{constructor(t,e,i){if(this._$cssResult$=!0,i!==kt)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=t,this.t=e}get styleSheet(){let t=this.o;const e=this.t;if(vt&&t===void 0){const i=e!==void 0&&e.length===1;i&&(t=Nt.get(e)),t===void 0&&((this.o=t=new CSSStyleSheet).replaceSync(this.cssText),i&&Nt.set(e,t))}return t}toString(){return this.cssText}};const Se=c=>new Kt(typeof c=="string"?c:c+"",void 0,kt),Ae=(c,...t)=>{const e=c.length===1?c[0]:t.reduce((i,a,r)=>i+(s=>{if(s._$cssResult$===!0)return s.cssText;if(typeof s=="number")return s;throw Error("Value passed to 'css' function must be a 'css' function result: "+s+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(a)+c[r+1],c[0]);return new Kt(e,c,kt)},Ee=(c,t)=>{if(vt)c.adoptedStyleSheets=t.map(e=>e instanceof CSSStyleSheet?e:e.styleSheet);else for(const e of t){const i=document.createElement("style"),a=rt.litNonce;a!==void 0&&i.setAttribute("nonce",a),i.textContent=e.cssText,c.appendChild(i)}},Lt=vt?c=>c:c=>c instanceof CSSStyleSheet?(t=>{let e="";for(const i of t.cssRules)e+=i.cssText;return Se(e)})(c):c;/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const{is:Me,defineProperty:Ce,getOwnPropertyDescriptor:Te,getOwnPropertyNames:ze,getOwnPropertySymbols:Ne,getPrototypeOf:Le}=Object,P=globalThis,Dt=P.trustedTypes,De=Dt?Dt.emptyScript:"",lt=P.reactiveElementPolyfillSupport,G=(c,t)=>c,mt={toAttribute(c,t){switch(t){case Boolean:c=c?De:null;break;case Object:case Array:c=c==null?c:JSON.stringify(c)}return c},fromAttribute(c,t){let e=c;switch(t){case Boolean:e=c!==null;break;case Number:e=c===null?null:Number(c);break;case Object:case Array:try{e=JSON.parse(c)}catch{e=null}}return e}},Qt=(c,t)=>!Me(c,t),It={attribute:!0,type:String,converter:mt,reflect:!1,useDefault:!1,hasChanged:Qt};Symbol.metadata??(Symbol.metadata=Symbol("metadata")),P.litPropertyMetadata??(P.litPropertyMetadata=new WeakMap);let q=class extends HTMLElement{static addInitializer(t){this._$Ei(),(this.l??(this.l=[])).push(t)}static get observedAttributes(){return this.finalize(),this._$Eh&&[...this._$Eh.keys()]}static createProperty(t,e=It){if(e.state&&(e.attribute=!1),this._$Ei(),this.prototype.hasOwnProperty(t)&&((e=Object.create(e)).wrapped=!0),this.elementProperties.set(t,e),!e.noAccessor){const i=Symbol(),a=this.getPropertyDescriptor(t,i,e);a!==void 0&&Ce(this.prototype,t,a)}}static getPropertyDescriptor(t,e,i){const{get:a,set:r}=Te(this.prototype,t)??{get(){return this[e]},set(s){this[e]=s}};return{get:a,set(s){const o=a==null?void 0:a.call(this);r==null||r.call(this,s),this.requestUpdate(t,o,i)},configurable:!0,enumerable:!0}}static getPropertyOptions(t){return this.elementProperties.get(t)??It}static _$Ei(){if(this.hasOwnProperty(G("elementProperties")))return;const t=Le(this);t.finalize(),t.l!==void 0&&(this.l=[...t.l]),this.elementProperties=new Map(t.elementProperties)}static finalize(){if(this.hasOwnProperty(G("finalized")))return;if(this.finalized=!0,this._$Ei(),this.hasOwnProperty(G("properties"))){const e=this.properties,i=[...ze(e),...Ne(e)];for(const a of i)this.createProperty(a,e[a])}const t=this[Symbol.metadata];if(t!==null){const e=litPropertyMetadata.get(t);if(e!==void 0)for(const[i,a]of e)this.elementProperties.set(i,a)}this._$Eh=new Map;for(const[e,i]of this.elementProperties){const a=this._$Eu(e,i);a!==void 0&&this._$Eh.set(a,e)}this.elementStyles=this.finalizeStyles(this.styles)}static finalizeStyles(t){const e=[];if(Array.isArray(t)){const i=new Set(t.flat(1/0).reverse());for(const a of i)e.unshift(Lt(a))}else t!==void 0&&e.push(Lt(t));return e}static _$Eu(t,e){const i=e.attribute;return i===!1?void 0:typeof i=="string"?i:typeof t=="string"?t.toLowerCase():void 0}constructor(){super(),this._$Ep=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this._$Em=null,this._$Ev()}_$Ev(){var t;this._$ES=new Promise(e=>this.enableUpdating=e),this._$AL=new Map,this._$E_(),this.requestUpdate(),(t=this.constructor.l)==null||t.forEach(e=>e(this))}addController(t){var e;(this._$EO??(this._$EO=new Set)).add(t),this.renderRoot!==void 0&&this.isConnected&&((e=t.hostConnected)==null||e.call(t))}removeController(t){var e;(e=this._$EO)==null||e.delete(t)}_$E_(){const t=new Map,e=this.constructor.elementProperties;for(const i of e.keys())this.hasOwnProperty(i)&&(t.set(i,this[i]),delete this[i]);t.size>0&&(this._$Ep=t)}createRenderRoot(){const t=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return Ee(t,this.constructor.elementStyles),t}connectedCallback(){var t;this.renderRoot??(this.renderRoot=this.createRenderRoot()),this.enableUpdating(!0),(t=this._$EO)==null||t.forEach(e=>{var i;return(i=e.hostConnected)==null?void 0:i.call(e)})}enableUpdating(t){}disconnectedCallback(){var t;(t=this._$EO)==null||t.forEach(e=>{var i;return(i=e.hostDisconnected)==null?void 0:i.call(e)})}attributeChangedCallback(t,e,i){this._$AK(t,i)}_$ET(t,e){var r;const i=this.constructor.elementProperties.get(t),a=this.constructor._$Eu(t,i);if(a!==void 0&&i.reflect===!0){const s=(((r=i.converter)==null?void 0:r.toAttribute)!==void 0?i.converter:mt).toAttribute(e,i.type);this._$Em=t,s==null?this.removeAttribute(a):this.setAttribute(a,s),this._$Em=null}}_$AK(t,e){var r,s;const i=this.constructor,a=i._$Eh.get(t);if(a!==void 0&&this._$Em!==a){const o=i.getPropertyOptions(a),n=typeof o.converter=="function"?{fromAttribute:o.converter}:((r=o.converter)==null?void 0:r.fromAttribute)!==void 0?o.converter:mt;this._$Em=a;const d=n.fromAttribute(e,o.type);this[a]=d??((s=this._$Ej)==null?void 0:s.get(a))??d,this._$Em=null}}requestUpdate(t,e,i,a=!1,r){var s;if(t!==void 0){const o=this.constructor;if(a===!1&&(r=this[t]),i??(i=o.getPropertyOptions(t)),!((i.hasChanged??Qt)(r,e)||i.useDefault&&i.reflect&&r===((s=this._$Ej)==null?void 0:s.get(t))&&!this.hasAttribute(o._$Eu(t,i))))return;this.C(t,e,i)}this.isUpdatePending===!1&&(this._$ES=this._$EP())}C(t,e,{useDefault:i,reflect:a,wrapped:r},s){i&&!(this._$Ej??(this._$Ej=new Map)).has(t)&&(this._$Ej.set(t,s??e??this[t]),r!==!0||s!==void 0)||(this._$AL.has(t)||(this.hasUpdated||i||(e=void 0),this._$AL.set(t,e)),a===!0&&this._$Em!==t&&(this._$Eq??(this._$Eq=new Set)).add(t))}async _$EP(){this.isUpdatePending=!0;try{await this._$ES}catch(e){Promise.reject(e)}const t=this.scheduleUpdate();return t!=null&&await t,!this.isUpdatePending}scheduleUpdate(){return this.performUpdate()}performUpdate(){var i;if(!this.isUpdatePending)return;if(!this.hasUpdated){if(this.renderRoot??(this.renderRoot=this.createRenderRoot()),this._$Ep){for(const[r,s]of this._$Ep)this[r]=s;this._$Ep=void 0}const a=this.constructor.elementProperties;if(a.size>0)for(const[r,s]of a){const{wrapped:o}=s,n=this[r];o!==!0||this._$AL.has(r)||n===void 0||this.C(r,void 0,s,n)}}let t=!1;const e=this._$AL;try{t=this.shouldUpdate(e),t?(this.willUpdate(e),(i=this._$EO)==null||i.forEach(a=>{var r;return(r=a.hostUpdate)==null?void 0:r.call(a)}),this.update(e)):this._$EM()}catch(a){throw t=!1,this._$EM(),a}t&&this._$AE(e)}willUpdate(t){}_$AE(t){var e;(e=this._$EO)==null||e.forEach(i=>{var a;return(a=i.hostUpdated)==null?void 0:a.call(i)}),this.hasUpdated||(this.hasUpdated=!0,this.firstUpdated(t)),this.updated(t)}_$EM(){this._$AL=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this._$ES}shouldUpdate(t){return!0}update(t){this._$Eq&&(this._$Eq=this._$Eq.forEach(e=>this._$ET(e,this[e]))),this._$EM()}updated(t){}firstUpdated(t){}};q.elementStyles=[],q.shadowRootOptions={mode:"open"},q[G("elementProperties")]=new Map,q[G("finalized")]=new Map,lt==null||lt({ReactiveElement:q}),(P.reactiveElementVersions??(P.reactiveElementVersions=[])).push("2.1.2");/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const Z=globalThis,Pt=c=>c,st=Z.trustedTypes,Ot=st?st.createPolicy("lit-html",{createHTML:c=>c}):void 0,Jt="$lit$",D=`lit$${Math.random().toFixed(9).slice(2)}$`,te="?"+D,Ie=`<${te}>`,H=document,Q=()=>H.createComment(""),J=c=>c===null||typeof c!="object"&&typeof c!="function",$t=Array.isArray,Pe=c=>$t(c)||typeof(c==null?void 0:c[Symbol.iterator])=="function",dt=`[ 	
\f\r]`,V=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,Rt=/-->/g,Ft=/>/g,O=RegExp(`>|${dt}(?:([^\\s"'>=/]+)(${dt}*=${dt}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`,"g"),Ht=/'/g,Wt=/"/g,ee=/^(?:script|style|textarea|title)$/i,Oe=c=>(t,...e)=>({_$litType$:c,strings:t,values:e}),C=Oe(1),U=Symbol.for("lit-noChange"),S=Symbol.for("lit-nothing"),qt=new WeakMap,R=H.createTreeWalker(H,129);function ie(c,t){if(!$t(c)||!c.hasOwnProperty("raw"))throw Error("invalid template strings array");return Ot!==void 0?Ot.createHTML(t):t}const Re=(c,t)=>{const e=c.length-1,i=[];let a,r=t===2?"<svg>":t===3?"<math>":"",s=V;for(let o=0;o<e;o++){const n=c[o];let d,l,p=-1,h=0;for(;h<n.length&&(s.lastIndex=h,l=s.exec(n),l!==null);)h=s.lastIndex,s===V?l[1]==="!--"?s=Rt:l[1]!==void 0?s=Ft:l[2]!==void 0?(ee.test(l[2])&&(a=RegExp("</"+l[2],"g")),s=O):l[3]!==void 0&&(s=O):s===O?l[0]===">"?(s=a??V,p=-1):l[1]===void 0?p=-2:(p=s.lastIndex-l[2].length,d=l[1],s=l[3]===void 0?O:l[3]==='"'?Wt:Ht):s===Wt||s===Ht?s=O:s===Rt||s===Ft?s=V:(s=O,a=void 0);const g=s===O&&c[o+1].startsWith("/>")?" ":"";r+=s===V?n+Ie:p>=0?(i.push(d),n.slice(0,p)+Jt+n.slice(p)+D+g):n+D+(p===-2?o:g)}return[ie(c,r+(c[e]||"<?>")+(t===2?"</svg>":t===3?"</math>":"")),i]};class tt{constructor({strings:t,_$litType$:e},i){let a;this.parts=[];let r=0,s=0;const o=t.length-1,n=this.parts,[d,l]=Re(t,e);if(this.el=tt.createElement(d,i),R.currentNode=this.el.content,e===2||e===3){const p=this.el.content.firstChild;p.replaceWith(...p.childNodes)}for(;(a=R.nextNode())!==null&&n.length<o;){if(a.nodeType===1){if(a.hasAttributes())for(const p of a.getAttributeNames())if(p.endsWith(Jt)){const h=l[s++],g=a.getAttribute(p).split(D),u=/([.?@])?(.*)/.exec(h);n.push({type:1,index:r,name:u[2],strings:g,ctor:u[1]==="."?He:u[1]==="?"?We:u[1]==="@"?qe:nt}),a.removeAttribute(p)}else p.startsWith(D)&&(n.push({type:6,index:r}),a.removeAttribute(p));if(ee.test(a.tagName)){const p=a.textContent.split(D),h=p.length-1;if(h>0){a.textContent=st?st.emptyScript:"";for(let g=0;g<h;g++)a.append(p[g],Q()),R.nextNode(),n.push({type:2,index:++r});a.append(p[h],Q())}}}else if(a.nodeType===8)if(a.data===te)n.push({type:2,index:r});else{let p=-1;for(;(p=a.data.indexOf(D,p+1))!==-1;)n.push({type:7,index:r}),p+=D.length-1}r++}}static createElement(t,e){const i=H.createElement("template");return i.innerHTML=t,i}}function B(c,t,e=c,i){var s,o;if(t===U)return t;let a=i!==void 0?(s=e._$Co)==null?void 0:s[i]:e._$Cl;const r=J(t)?void 0:t._$litDirective$;return(a==null?void 0:a.constructor)!==r&&((o=a==null?void 0:a._$AO)==null||o.call(a,!1),r===void 0?a=void 0:(a=new r(c),a._$AT(c,e,i)),i!==void 0?(e._$Co??(e._$Co=[]))[i]=a:e._$Cl=a),a!==void 0&&(t=B(c,a._$AS(c,t.values),a,i)),t}class Fe{constructor(t,e){this._$AV=[],this._$AN=void 0,this._$AD=t,this._$AM=e}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(t){const{el:{content:e},parts:i}=this._$AD,a=((t==null?void 0:t.creationScope)??H).importNode(e,!0);R.currentNode=a;let r=R.nextNode(),s=0,o=0,n=i[0];for(;n!==void 0;){if(s===n.index){let d;n.type===2?d=new et(r,r.nextSibling,this,t):n.type===1?d=new n.ctor(r,n.name,n.strings,this,t):n.type===6&&(d=new je(r,this,t)),this._$AV.push(d),n=i[++o]}s!==(n==null?void 0:n.index)&&(r=R.nextNode(),s++)}return R.currentNode=H,a}p(t){let e=0;for(const i of this._$AV)i!==void 0&&(i.strings!==void 0?(i._$AI(t,i,e),e+=i.strings.length-2):i._$AI(t[e])),e++}}class et{get _$AU(){var t;return((t=this._$AM)==null?void 0:t._$AU)??this._$Cv}constructor(t,e,i,a){this.type=2,this._$AH=S,this._$AN=void 0,this._$AA=t,this._$AB=e,this._$AM=i,this.options=a,this._$Cv=(a==null?void 0:a.isConnected)??!0}get parentNode(){let t=this._$AA.parentNode;const e=this._$AM;return e!==void 0&&(t==null?void 0:t.nodeType)===11&&(t=e.parentNode),t}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(t,e=this){t=B(this,t,e),J(t)?t===S||t==null||t===""?(this._$AH!==S&&this._$AR(),this._$AH=S):t!==this._$AH&&t!==U&&this._(t):t._$litType$!==void 0?this.$(t):t.nodeType!==void 0?this.T(t):Pe(t)?this.k(t):this._(t)}O(t){return this._$AA.parentNode.insertBefore(t,this._$AB)}T(t){this._$AH!==t&&(this._$AR(),this._$AH=this.O(t))}_(t){this._$AH!==S&&J(this._$AH)?this._$AA.nextSibling.data=t:this.T(H.createTextNode(t)),this._$AH=t}$(t){var r;const{values:e,_$litType$:i}=t,a=typeof i=="number"?this._$AC(t):(i.el===void 0&&(i.el=tt.createElement(ie(i.h,i.h[0]),this.options)),i);if(((r=this._$AH)==null?void 0:r._$AD)===a)this._$AH.p(e);else{const s=new Fe(a,this),o=s.u(this.options);s.p(e),this.T(o),this._$AH=s}}_$AC(t){let e=qt.get(t.strings);return e===void 0&&qt.set(t.strings,e=new tt(t)),e}k(t){$t(this._$AH)||(this._$AH=[],this._$AR());const e=this._$AH;let i,a=0;for(const r of t)a===e.length?e.push(i=new et(this.O(Q()),this.O(Q()),this,this.options)):i=e[a],i._$AI(r),a++;a<e.length&&(this._$AR(i&&i._$AB.nextSibling,a),e.length=a)}_$AR(t=this._$AA.nextSibling,e){var i;for((i=this._$AP)==null?void 0:i.call(this,!1,!0,e);t!==this._$AB;){const a=Pt(t).nextSibling;Pt(t).remove(),t=a}}setConnected(t){var e;this._$AM===void 0&&(this._$Cv=t,(e=this._$AP)==null||e.call(this,t))}}class nt{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(t,e,i,a,r){this.type=1,this._$AH=S,this._$AN=void 0,this.element=t,this.name=e,this._$AM=a,this.options=r,i.length>2||i[0]!==""||i[1]!==""?(this._$AH=Array(i.length-1).fill(new String),this.strings=i):this._$AH=S}_$AI(t,e=this,i,a){const r=this.strings;let s=!1;if(r===void 0)t=B(this,t,e,0),s=!J(t)||t!==this._$AH&&t!==U,s&&(this._$AH=t);else{const o=t;let n,d;for(t=r[0],n=0;n<r.length-1;n++)d=B(this,o[i+n],e,n),d===U&&(d=this._$AH[n]),s||(s=!J(d)||d!==this._$AH[n]),d===S?t=S:t!==S&&(t+=(d??"")+r[n+1]),this._$AH[n]=d}s&&!a&&this.j(t)}j(t){t===S?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,t??"")}}class He extends nt{constructor(){super(...arguments),this.type=3}j(t){this.element[this.name]=t===S?void 0:t}}class We extends nt{constructor(){super(...arguments),this.type=4}j(t){this.element.toggleAttribute(this.name,!!t&&t!==S)}}class qe extends nt{constructor(t,e,i,a,r){super(t,e,i,a,r),this.type=5}_$AI(t,e=this){if((t=B(this,t,e,0)??S)===U)return;const i=this._$AH,a=t===S&&i!==S||t.capture!==i.capture||t.once!==i.once||t.passive!==i.passive,r=t!==S&&(i===S||a);a&&this.element.removeEventListener(this.name,this,i),r&&this.element.addEventListener(this.name,this,t),this._$AH=t}handleEvent(t){var e;typeof this._$AH=="function"?this._$AH.call(((e=this.options)==null?void 0:e.host)??this.element,t):this._$AH.handleEvent(t)}}class je{constructor(t,e,i){this.element=t,this.type=6,this._$AN=void 0,this._$AM=e,this.options=i}get _$AU(){return this._$AM._$AU}_$AI(t){B(this,t)}}const ht=Z.litHtmlPolyfillSupport;ht==null||ht(tt,et),(Z.litHtmlVersions??(Z.litHtmlVersions=[])).push("3.3.3");const Ue=(c,t,e)=>{const i=(e==null?void 0:e.renderBefore)??t;let a=i._$litPart$;if(a===void 0){const r=(e==null?void 0:e.renderBefore)??null;i._$litPart$=a=new et(t.insertBefore(Q(),r),r,void 0,e??{})}return a._$AI(c),a};/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const F=globalThis;class K extends q{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){var e;const t=super.createRenderRoot();return(e=this.renderOptions).renderBefore??(e.renderBefore=t.firstChild),t}update(t){const e=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(t),this._$Do=Ue(e,this.renderRoot,this.renderOptions)}connectedCallback(){var t;super.connectedCallback(),(t=this._$Do)==null||t.setConnected(!0)}disconnectedCallback(){var t;super.disconnectedCallback(),(t=this._$Do)==null||t.setConnected(!1)}render(){return U}}var Zt;K._$litElement$=!0,K.finalized=!0,(Zt=F.litElementHydrateSupport)==null||Zt.call(F,{LitElement:K});const pt=F.litElementPolyfillSupport;pt==null||pt({LitElement:K});(F.litElementVersions??(F.litElementVersions=[])).push("4.2.2");/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Be=[["path",{d:"M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Xe=[["path",{d:"m7 7 10 10"}],["path",{d:"M17 7v10H7"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ve=[["path",{d:"M7 7h10v10"}],["path",{d:"M7 17 17 7"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ye=[["path",{d:"M20 6 9 17l-5-5"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ge=[["path",{d:"m6 9 6 6 6-6"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ze=[["path",{d:"m9 18 6-6-6-6"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ke=[["path",{d:"m18 15-6-6-6 6"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Qe=[["circle",{cx:"12",cy:"12",r:"10"}],["line",{x1:"12",x2:"12",y1:"8",y2:"12"}],["line",{x1:"12",x2:"12.01",y1:"16",y2:"16"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Je=[["circle",{cx:"12",cy:"12",r:"10"}],["polyline",{points:"12 6 12 12 16 14"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ti=[["rect",{width:"16",height:"16",x:"4",y:"4",rx:"2"}],["rect",{width:"6",height:"6",x:"9",y:"9",rx:"1"}],["path",{d:"M15 2v2"}],["path",{d:"M15 20v2"}],["path",{d:"M2 15h2"}],["path",{d:"M2 9h2"}],["path",{d:"M20 15h2"}],["path",{d:"M20 9h2"}],["path",{d:"M9 2v2"}],["path",{d:"M9 20v2"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ei=[["path",{d:"M7 16.3c2.2 0 4-1.83 4-4.05 0-1.16-.57-2.26-1.71-3.19S7.29 6.75 7 5.3c-.29 1.45-1.14 2.84-2.29 3.76S3 11.1 3 12.25c0 2.22 1.8 4.05 4 4.05z"}],["path",{d:"M12.56 6.6A10.97 10.97 0 0 0 14 3.02c.5 2.5 2 4.9 4 6.5s3 3.5 3 5.5a6.98 6.98 0 0 1-11.91 4.97"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ii=[["path",{d:"M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"}],["circle",{cx:"12",cy:"12",r:"3"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const si=[["rect",{width:"18",height:"18",x:"3",y:"3",rx:"2"}],["path",{d:"M3 9h18"}],["path",{d:"M3 15h18"}],["path",{d:"M9 3v18"}],["path",{d:"M15 3v18"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ni=[["path",{d:"M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"}],["path",{d:"M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const oi=[["circle",{cx:"12",cy:"12",r:"10"}],["path",{d:"M12 16v-4"}],["path",{d:"M12 8h.01"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ci=[["path",{d:"M12.83 2.18a2 2 0 0 0-1.66 0L2.6 6.08a1 1 0 0 0 0 1.83l8.58 3.91a2 2 0 0 0 1.66 0l8.58-3.9a1 1 0 0 0 0-1.83z"}],["path",{d:"M2 12a1 1 0 0 0 .58.91l8.6 3.91a2 2 0 0 0 1.65 0l8.58-3.9A1 1 0 0 0 22 12"}],["path",{d:"M2 17a1 1 0 0 0 .58.91l8.6 3.91a2 2 0 0 0 1.65 0l8.58-3.9A1 1 0 0 0 22 17"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const li=[["path",{d:"M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"}],["path",{d:"M9 18h6"}],["path",{d:"M10 22h4"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const di=[["polyline",{points:"15 3 21 3 21 9"}],["polyline",{points:"9 21 3 21 3 15"}],["line",{x1:"21",x2:"14",y1:"3",y2:"10"}],["line",{x1:"3",x2:"10",y1:"21",y2:"14"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const hi=[["polyline",{points:"4 14 10 14 10 20"}],["polyline",{points:"20 10 14 10 14 4"}],["line",{x1:"14",x2:"21",y1:"10",y2:"3"}],["line",{x1:"3",x2:"10",y1:"21",y2:"14"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const pi=[["path",{d:"M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const gi=[["path",{d:"M9 18V5l12-2v13"}],["circle",{cx:"6",cy:"18",r:"3"}],["circle",{cx:"18",cy:"16",r:"3"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ui=[["rect",{x:"14",y:"4",width:"4",height:"16",rx:"1"}],["rect",{x:"6",y:"4",width:"4",height:"16",rx:"1"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const mi=[["polygon",{points:"6 3 20 12 6 21 6 3"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const _i=[["path",{d:"M4.9 19.1C1 15.2 1 8.8 4.9 4.9"}],["path",{d:"M7.8 16.2c-2.3-2.3-2.3-6.1 0-8.5"}],["circle",{cx:"12",cy:"12",r:"2"}],["path",{d:"M16.2 7.8c2.3 2.3 2.3 6.1 0 8.5"}],["path",{d:"M19.1 4.9C23 8.8 23 15.1 19.1 19"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const bi=[["path",{d:"M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"}],["path",{d:"M21 3v5h-5"}],["path",{d:"M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"}],["path",{d:"M8 16H3v5"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const yi=[["path",{d:"M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"}],["circle",{cx:"12",cy:"12",r:"3"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const xi=[["path",{d:"M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const wi=[["polygon",{points:"19 20 9 12 19 4 19 20"}],["line",{x1:"5",x2:"5",y1:"19",y2:"5"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const vi=[["polygon",{points:"5 4 15 12 5 20 5 4"}],["line",{x1:"19",x2:"19",y1:"5",y2:"19"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const ki=[["line",{x1:"4",x2:"4",y1:"21",y2:"14"}],["line",{x1:"4",x2:"4",y1:"10",y2:"3"}],["line",{x1:"12",x2:"12",y1:"21",y2:"12"}],["line",{x1:"12",x2:"12",y1:"8",y2:"3"}],["line",{x1:"20",x2:"20",y1:"21",y2:"16"}],["line",{x1:"20",x2:"20",y1:"12",y2:"3"}],["line",{x1:"2",x2:"6",y1:"14",y2:"14"}],["line",{x1:"10",x2:"14",y1:"8",y2:"8"}],["line",{x1:"18",x2:"22",y1:"16",y2:"16"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const $i=[["path",{d:"M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"}],["path",{d:"M20 3v4"}],["path",{d:"M22 5h-4"}],["path",{d:"M4 17v2"}],["path",{d:"M5 18H3"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Si=[["circle",{cx:"12",cy:"12",r:"4"}],["path",{d:"M12 2v2"}],["path",{d:"M12 20v2"}],["path",{d:"m4.93 4.93 1.41 1.41"}],["path",{d:"m17.66 17.66 1.41 1.41"}],["path",{d:"M2 12h2"}],["path",{d:"M20 12h2"}],["path",{d:"m6.34 17.66-1.41 1.41"}],["path",{d:"m19.07 4.93-1.41 1.41"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ei=[["polyline",{points:"22 7 13.5 15.5 8.5 10.5 2 17"}],["polyline",{points:"16 7 22 7 22 13"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Mi=[["path",{d:"m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"}],["path",{d:"M12 9v4"}],["path",{d:"M12 17h.01"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ci=[["rect",{width:"20",height:"15",x:"2",y:"7",rx:"2",ry:"2"}],["polyline",{points:"17 2 12 7 7 2"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const zi=[["path",{d:"M11 4.702a.705.705 0 0 0-1.203-.498L6.413 7.587A1.4 1.4 0 0 1 5.416 8H3a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1h2.416a1.4 1.4 0 0 1 .997.413l3.383 3.384A.705.705 0 0 0 11 19.298z"}],["line",{x1:"22",x2:"16",y1:"9",y2:"15"}],["line",{x1:"16",x2:"22",y1:"9",y2:"15"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Ni=[["path",{d:"M12 20h.01"}],["path",{d:"M8.5 16.429a5 5 0 0 1 7 0"}],["path",{d:"M5 12.859a10 10 0 0 1 5.17-2.69"}],["path",{d:"M19 12.859a10 10 0 0 0-2.007-1.523"}],["path",{d:"M2 8.82a15 15 0 0 1 4.177-2.643"}],["path",{d:"M22 8.82a15 15 0 0 0-11.288-3.764"}],["path",{d:"m2 2 20 20"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Li=[["path",{d:"M12 20h.01"}],["path",{d:"M2 8.82a15 15 0 0 1 20 0"}],["path",{d:"M5 12.859a10 10 0 0 1 14 0"}],["path",{d:"M8.5 16.429a5 5 0 0 1 7 0"}]];/**
 * @license lucide v0.475.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Pi=[["path",{d:"M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"}]],Oi={home:ni,lightbulb:li,sun:Si,moon:pi,zap:Pi,music:gi,sliders:ki,activity:Be,shield:xi,power:fi,"chevron-right":Ze,"chevron-down":Ge,"chevron-up":Ke,x:Ii,check:Ye,"alert-triangle":Mi,"alert-circle":Qe,clock:Je,thermometer:Ai,droplets:ei,wind:Di,sparkles:$i,play:mi,pause:ui,"skip-forward":vi,"skip-back":wi,"volume-2":Ti,"volume-x":zi,"arrow-up-right":Ve,"arrow-down-right":Xe,layers:ci,tv:Ci,cpu:ti,"refresh-cw":bi,eye:ii,settings:yi,flame:ai,radio:_i,wifi:Li,"wifi-off":Ni,maximize:di,minimize:hi,gauge:ri,"trending-up":Ei,grid:si,info:oi};function y(c,t={}){const e=t.size||22,i=t.strokeWidth||1.8,a=t.color||"currentColor",r=t.className||"",s=Oi[c.toLowerCase()];return s?C`
    <svg
      ...
      class="wit-icon ${r}"
    >
      ${s.map(([o,n])=>o==="path"?C`<path d="${n.d}"></path>`:o==="circle"?C`<circle cx="${n.cx}" cy="${n.cy}" r="${n.r}"></circle>`:o==="line"?C`<line x1="${n.x1}" y1="${n.y1}" x2="${n.x2}" y2="${n.y2}"></line>`:o==="rect"?C`<rect width="${n.width}" height="${n.height}" x="${n.x}" y="${n.y}" rx="${n.rx||0}"></rect>`:o==="polygon"?C`<polygon points="${n.points}"></polygon>`:o==="polyline"?C`<polyline points="${n.points}"></polyline>`:null)}
    </svg>
  `:C`
      <svg
        xmlns="http://www.w3.org/2000/svg"
    ...
    `}const I={weather:"weather.forecast_casa",showroomPower:"sensor.showroom_potencia_activa",showroomEnergy:"sensor.showroom_energia_activa"},ft=[{id:"ground.showroom",floor:"ground",label:"Showroom",power:I.showroomPower,circuits:[{entity:"switch.interruptor_inteligente_switch_1",label:"Spots ventana",watts:100},{entity:"switch.interruptor_inteligente_switch_2",label:"Spots 2x3",watts:120},{entity:"switch.interruptor_inteligente_switch_3",label:"Spots 3x3",watts:180},{entity:"switch.interruptor_inteligente_switch_4",label:"Spots TV",watts:25},{entity:"switch.interruptor_inteligente_2_switch_1",label:"Paneles 3k/6k",watts:96},{entity:"switch.interruptor_inteligente_2_switch_2",label:"Colgantes",watts:10},{entity:"switch.interruptor_inteligente_2_switch_3",label:"Slims",watts:432},{entity:"switch.interruptor_inteligente_2_switch_4",label:"Downlights",watts:144},{entity:"switch.smart_relay_switch_4_switch",label:"Paneles",watts:288},{entity:"switch.smart_relay_switch_3_switch",label:"Reflector exterior"}]},{id:"ground.lobby",floor:"ground",label:"Lobby",circuits:[{entity:"switch.interruptor_inteligente_3_switch_1",label:"Central colgante"},{entity:"switch.interruptor_inteligente_3_switch_2",label:"Spots decorativos"},{entity:"switch.interruptor_inteligente_3_switch_3",label:"Tira LED"},{entity:"switch.interruptor_inteligente_3_switch_4",label:"Spots principales"}]},{id:"ground.grabacion",floor:"ground",label:"Grabación",circuits:[{entity:"switch.4gang_switch_sala_grabacion_interruptor_1",label:"Tira LED",watts:24},{entity:"switch.4gang_switch_sala_grabacion_interruptor_2",label:"Paneles",watts:96},{entity:"switch.4gang_switch_sala_grabacion_interruptor_3",label:"Spots",watts:50},{entity:"switch.4gang_switch_sala_grabacion_interruptor_4",label:"Otras luces",watts:30}]},{id:"upper.witronix",floor:"upper",label:"Witronix",temperature:"sensor.t_h_sensor_temperature",humidity:"sensor.t_h_sensor_humidity",circuits:[{entity:"switch.oficina_gerencial_interruptor_1",label:"Witronix LED",watts:48}]},{id:"upper.mindtec",floor:"upper",label:"Mindtec",circuits:[{entity:"switch.oficina_mindtec_interruptor_1",label:"Iluminación Mindtec",watts:48}]},{id:"upper.mindtec_admin",floor:"upper",label:"Mindtec Admin",temperature:"sensor.t_h_sensor_2_temperature",humidity:"sensor.t_h_sensor_2_humidity",circuits:[{entity:"switch.oficina_grande_interruptor_1",label:"Oficina grande 1",watts:168},{entity:"switch.oficina_grande_interruptor_2",label:"Oficina grande 2",watts:168}]},{id:"upper.sala_multiuso",floor:"upper",label:"Sala multiuso",circuits:[{entity:"switch.b2_gang_interruptor_1",label:"Multifuncional",watts:96},{entity:"switch.b2_gang_interruptor_2",label:"Pasillos",watts:117}]},{id:"upper.taller",floor:"upper",label:"Taller",circuits:[{entity:"switch.taller_interruptor_1",label:"Iluminación Taller",watts:144}]}],Ri=[...new Set([I.weather,I.showroomPower,I.showroomEnergy,...ft.flatMap(c=>[c.temperature,c.humidity,c.power,...c.circuits.map(t=>t.entity)])].filter(c=>!!c))],Fi={ground:{label:"Planta Baja",context:"Showroom",image:"./building/planta-baja.png"},upper:{label:"Planta Alta",context:"Taller",image:"./building/planta-alta.png"}},Hi={"clear-night":"Despejado de noche",cloudy:"Nublado",exceptional:"Condición excepcional",fog:"Niebla",hail:"Granizo",lightning:"Tormenta eléctrica","lightning-rainy":"Tormenta y lluvia",partlycloudy:"Parcialmente nublado",pouring:"Lluvia intensa",rainy:"Lluvioso",snowy:"Nevado","snowy-rainy":"Aguanieve",sunny:"Soleado",windy:"Ventoso","windy-variant":"Viento y nubes"},j=c=>!c||c.state==="unknown"||c.state==="unavailable",Y=(c,t)=>{var a;if(j(c))return null;const e=t?(a=c==null?void 0:c.attributes)==null?void 0:a[t]:c==null?void 0:c.state,i=Number(e);return Number.isFinite(i)?i:null},St=class St extends K{constructor(){super(...arguments),this._hass=null,this._panel={},this._theme="dark",this._activeFloor="ground",this._actionState="idle",this._actionMessage="",this._clock=new Date,this._history=[],this._historyTrend=null,this._historyState="idle",this._historyRequested=!1,this._touchStartX=null,this._touchLastX=null,this._mouseStartX=null,this._mouseLastX=null,this._onTouchMove=t=>{if(this._touchStartX===null)return;const e=t.touches[0];e&&(this._touchLastX=e.clientX,Math.abs(e.clientX-this._touchStartX)>12&&t.preventDefault())},this._onTouchEnd=()=>{this._touchStartX!==null&&this._touchLastX!==null&&this._finishFloorSwipe(this._touchLastX-this._touchStartX),this._touchStartX=null,this._touchLastX=null},this._onTouchCancel=()=>{this._touchStartX=null,this._touchLastX=null},this._onPointerMove=t=>{this._mouseStartX!==null&&(this._mouseLastX=t.clientX)},this._onPointerUp=()=>{this._mouseStartX!==null&&this._mouseLastX!==null&&this._finishFloorSwipe(this._mouseLastX-this._mouseStartX),this._mouseStartX=null,this._mouseLastX=null},this._onPointerCancel=()=>{this._mouseStartX=null,this._mouseLastX=null}}set hass(t){const e=this._hass;this._hass=t,this.requestUpdate("hass",e),this.isConnected&&!this._historyRequested&&this._loadHistory()}get hass(){return this._hass}set panel(t){const e=this._panel;this._panel=t&&typeof t=="object"?t:{},this.requestUpdate("panel",e)}get panel(){return this._panel}set theme(t){if(t!=="dark"&&t!=="light")return;const e=this._theme;this._theme=t,this.setAttribute("data-theme",t),this.requestUpdate("theme",e)}get theme(){return this._theme}connectedCallback(){super.connectedCallback(),this.setAttribute("data-theme",this._theme),this._clockTimer=window.setInterval(()=>{this._clock=new Date,this.requestUpdate()},3e4),window.addEventListener("touchmove",this._onTouchMove,{passive:!1}),window.addEventListener("touchend",this._onTouchEnd,{passive:!0}),window.addEventListener("touchcancel",this._onTouchCancel,{passive:!0}),window.addEventListener("pointermove",this._onPointerMove),window.addEventListener("pointerup",this._onPointerUp),window.addEventListener("pointercancel",this._onPointerCancel),this._historyRequested||this._loadHistory()}disconnectedCallback(){super.disconnectedCallback(),this._clockTimer&&window.clearInterval(this._clockTimer),window.removeEventListener("touchmove",this._onTouchMove),window.removeEventListener("touchend",this._onTouchEnd),window.removeEventListener("touchcancel",this._onTouchCancel),window.removeEventListener("pointermove",this._onPointerMove),window.removeEventListener("pointerup",this._onPointerUp),window.removeEventListener("pointercancel",this._onPointerCancel)}updated(t){t.has("theme")&&this.setAttribute("data-theme",this._theme)}_state(t){var e,i;return(i=(e=this._hass)==null?void 0:e.states)==null?void 0:i[t]}_zones(){return ft.filter(t=>t.floor===this._activeFloor)}_availableCircuits(t){return t.circuits.filter(e=>!j(this._state(e.entity)))}_onCircuits(t){return this._availableCircuits(t).filter(e=>{var i;return((i=this._state(e.entity))==null?void 0:i.state)==="on"})}_zonePower(t){const e=t.power?Y(this._state(t.power)):null;if(e!==null)return{value:e,measured:!0};const i=this._onCircuits(t).filter(a=>typeof a.watts=="number");return i.length?{value:i.reduce((a,r)=>a+Number(r.watts),0),measured:!1}:null}_format(t,e,i=0){return t===null?"No disponible":`${new Intl.NumberFormat("es-BO",{maximumFractionDigits:i}).format(t)} ${e}`}_weather(){const t=this._state(I.weather);return{available:!j(t),temperature:Y(t,"temperature"),condition:Hi[String((t==null?void 0:t.state)||"")]||(j(t)?"No disponible":String(t==null?void 0:t.state))}}_environment(){const t=this._zones().map(a=>a.temperature&&Y(this._state(a.temperature))).filter(a=>typeof a=="number"),e=this._zones().map(a=>a.humidity&&Y(this._state(a.humidity))).filter(a=>typeof a=="number"),i=a=>a.length?a.reduce((r,s)=>r+s,0)/a.length:null;return{temperature:i(t),humidity:i(e)}}_activePower(){const t=Y(this._state(I.showroomPower));return t===null?null:t}async _loadHistory(){var i,a;const t=(a=(i=this._hass)==null?void 0:i.connection)==null?void 0:a.sendMessagePromise;if(!t||j(this._state(I.showroomPower))){this._historyState="unavailable",this.requestUpdate();return}this._historyRequested=!0,this._historyState="loading",this.requestUpdate();const e=Date.now();try{const r=await t({type:"history/history_during_period",start_time:new Date(e-1728e5).toISOString(),end_time:new Date(e).toISOString(),entity_ids:[I.showroomPower],minimal_response:!0,no_attributes:!0,significant_changes_only:!1}),s=Array.isArray(r)?Array.isArray(r[0])?r[0]:r:Object.values(r||{})[0],o=(Array.isArray(s)?s:[]).map(h=>({value:Number((h==null?void 0:h.state)??(h==null?void 0:h.s)),time:Date.parse(String((h==null?void 0:h.last_changed)??(h==null?void 0:h.last_updated)??(h==null?void 0:h.lu)??(h==null?void 0:h.lc)??""))})).filter(h=>Number.isFinite(h.value)&&Number.isFinite(h.time)),n=new Map;o.forEach(h=>n.set(Math.floor(h.time/36e5),h.value));const d=Math.floor(e/36e5),l=Array.from({length:24},(h,g)=>n.get(d-23+g)).filter(h=>typeof h=="number"),p=Array.from({length:24},(h,g)=>n.get(d-47+g)).filter(h=>typeof h=="number");if(this._history=l,l.length&&p.length){const h=l.reduce((u,f)=>u+f,0)/l.length,g=p.reduce((u,f)=>u+f,0)/p.length;this._historyTrend=g>0?(h-g)/g*100:null}this._historyState=l.length?"ready":"unavailable"}catch{this._historyState="error"}this.requestUpdate()}_setFloor(t){t!==this._activeFloor&&(this._activeFloor=t,this.requestUpdate())}_onTouchStart(t){const e=t.changedTouches[0]||t.touches[0];e&&(this._touchStartX=e.clientX,this._touchLastX=e.clientX)}_onPointerDown(t){t.pointerType==="touch"||t.button!==0||(this._mouseStartX=t.clientX,this._mouseLastX=t.clientX)}_finishFloorSwipe(t){Math.abs(t)<54||this._setFloor(t<0?"upper":"ground")}async _toggleZone(t){var a;if(!((a=this._hass)!=null&&a.callService)||this._actionState==="loading")return;const e=this._availableCircuits(t);if(!e.length)return;const i=this._onCircuits(t).length!==e.length;this._actionState="loading",this._actionMessage=`${i?"Encendiendo":"Apagando"} ${t.label}`,this.requestUpdate();try{await this._hass.callService("switch",i?"turn_on":"turn_off",{entity_id:e.map(r=>r.entity)}),this._actionState="idle",this._actionMessage=`Orden enviada a ${t.label}`}catch(r){this._actionState="error",this._actionMessage=r instanceof Error?r.message:"No se pudo ejecutar la acción"}this.requestUpdate()}async _setAll(t){var i;if(!((i=this._hass)!=null&&i.callService)||this._actionState==="loading")return;const e=ft.flatMap(a=>this._availableCircuits(a).map(r=>r.entity));if(e.length){this._actionState="loading",this._actionMessage=t?"Encendiendo circuitos disponibles":"Apagando circuitos disponibles",this.requestUpdate();try{await this._hass.callService("switch",t?"turn_on":"turn_off",{entity_id:e}),this._actionState="idle",this._actionMessage="Orden enviada. Esperando confirmación de Home Assistant"}catch(a){this._actionState="error",this._actionMessage=a instanceof Error?a.message:"No se pudo ejecutar la acción"}this.requestUpdate()}}_scrollTo(t){var e;(e=this.renderRoot.querySelector(`#${t}`))==null||e.scrollIntoView({behavior:"smooth",block:"start"})}_toggleMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_renderZoneRow(t){const e=this._availableCircuits(t),i=this._onCircuits(t),a=t.circuits.length-e.length,r=this._zonePower(t),s=e.length?a?"warning":i.length?"normal":"off":"unavailable",o=s==="unavailable"?"No disponible":s==="warning"?"Parcial":s==="normal"?"Normal":"Apagada";return C`
      <div class="circuit-row" role="row">
        <div class="zone-name" role="cell">${y("lightbulb",{size:18})}<strong>${t.label}</strong></div>
        <div role="cell"><span class="status status-${s}"><i></i>${o}</span></div>
        <div role="cell" class="numeric">${r?this._format(r.value,"W"):"No disponible"}${r&&!r.measured?C`<small>nominal</small>`:S}</div>
        <div role="cell" class="numeric">${e.length} / ${t.circuits.length}</div>
        <div role="cell">
          <button class="toggle ${i.length?"is-on":""}" ?disabled=${!e.length||this._actionState==="loading"} @click=${()=>this._toggleZone(t)} aria-label="${i.length?"Apagar":"Encender"} ${t.label}" aria-pressed=${i.length>0}><span></span></button>
        </div>
      </div>`}render(){var p;const t=Fi[this._activeFloor],e=this._weather(),i=this._environment(),a=this._activePower(),r=this._history.length?Math.max(...this._history,1):1,o=(Array.isArray(this._panel.alarm_entities)?this._panel.alarm_entities:[]).map(h=>this._state(h)).filter(h=>h&&!j(h)&&h.state!=="off"),n=!!((p=this._hass)!=null&&p.states&&Object.keys(this._hass.states).length),d=new Intl.DateTimeFormat("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}).format(this._clock),l=new Intl.DateTimeFormat("es-BO",{day:"2-digit",month:"short",year:"numeric"}).format(this._clock);return C`
      <div class="bms-shell">
        <header class="topbar">
          ...
          <div class="header-status"><div class="clock"><strong>${d}</strong><span>${l}</span></div><div class="weather-chip">${y("sun",{size:22})}<span><strong>${this._format(e.temperature,"°C",1)}</strong>${e.condition}</span></div><div class="mode-chip ${n?"ok":"warn"}">${y(n?"check":"alert-circle",{size:18})}<span><small>Estado</small><strong>${n?"Supervisión activa":"Conexión pendiente"}</strong></span></div></div>
        </header>
        ...
        <main class="dashboard-grid">
          <section class="main-column">
            <article id="floor-plan" class="panel floor-card">
              <div class="panel-head"><div class="panel-title">${y("home",{size:19})}<h2>PLANO DEL EDIFICIO <span>${t.context}</span></h2></div><div class="floor-selector" role="group" aria-label="Seleccionar planta"><button class=${this._activeFloor==="ground"?"selected":""} @click=${()=>this._setFloor("ground")}>Planta Baja</button><button class=${this._activeFloor==="upper"?"selected":""} @click=${()=>this._setFloor("upper")}>Planta Alta</button></div></div>
            ...
            <article id="circuits" class="panel circuits-card">
              <div class="panel-head"><div class="panel-title">${y("lightbulb",{size:19})}<h2>CIRCUITOS POR ZONA</h2></div><span class="head-meta">${t.label}</span></div>
              <div class="circuit-table" role="table" aria-label="Circuitos por zona">
                ...
                ${this._zones().map(h=>this._renderZoneRow(h))}
              </div>
            </article>
          </section>
          ...
          <section class="side-column">
            <article id="alerts" class="panel side-card alerts-card">
              ...
              ${o.length?C`<div class="alert-list">${o.map(h=>{var g;return C`<div class="alert-row">${y("alert-triangle",{size:20})}<div><strong>${String(((g=h==null?void 0:h.attributes)==null?void 0:g.friendly_name)||(h==null?void 0:h.entity_id))}</strong><span>${String(h==null?void 0:h.state)}</span></div></div>`})}</div>`:C`<div class="empty-state">${y("shield",{size:26})}<div><strong>Sin alarmas configuradas</strong><span>Conecta entidades en <code>alarm_entities</code> para habilitar esta sección.</span></div></div>`}
            </article>
            ...
            <article id="environment" class="panel side-card">
              ...
              <div class="environment-grid"><div>${y("thermometer",{size:27})}<strong>${this._format(i.temperature,"°C",1)}</strong><span>Interior promedio</span></div><div>${y("droplets",{size:27})}<strong>${this._format(i.humidity,"%",0)}</strong><span>Humedad promedio</span></div><div>${y("sun",{size:27})}<strong>${this._format(e.temperature,"°C",1)}</strong><span>Exterior</span></div><div>${y("activity",{size:27})}<strong class="condition">${e.condition}</strong><span>WTX - MDTC</span></div></div>
            </article>
            ...
            <article id="consumption" class="panel side-card consumption-card">
              ...
              <div class="power-reading">${y("zap",{size:43})}<div><strong>${this._format(a,"W")}</strong><span>${a===null?"Medición no disponible":"Showroom - potencia activa"}</span></div>${this._historyTrend!==null?C`<b class=${this._historyTrend<=0?"trend good":"trend bad"}>${this._historyTrend>0?"+":""}${this._historyTrend.toFixed(0)} %<small>vs. 24 h previas</small></b>`:S}</div>
              ${this._historyState==="ready"?C`<div class="history-bars" aria-label="Histórico real de potencia de las últimas 24 horas">${this._history.map(h=>C`<i style="height:${Math.max(4,h/r*100)}%" title="${this._format(h,"W")}"></i>`)}</div>`:C`<div class="history-empty"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><p>${this._historyState==="loading"?"Cargando histórico real de Home Assistant":this._historyState==="error"?"No se pudo cargar el histórico":"Histórico no disponible para esta medición"}.</p></div>`}
            </article>
            ...
            <article class="panel side-card quick-card">
              ...
              ${this._actionMessage?C`<div class="action-feedback ${this._actionState}">${this._actionState==="loading"?y("refresh-cw",{size:15}):y(this._actionState==="error"?"alert-circle":"check",{size:15})}<span>${this._actionMessage}</span></div>`:S}
            </article>
          </section>
        </main>
      </div>`}};St.styles=Ae`

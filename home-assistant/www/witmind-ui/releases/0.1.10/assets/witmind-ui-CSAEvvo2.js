...
`;class B extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",e=>this._handleClick(e)),this.shadowRoot.addEventListener("scroll",e=>this._handleEnergyScroll(e),!0)}set hass(e){var o,a,l,c;const t=this._hass,r=this._config(),i=this._relevantHassChanged(t,e),s=!t||((o=t.states)==null?void 0:o[r.energySensor])!==((a=e==null?void 0:e.states)==null?void 0:a[r.energySensor]),n=!((l=t==null?void 0:t.states)!=null&&l[r.energySensor])&&!!((c=e==null?void 0:e.states)!=null&&c[r.energySensor]);this._hass=e,i&&this._syncStatesFromHass(),this._started?s&&(n?this._loadEnergyStatistics():this._scheduleEnergyRefresh()):(this._started=!0,this._start()),i&&this._updateStatePresentation()}get hass(){return this._hass}set panel(e){const t=this._config().weather;this._panel=e;const r=this._config().weather;this._started&&t!==r&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._updateStatePresentation())}get panel(){return this._panel}set narrow(e){this._narrow=!!e,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._updateStatePresentation(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const e of this._switchTimers.values())clearTimeout(e);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_updateStatePresentation(){var o;if(!((o=this.shadowRoot)!=null&&o.querySelector(".app-shell"))){this._requestRender();return}const e=a=>{var h;const l=a.dataset.entity;if(!l)return;const c=this._state(l),d=this._visibleSwitchState(l),g=d==="on",p=!c||["unknown","unavailable"].includes(c.state),u=this._pendingSwitches.has(l),m=this._switchErrors.get(l),f=m||(u?d==="on"?"Encendiendo…":"Apagando…":p?"No disponible":g?"Encendido":"Apagado");a.classList.toggle("is-on",g),a.classList.toggle("is-pending",u),a.classList.toggle("is-error",!!m),a.disabled=p,a.setAttribute("aria-pressed",String(g));const $=((h=a.querySelector(".device-copy strong"))==null?void 0:h.textContent)||l;a.setAttribute("aria-label",`${$}: ${f}`);const v=a.querySelector(".device-copy small");v&&(v.textContent=f)};this.shadowRoot.querySelectorAll('[data-action="toggle-switch"]').forEach(e),this.shadowRoot.querySelectorAll('[data-action="run-scene"]').forEach(a=>{const l=this._allScenes().find(u=>u.key===a.dataset.sceneKey);if(!l)return;const c=this._sceneStatus(l),d=this._pendingAction===l.key,g=d?"Aplicando...":c.active?"Activo":c.unavailable?"Sin datos":"Inactivo";a.classList.toggle("is-pending",d),a.classList.toggle("is-active",c.active),a.classList.toggle("is-unavailable",c.unavailable),a.disabled=!!(this._pendingAction&&!d),a.setAttribute("aria-pressed",String(c.active));const p=a.querySelector(".scene-state");p&&(p.textContent=g)});const t=this._config(),r=[...t.spots,...t.samples,...t.reflector?[t.reflector]:[]],i=r.filter(a=>this._visibleSwitchState(a.entity)==="on").length,s=this.shadowRoot.querySelector("[data-lights-summary]");s&&(s.textContent=`${i} de ${r.length}`);const n=this.shadowRoot.querySelector("[data-energy-summary]");n&&(n.textContent=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`)}_handleClick(e){const t=e.target.closest("[data-action]");if(!t||!this._hass)return;const r=t.dataset.action;if(r==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(r==="toggle-theme"){this._toggleTheme();return}if(r==="set-view"){const i=t.dataset.view;["home","lights","energy","system"].includes(i)&&(this._activeView=i,this._requestRender());return}if(r==="toggle-switch"){this._toggleSwitch(t.dataset.entity);return}if(r==="run-scene"){this._runScene(t.dataset.sceneKey||t.dataset.entity,t.dataset.label);return}if(r==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(r==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(r==="general-off"){const i=this._config().generalOffScript;i?this._hass.callService("script","turn_on",{entity_id:i}).catch(s=>{this._notify("No se pudo ejecutar Apagar todo Witmind.","error"),console.error("Error ejecutando el apagado general de Witmind:",s)}):this._notify("El apagado general aún no tiene una entidad configurada.","error");return}if(r==="cancel-power-confirm"){const i=e.target.closest("[data-dialog-card]");if(t.classList.contains("dialog-backdrop")&&i)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(r==="confirm-power"){this._confirmGeneralPower();return}if(r==="clear-scene"){this._clearScene();return}if(r==="media"){this._mediaAction(t.dataset.service);return}if(r==="energy-range"){this._setEnergyRange(t.dataset.range);return}if(r==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(r==="energy-day-next"){this._shiftEnergyDay(1);return}if(r==="energy-day-today"){this._resetEnergyDay();return}r==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(e){console.warn("No se pudo guardar el tema del showroom:",e)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this._requestRender()}_config(){var u;const e=((u=this._panel)==null?void 0:u.config)||{},t=e.panel_kind==="lobby"||e.panel_kind==="general"||e.static_only===!0||e.staticOnly===!0,r=m=>Array.isArray(m)?[...new Set(m.filter(Boolean).map(f=>String(f)))]:[],i=(m,f)=>(Array.isArray(m)&&(m.length||t)?m:f).filter(v=>v==null?void 0:v.entity).map((v,h)=>({entity:String(v.entity),name:v.name||`Dispositivo ${h+1}`,subtitle:v.subtitle||"Iluminación",icon:v.icon||"bulb"})),s=i(e.spots,b.spots),n=i(e.samples||e.muestras,b.samples),o=[...s,...n].map(m=>m.entity),a=r(e.scene_control_entities||e.sceneControlEntities),l=a.length?a:o,c=(m,f,$)=>(Array.isArray(m)&&(m.length||t)?m:f).filter(h=>(h==null?void 0:h.entity)||(h==null?void 0:h.id)||(h==null?void 0:h.key)).map((h,S)=>{const k=h.entity?String(h.entity):"",D=String(h.id||h.key||k||`${$}-${S+1}`),y=f.find(E=>{const R=E.entity?String(E.entity):"",q=String(E.id||E.key||R||"");return k&&R===k||q===D}),M=k||(y!=null&&y.entity?String(y.entity):""),A=String(h.id||h.key||(y==null?void 0:y.id)||M||`${$}-${S+1}`),w=M||`${$}:${A}`,_=Array.isArray(h.on_entities)||Array.isArray(h.onEntities),N=Array.isArray(h.off_entities)||Array.isArray(h.offEntities),C=r(_?h.on_entities||h.onEntities:y==null?void 0:y.onEntities),P=r(N?h.off_entities||h.offEntities:[]),W=N?P.filter(E=>!C.includes(E)):l.filter(E=>!C.includes(E));return{key:w,id:A,entity:M,name:h.name||(y==null?void 0:y.name)||`Escena ${S+1}`,subtitle:h.subtitle||(y==null?void 0:y.subtitle)||"Escena del showroom",icon:h.icon||(y==null?void 0:y.icon)||"presentation",directOnly:h.direct_only??h.directOnly??(y==null?void 0:y.directOnly)??!1,onEntities:C,offEntities:W}}),d=e.reflector||b.reflector,g=Number(e.history_hours??e.historyHours),p=Number(e.chart_hours??e.chartHours);return{title:e.title||b.title,subtitle:e.subtitle||b.subtitle,siteLabel:e.site_label||e.siteLabel||b.siteLabel,logo:e.logo||b.logo,weather:e.weather||b.weather,mediaPlayer:e.media_player||e.mediaPlayer||b.mediaPlayer,lightCountSensor:e.light_count_sensor||e.lightCountSensor||b.lightCountSensor,energySensor:e.energy_sensor||e.energySensor||b.energySensor,batteryLevel:e.battery_level||e.batteryLevel||b.batteryLevel,powerOnScript:e.power_on_script||e.powerOnScript||b.powerOnScript,powerOffScript:e.power_off_script||e.powerOffScript||b.powerOffScript,historyHours:Number.isFinite(g)&&g>0?Math.min(24,g):b.historyHours,chartHours:Number.isFinite(p)&&p>0?Math.min(72,p):b.chartHours,showForecast:e.show_forecast??e.showForecast??b.showForecast,panelKind:e.panel_kind||e.panelKind||"showroom",staticOnly:e.static_only??e.staticOnly??!1,generalOffScript:e.general_off_script||e.generalOffScript||"",spots:s,samples:n,sceneControlEntities:l,reflector:d!=null&&d.entity?{entity:String(d.entity),name:d.name||b.reflector.name,subtitle:d.subtitle||b.reflector.subtitle,icon:d.icon||b.reflector.icon}:null,scenes:c(e.scenes,b.scenes,"scene"),sampleScenes:c(e.sample_scenes||e.sampleScenes,b.sampleScenes,"sample")}}_allDevices(){const e=this._config();return[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]]}_allScenes(e=this._config()){return[...e.scenes,...e.sampleScenes]}_trackedEntities(){const e=this._config();return new Set([...this._allDevices().map(t=>t.entity),...e.sceneControlEntities,...this._allScenes(e).flatMap(t=>[t.entity,...t.onEntities,...t.offEntities]),e.weather,e.mediaPlayer,e.lightCountSensor,e.energySensor,e.batteryLevel,e.powerOnScript,e.powerOffScript].filter(Boolean))}_relevantHassChanged(e,t){var r,i;if(!e||!t)return!0;for(const s of this._trackedEntities())if(((r=e.states)==null?void 0:r[s])!==((i=t.states)==null?void 0:i[s]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var e;if((e=this._hass)!=null&&e.states)for(const t of this._trackedEntities()){const r=this._hass.states[t];r&&this._applyLiveState(t,r)}}_applyLiveState(e,t){if(!t){this._liveStates.delete(e);return}this._liveStates.set(e,t);const r=this._pendingSwitches.get(e);if(r&&t.state===r.desired){this._pendingSwitches.delete(e),this._switchErrors.delete(e);const i=this._switchTimers.get(e);i&&clearTimeout(i),this._switchTimers.delete(e)}}async _fetchCurrentStates(){var e;if((e=this._hass)!=null&&e.callWS)try{const t=await this._hass.callWS({type:"get_states"}),r=this._trackedEntities();for(const i of t||[])r.has(i.entity_id)&&this._applyLiveState(i.entity_id,i);this._updateStatePresentation()}catch(t){console.error("No se pudieron sincronizar los estados del showroom:",t)}}async _subscribeStateChanges(){var e;if(!(!((e=this._hass)!=null&&e.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(t=>{var i;const r=(i=t==null?void 0:t.data)==null?void 0:i.entity_id;!r||!this._trackedEntities().has(r)||(this._applyLiveState(r,t.data.new_state),r===this._config().energySensor&&this._scheduleEnergyRefresh(),this._updateStatePresentation())},"state_changed")}catch(t){console.error("No se pudo suscribir a state_changed:",t)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var t;const e=this._config();if(!(!((t=this._hass)!=null&&t.connection)||!e.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===e.weather)){this._resetForecastSubscription();try{this._forecastEntity=e.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(r=>{this._forecast=Array.isArray(r==null?void 0:r.forecast)?r.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:e.weather})}catch(r){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",r)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(e=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const t=this._energyRefreshInterval(),r=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,i=e?0:this._energyLastLoadedAt?Math.max(1e3,t-r):t;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),i)}_energyRangeDefinition(e=this._energyRange){const t=new Date;let r,i=t,s,n,o,a=!1;if(e==="year")r=new Date(t.getFullYear(),0,1,0,0,0,0),s="month",n=`Año ${t.getFullYear()}`,o="mes";else if(e==="month")r=new Date(t.getFullYear(),t.getMonth(),1,0,0,0,0),s="day",n=t.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),o="día";else{const l=new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0);r=new Date(l.getFullYear(),l.getMonth(),l.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),a=r.getTime()===l.getTime(),i=a?t:new Date(r.getFullYear(),r.getMonth(),r.getDate()+1,0,0,0,0),s="hour",n=r.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),o="hora"}return{start:r,end:i,period:s,title:n,intervalLabel:o,isToday:a}}_energyValueToKWh(e,t){const r=Number(e);if(!Number.isFinite(r))return null;const i=String(t||"kWh").trim().toLowerCase().replaceAll(" ","");return i==="kwh"?r:i==="wh"?r/1e3:i==="mwh"?r*1e3:null}_deriveEnergyStateDeltas(e,t,r){const i=Number(t instanceof Date?t.getTime():t),s=Number(r instanceof Date?r.getTime():r),n=e.filter(l=>Number.isFinite(Number(l.start))&&Number.isFinite(Number(l.state))).sort((l,c)=>Number(l.start)-Number(c.start)),o=[];let a=null;for(const l of n){const c=Number(l.start),d=Number(l.state);if(c<i){a=d;continue}if(c>=s)break;let g=0;if(Number.isFinite(a)){const p=d-a;g=p>=0?p:Math.max(0,d)}o.push({...l,change:g}),a=d}return o}_aggregateEnergyDayRows(e,t,r){var l;const i=new Date(t.start),s=new Date(t.end),n=t.isToday?new Date(s.getFullYear(),s.getMonth(),s.getDate(),s.getHours(),0,0,0):new Date(i.getFullYear(),i.getMonth(),i.getDate(),23,0,0,0),o=new Map;for(let c=new Date(i);c<=n;c.setHours(c.getHours()+1)){const d=c.getTime();o.set(d,{start:d,end:new Date(c.getFullYear(),c.getMonth(),c.getDate(),c.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:t.isToday&&d===n.getTime(),live:!1})}let a=null;for(const c of e){const d=new Date(Number(c.start));if(!Number.isFinite(d.getTime()))continue;const g=new Date(d.getFullYear(),d.getMonth(),d.getDate(),d.getHours(),0,0,0).getTime(),p=o.get(g);p&&(p.change+=Math.max(0,Number(c.change)||0),p.samples+=1,Number.isFinite(c.state)&&(!a||Number(c.end||c.start)>Number(a.end||a.start))&&(a=c))}if(t.isToday){const c=this._energyValueToKWh(r==null?void 0:r.state,(l=r==null?void 0:r.attributes)==null?void 0:l.unit_of_measurement),d=a==null?void 0:a.state,g=Number((a==null?void 0:a.end)||(a==null?void 0:a.start)),p=o.get(n.getTime());if(p&&Number.isFinite(c)&&Number.isFinite(d)&&Number.isFinite(g)&&g>=n.getTime()&&s.getTime()-g>=0&&s.getTime()-g<=900*1e3){const u=c-d;Number.isFinite(u)&&u>=0&&(p.change+=u,p.live=u>0)}}return[...o.values()].filter(c=>c.samples>0||c.live)}_calculateCurrentMonthEnergy(e,t,r){var g;const i=new Date(t),s=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0).getTime(),n=e.filter(p=>Number.isFinite(Number(p.start))&&Number(p.start)>=s).sort((p,u)=>Number(p.start)-Number(u.start));let o=n.reduce((p,u)=>p+Math.max(0,Number(u.change)||0),0),a=null;for(const p of n)Number.isFinite(Number(p.state))&&(!a||Number(p.end||p.start)>Number(a.end||a.start))&&(a=p);const l=this._energyValueToKWh(r==null?void 0:r.state,(g=r==null?void 0:r.attributes)==null?void 0:g.unit_of_measurement),c=Number(a==null?void 0:a.state),d=Number((a==null?void 0:a.end)||(a==null?void 0:a.start));if(Number.isFinite(l)&&Number.isFinite(c)&&Number.isFinite(d)&&d>=s&&d<=i.getTime()&&i.getTime()-d<=7200*1e3){const p=l-c;Number.isFinite(p)&&p>=0&&(o+=p)}return n.length||Number.isFinite(c)?o:null}_aggregateEnergyCalendarRows(e,t,r,i){var d;if(!["month","year"].includes(i))return[];const s=new Map;let n=null;for(const g of e){const p=Number(g.start);if(!Number.isFinite(p))continue;const u=new Date(p),m=i==="month"?new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0).getTime():new Date(u.getFullYear(),u.getMonth(),1,0,0,0,0).getTime(),f=s.get(m)||{start:m,change:0,samples:0,partial:!1,live:!1};f.change+=Math.max(0,Number(g.change)||0),f.samples+=1,s.set(m,f),Number.isFinite(Number(g.state))&&(!n||p>Number(n.start))&&(n=g)}const o=this._energyValueToKWh(r==null?void 0:r.state,(d=r==null?void 0:r.attributes)==null?void 0:d.unit_of_measurement),a=Number(n==null?void 0:n.state),l=Number(n==null?void 0:n.start),c=new Date(t.end);if(Number.isFinite(o)&&Number.isFinite(a)&&Number.isFinite(l)&&l<=c.getTime()&&c.getTime()-l<=7200*1e3){const g=o-a;if(Number.isFinite(g)&&g>=0){const p=i==="month"?new Date(c.getFullYear(),c.getMonth(),c.getDate(),0,0,0,0).getTime():new Date(c.getFullYear(),c.getMonth(),1,0,0,0,0).getTime(),u=s.get(p)||{start:p,change:0,samples:0,partial:!1,live:!1};u.change+=g,u.live=g>0,s.set(p,u)}}return[...s.values()].sort((g,p)=>Number(g.start)-Number(p.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(e=>[Number(e.start),Number(e.change)||0,Number(e.samples)||0,!!e.partial,!!e.live])])}async _loadEnergyStatistics(){var p;if(!((p=this._hass)!=null&&p.connection))return;clearTimeout(this._energyRefreshTimer);const e=this._config(),t=this._state(e.energySensor),r=Number(t==null?void 0:t.state);if(!t){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${e.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(r)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${e.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const i=++this._energyRequestId,s=this._energyViewSignature(),n=this._energyData.length>0,o=this._energyRangeDefinition(),a=new Date,l=new Date(a.getFullYear(),a.getMonth(),1,0,0,0,0),c=new Date(o.start.getTime()-3600*1e3),d=this._energyRange==="year"?"hour":"5minute",g=new Date(l.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,n||this._requestRender();try{const u=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[e.energySensor]});if(i!==this._energyRequestId)return;const f=(Array.isArray(u)?u:[]).find(w=>(w==null?void 0:w.statistic_id)===e.energySensor)||null;if(!f||!f.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const $=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:c.toISOString(),end_time:o.end.toISOString(),statistic_ids:[e.energySensor],period:d,units:{energy:"kWh"},types:["state"]}),v=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:g.toISOString(),end_time:a.toISOString(),statistic_ids:[e.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[h,S]=await Promise.all([$,v]);if(i!==this._energyRequestId)return;const k=w=>(Array.isArray(w==null?void 0:w[e.energySensor])?w[e.energySensor]:[]).map(_=>({start:Number(_.start),end:Number(_.end),change:_.change===void 0||_.change===null?null:Math.max(0,Number(_.change)||0),state:_.state===void 0||_.state===null?null:Number(_.state)})).filter(_=>Number.isFinite(_.start)&&(Number.isFinite(_.state)||Number.isFinite(_.change))),D=k(h),y=this._deriveEnergyStateDeltas(D,o.start,o.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows(y,o,t):this._aggregateEnergyCalendarRows(y,o,t,this._energyRange);const M=k(S),A=this._deriveEnergyStateDeltas(M,l,a);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(A,a,t),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((w,_)=>w+Math.max(0,Number(_.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const w=new Date(a.getFullYear(),a.getMonth(),1,0,0,0,0).getTime(),_=this._energyData.find(N=>Number(N.start)===w);_&&(_.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(u){if(i!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(u==null?void 0:u.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",u)}finally{if(i===this._energyRequestId){this._energyLoading=!1;const u=s!==this._energyViewSignature();!n||u&&this._activeView==="energy"?this._requestRender():this._updateStatePresentation(),this._scheduleEnergyRefresh()}}}_setEnergyRange(e){!["day","month","year"].includes(e)||e===this._energyRange||(this._energyRange=e,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(e){if(this._energyRange!=="day")return;const t=Number(e);if(!Number.isFinite(t)||t===0)return;const r=Math.min(0,this._energyDayOffset+t);r!==this._energyDayOffset&&(this._energyDayOffset=r,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(e,t=2){const r=Number(e);return Number.isFinite(r)?r.toLocaleString("es-BO",{minimumFractionDigits:t,maximumFractionDigits:t}):"--"}_energyLabel(e,t=this._energyRange){const r=new Date(Number(e));return Number.isFinite(r.getTime())?t==="year"?r.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):t==="month"?String(r.getDate()):r.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=t<=0||t-e.scrollLeft<=8}_handleEnergyScroll(e){var i,s;const t=(s=(i=e.target)==null?void 0:i.closest)==null?void 0:s.call(i,"[data-energy-scroll]");if(!t)return;const r=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=r<=0||r-t.scrollLeft<=8}_restoreEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){e.scrollLeft=t,this._energyScrollLeft=e.scrollLeft;return}e.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,t))}_energyChart(){const e=this._energyData;if(this._energyLoading&&!e.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!e.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const t=280,r=14,i=16,s=22,n=42,o=this._energyRange==="day"?64:this._energyRange==="month"?36:58,a=Math.max(620,e.length*o+r+i),l=t-s-n,c=a-r-i,d=Math.max(...e.map(h=>h.change),.001),g=this._energyRange==="day"?10:e.length>24?4:7,p=Math.max(6,(c-g*Math.max(0,e.length-1))/e.length),u=this._energyRange==="month"?Math.max(1,Math.ceil(e.length/10)):this._energyRange==="day"?2:1,m=[0,.25,.5,.75,1],f=m.map(h=>{const S=s+l*(1-h);return`<line x1="${r}" y1="${S.toFixed(1)}" x2="${a-i}" y2="${S.toFixed(1)}" class="energy-grid-line"/>`}).join(""),$=m.map(h=>{const S=s+l*(1-h),k=this._formatEnergy(d*h,d<1?2:1);return`<span class="energy-y-tick" style="top:${S.toFixed(1)}px">${this._escape(k)}</span>`}).join(""),v=e.map((h,S)=>{const k=r+S*(p+g),D=h.change>0?Math.max(2,h.change/d*l):1,y=s+l-D,M=S%u===0||S===e.length-1,A=this._energyLabel(h.start),w=h.partial?" · en curso":"",_=h.partial?`<text x="${(k+p/2).toFixed(1)}" y="${(s+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${h.partial?"is-current":""}"><rect x="${k.toFixed(1)}" y="${y.toFixed(1)}" width="${p.toFixed(1)}" height="${D.toFixed(1)}" rx="${Math.min(4,p/2).toFixed(1)}" class="energy-bar ${h.partial?"is-partial":""}"><title>${this._escape(A)} · ${this._formatEnergy(h.change)} kWh${w}</title></rect>${_}${M?`<text x="${(k+p/2).toFixed(1)}" y="${t-14}" text-anchor="middle" class="energy-axis-text">${this._escape(A)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${$}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${a}" height="${t}" viewBox="0 0 ${a} ${t}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${f}${v}</svg></div></div>`}async _loadHistory(){var o;const e=this._config();if(!((o=this._hass)!=null&&o.callApi))return;const t=Math.max(e.historyHours,e.chartHours),r=new Date(Date.now()-t*60*60*1e3).toISOString(),i=new Date().toISOString(),s=[e.lightCountSensor,...e.spots.map(a=>a.entity),...e.samples.map(a=>a.entity)].filter(Boolean).join(",");if(!s)return;const n=`history/period/${encodeURIComponent(r)}?filter_entity_id=${encodeURIComponent(s)}&end_time=${encodeURIComponent(i)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",n),this._historyError=""}catch(a){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",a)}this._requestRender()}_state(e){var t,r;return this._liveStates.get(e)||((r=(t=this._hass)==null?void 0:t.states)==null?void 0:r[e])}_isUnavailable(e){var r;const t=(r=this._state(e))==null?void 0:r.state;return!t||t==="unknown"||t==="unavailable"}_visibleSwitchState(e){var t,r;return((t=this._pendingSwitches.get(e))==null?void 0:t.desired)||((r=this._state(e))==null?void 0:r.state)||"unavailable"}async _toggleSwitch(e){if(!e||!this._hass)return;const t=this._state(e);if(!t||["unknown","unavailable"].includes(t.state)){this._switchErrors.set(e,"No disponible"),this._updateStatePresentation();return}const i=this._visibleSwitchState(e)==="on"?"off":"on",s=i==="on"?"turn_on":"turn_off",n=e.split(".")[0]||"switch";this._pendingSwitches.set(e,{desired:i,startedAt:Date.now()}),this._switchErrors.delete(e),this._updateStatePresentation();try{await this._hass.callService(n,s,{entity_id:e});const o=this._switchTimers.get(e);o&&clearTimeout(o);const a=setTimeout(()=>this._verifySwitchState(e,i),6e3);this._switchTimers.set(e,a)}catch(o){this._pendingSwitches.delete(e),this._switchErrors.set(e,"La acción falló"),this._updateStatePresentation(),console.error(`Error ejecutando ${s} en ${e}:`,o)}}async _verifySwitchState(e,t){var i;await this._fetchCurrentStates();const r=((i=this._state(e))==null?void 0:i.state)===t;this._pendingSwitches.delete(e),this._switchTimers.delete(e),r?this._switchErrors.delete(e):this._switchErrors.set(e,"Sin confirmación"),this._updateStatePresentation()}_sceneStatus(e){const t=[...e.onEntities.map(s=>({entityId:s,desired:"on"})),...e.offEntities.map(s=>({entityId:s,desired:"off"}))];if(!t.length)return{active:!1,unavailable:!1,mismatches:[]};const r=t.some(({entityId:s})=>this._isUnavailable(s)),i=t.filter(({entityId:s,desired:n})=>{var o;return((o=this._state(s))==null?void 0:o.state)!==n});return{active:!r&&i.length===0,unavailable:r,mismatches:i}}_sceneExpectations(e){return[...e.offEntities.map(t=>({entityId:t,desired:"off"})),...e.onEntities.map(t=>({entityId:t,desired:"on"}))]}_markExpectedStates(e){const t=Date.now();for(const{entityId:r,desired:i}of e)this._pendingSwitches.set(r,{desired:i,startedAt:t}),this._switchErrors.delete(r)}_clearExpectedStates(e){for(const{entityId:t}of e){this._pendingSwitches.delete(t);const r=this._switchTimers.get(t);r&&clearTimeout(r),this._switchTimers.delete(t)}}async _setEntitiesState(e,t){const r=[...new Set((e||[]).filter(Boolean))];if(!r.length)return;const i=new Map;for(const n of r){const o=n.split(".")[0];o&&(i.has(o)||i.set(o,[]),i.get(o).push(n))}const s=t==="on"?"turn_on":"turn_off";for(const[n,o]of i)await this._hass.callService(n,s,{entity_id:o})}async _waitForExpectedStates(e,t=7e3){const r=Date.now()+t;let i=e;for(;Date.now()<r;){if(await this._fetchCurrentStates(),i=e.filter(({entityId:s,desired:n})=>{var o;return((o=this._state(s))==null?void 0:o.state)!==n}),!i.length)return{ok:!0,mismatches:[]};await new Promise(s=>setTimeout(s,450))}return{ok:!1,mismatches:i}}async _runScene(e,t){var o;if(!e||this._pendingAction)return;const r=this._config(),i=this._allScenes(r).find(a=>a.key===e||a.entity===e);if(!i){this._notify("La escena no está configurada.","error");return}const s=this._sceneExpectations(i);this._pendingAction=i.key,this._markExpectedStates(s),this._updateStatePresentation();let n=null;try{if(i.entity&&!i.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:i.entity})}catch(c){n=c,console.warn(`La escena ${i.entity} no respondió; se aplicará el perfil directo.`,c)}await this._setEntitiesState(i.offEntities,"off"),await this._setEntitiesState(i.onEntities,"on");const a=await this._waitForExpectedStates(s);if(!a.ok){const c=a.mismatches.map(d=>d.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${c}`)}const l=n?`${t||"Modo"} aplicado mediante control directo.`:`${t||"Modo"} activo.`;this._notify(l,"success")}catch(a){for(const{entityId:l,desired:c}of s)((o=this._state(l))==null?void 0:o.state)!==c&&this._switchErrors.set(l,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",a)}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _executeGeneralPower(e,t={}){var l;if(!this._hass||this._pendingAction||!["on","off"].includes(e))return!1;const r=this._config(),i=e==="on"?r.powerOnScript:r.powerOffScript,s=r.sceneControlEntities.map(c=>({entityId:c,desired:e})),n=t.successMessage||(e==="on"?"Iluminación general encendida.":"Iluminación general apagada."),o=t.errorMessage||(e==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=i||`direct-power-${e}`,this._markExpectedStates(s),this._updateStatePresentation();let a=null;try{if(i)try{await this._hass.callService("script","turn_on",{entity_id:i})}catch(d){a=d,console.warn(`El script ${i} no respondió; se aplicará el control directo.`,d)}await this._setEntitiesState(r.sceneControlEntities,e);const c=await this._waitForExpectedStates(s);if(!c.ok){const d=c.mismatches.map(g=>g.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${d}`)}return this._confirmAction="",this._notify(n,"success"),!0}catch(c){for(const{entityId:d,desired:g}of s)((l=this._state(d))==null?void 0:l.state)!==g&&this._switchErrors.set(d,"Sin confirmación");return this._notify(o,"error"),console.error("Error ejecutando el control general del showroom:",{error:c,scriptError:a,desired:e}),!1}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _confirmGeneralPower(){const e=this._confirmAction;["on","off"].includes(e)&&await this._executeGeneralPower(e,{successMessage:e==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(e){const t=this._config().mediaPlayer;if(!(!t||!e||this._isUnavailable(t)))try{await this._hass.callService("media_player",e,{entity_id:t})}catch(r){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${e}:`,r)}}_updateToastPresentation(){if(!this.shadowRoot)return;const e=this.shadowRoot.querySelector("[data-toast]");if(!this._toast){e==null||e.remove();return}if(e){e.className=`toast ${this._escape(this._toast.type)}`,e.textContent=this._toast.message;return}const t=document.createElement("div");t.dataset.toast="",t.className=`toast ${this._escape(this._toast.type)}`,t.setAttribute("role","status"),t.textContent=this._toast.message,this.shadowRoot.append(t)}_notify(e,t="success"){var r;clearTimeout(this._toastTimer),this._toast={message:e,type:t},(r=this.shadowRoot)!=null&&r.querySelector(".app-shell")?this._updateToastPresentation():this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._updateToastPresentation()},4200)}_historyMap(){var t;const e=new Map;for(const r of this._history||[]){const i=(t=r==null?void 0:r[0])==null?void 0:t.entity_id;i&&e.set(i,r)}return e}_historySegments(e,t,r){if(!Array.isArray(e)||!e.length)return[];const i=e.map(n=>({state:n.state,time:new Date(n.last_changed||n.last_updated).getTime()})).filter(n=>Number.isFinite(n.time)).sort((n,o)=>n.time-o.time);if(!i.length)return[];const s=[];for(let n=0;n<i.length;n+=1){const o=i[n],a=i[n+1],l=Math.max(t,o.time),c=Math.min(r,(a==null?void 0:a.time)??r);c<=l||s.push({state:o.state,left:(l-t)/(r-t)*100,width:(c-l)/(r-t)*100})}return s}_sparkline(e,t){var u;const r=this._historyMap().get(e)||[],i=Date.now(),s=i-t*60*60*1e3,n=r.map(m=>({value:Number(m.state),time:new Date(m.last_changed||m.last_updated).getTime()})).filter(m=>Number.isFinite(m.value)&&Number.isFinite(m.time)&&m.time>=s).sort((m,f)=>m.time-f.time),o=Number((u=this._state(e))==null?void 0:u.state);if(Number.isFinite(o)&&n.push({value:o,time:i}),!n.length)return{path:"",min:"--",max:"--",avg:"--"};const a=n.map(m=>m.value),l=Math.min(...a),c=Math.max(...a),d=a.reduce((m,f)=>m+f,0)/a.length,g=c-l||1;return{path:n.map((m,f)=>{const $=(m.time-s)/(i-s)*300,v=66-(m.value-l)/g*52;return`${f?"L":"M"}${Math.max(0,Math.min(300,$)).toFixed(1)},${v.toFixed(1)}`}).join(" "),min:this._formatNumber(l),max:this._formatNumber(c),avg:this._formatNumber(d)}}_formatNumber(e){return Number.isFinite(e)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(e):"--"}_updateClock(){var o,a,l;if(!this.shadowRoot)return;const e=new Date,t=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(e),r=((o=t.find(c=>c.type==="hour"))==null?void 0:o.value)||"--",i=((a=t.find(c=>c.type==="minute"))==null?void 0:a.value)||"--",s=(((l=t.find(c=>c.type==="dayPeriod"))==null?void 0:l.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const c of this.shadowRoot.querySelectorAll("[data-clock-time]"))c.textContent=`${r}:${i}`;for(const c of this.shadowRoot.querySelectorAll("[data-clock-period]"))c.textContent=s||"";const n=this.shadowRoot.querySelector("[data-current-time]");n&&(n.dateTime=e.toISOString(),n.setAttribute("aria-label",`${r}:${i} ${s}`.trim()))}_icon(e,t=""){const r=z[e]||z.bulb;return`<svg class="icon ${this._escape(t)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${r}</svg>`}_escape(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(e){const t=this._state(e.entity),r=this._visibleSwitchState(e.entity),i=r==="on",s=!t||["unknown","unavailable"].includes(t.state),n=this._pendingSwitches.has(e.entity),o=this._switchErrors.get(e.entity),a=o||(n?r==="on"?"Encendiendo…":"Apagando…":s?"No disponible":i?"Encendido":"Apagado");return`
      <button
    ...
    `}_renderWeather(){const e=this._config(),t=this._state(e.weather);if(!t)return`
      ...
      `;const r=t.attributes||{},i=t.state,s=this._forecast.slice(0,3);return`
      <section class="surface weather-card">
        ...
        ${e.showForecast?`
          <div class="forecast-row">
            ${s.length?s.map(n=>{const o=new Date(n.datetime),a=new Intl.DateTimeFormat("es-BO",{weekday:"short"}).format(o);return`
                <div class="forecast-item">
                  <span>${this._escape(a)}</span>
    ...
    `}_renderSceneBlock({eyebrow:e,title:t,scenes:r,className:i}){const s=this._config(),n=r.find(c=>this._sceneStatus(c).active),o=this._pendingAction===s.powerOffScript,a=s.sceneControlEntities.some(c=>{var d;return((d=this._state(c))==null?void 0:d.state)==="on"}),l=!!this._pendingAction||!a;return`
    ...
    `}_renderScenes(){const e=this._config(),t=[...e.scenes,...e.sampleScenes],r=t.find(o=>this._sceneStatus(o).active),i=this._pendingAction===e.powerOffScript,s=e.sceneControlEntities.some(o=>{var a;return((a=this._state(o))==null?void 0:a.state)==="on"}),n=!!this._pendingAction||!s;return`
    ...
    `}_renderActivity(){const e=this._energyRangeDefinition(),t=this._energyData.map(a=>Math.max(0,Number(a.change)||0)),r=t.reduce((a,l)=>a+l,0),i=t.length?r/t.length:0,s=t.length?Math.max(...t):0,n=new Date().toLocaleDateString("es-BO",{month:"short",year:"numeric"}).replace(".",""),o=this._energyRange==="day"?`${t.length} ${t.length===1?"hora":"horas"}`:this._energyRange==="month"?`${t.length} ${t.length===1?"día":"días"}`:`${t.length} ${t.length===1?"mes":"meses"}`;return`
      <section class="surface activity-card energy-shell">
        ...
        <div class="energy-toolbar">
          <div class="energy-tabs" role="tablist" aria-label="Período de consumo energético">
            ${[["day","Día"],["month","Mes"],["year","Año"]].map(([a,l])=>`<button class="energy-tab ${this._energyRange===a?"is-active":""}" data-action="energy-range" data-range="${a}" role="tab" aria-selected="${this._energyRange===a}">${l}</button>`).join("")}
          </div>
    ...
    `}_renderConfirmDialog(){if(!["on","off"].includes(this._confirmAction))return"";const e=this._confirmAction==="on",t=this._config(),r=e?t.powerOnScript:t.powerOffScript,i=this._pendingAction===r,s=e?"¿Encender toda la iluminación?":"¿Apagar todo el showroom?",n=e?"Se encenderán las luminarias generales del showroom. Después podrás elegir una escena o ajustar cada zona de forma individual.":"Se apagarán las luminarias generales del showroom y cualquier escena activa.",o=e?"Sí, encender":"Sí, apagar";return`
    ...
    `}render(){var g;if(!this.shadowRoot||!this._hass)return;this._captureEnergyChartScroll(),this.setAttribute("data-theme",this._theme);const e=this._config(),t=e.staticOnly||e.panelKind==="general",r=this._state(e.weather),i=(r==null?void 0:r.attributes)||{},s=r==null?void 0:r.state,n=this._theme==="dark"?"claro":"oscuro",o=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],a=o.filter(p=>this._visibleSwitchState(p.entity)==="on").length,l=(g=this._state(e.mediaPlayer))==null?void 0:g.state,c=l==="playing"?"Reproduciendo":l==="paused"?"En pausa":"Detenido",d=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`;this.shadowRoot.innerHTML=`
      <style>
        :host {
          --primary: #f26522;
      ...
      <div class="app-shell">
        <header class="topbar">
          ...
          ${t?"":`<div class="status-strip" aria-label="Resumen del showroom">
            <button class="status-pill ${a?"is-active":""}" data-action="set-view" data-view="lights">
              <span class="status-pill-icon">${this._icon("bulb")}</span>
              <span><strong data-lights-summary>${a} de ${o.length}</strong><small>Luces</small></span>
            </button>
          ...
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(F[s]||s||"Sin datos")}, ${this._escape(i.temperature??"Sin datos")}${this._escape(i.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(L[s]||"·")}</span>

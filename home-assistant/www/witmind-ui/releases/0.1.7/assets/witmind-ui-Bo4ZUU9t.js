...
`;class B extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",e=>this._handleClick(e)),this.shadowRoot.addEventListener("scroll",e=>this._handleEnergyScroll(e),!0)}set hass(e){var o,n,l,c;const t=this._hass,r=this._config(),i=this._relevantHassChanged(t,e),a=!t||((o=t.states)==null?void 0:o[r.energySensor])!==((n=e==null?void 0:e.states)==null?void 0:n[r.energySensor]),s=!((l=t==null?void 0:t.states)!=null&&l[r.energySensor])&&!!((c=e==null?void 0:e.states)!=null&&c[r.energySensor]);this._hass=e,i&&this._syncStatesFromHass(),this._started?a&&(s?this._loadEnergyStatistics():this._scheduleEnergyRefresh()):(this._started=!0,this._start()),i&&this._requestRender()}get hass(){return this._hass}set panel(e){const t=this._config().weather;this._panel=e;const r=this._config().weather;this._started&&t!==r&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._requestRender())}get panel(){return this._panel}set narrow(e){this._narrow=!!e,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._requestRender(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const e of this._switchTimers.values())clearTimeout(e);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_handleClick(e){const t=e.target.closest("[data-action]");if(!t||!this._hass)return;const r=t.dataset.action;if(r==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(r==="toggle-theme"){this._toggleTheme();return}if(r==="set-view"){const i=t.dataset.view;["home","lights","energy","system"].includes(i)&&(this._activeView=i,this._requestRender());return}if(r==="toggle-switch"){this._toggleSwitch(t.dataset.entity);return}if(r==="run-scene"){this._runScene(t.dataset.sceneKey||t.dataset.entity,t.dataset.label);return}if(r==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(r==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(r==="general-off"){const i=this._config().generalOffScript;i?this._hass.callService("script","turn_on",{entity_id:i}).catch(a=>{this._notify("No se pudo ejecutar Apagar todo Witmind.","error"),console.error("Error ejecutando el apagado general de Witmind:",a)}):this._notify("El apagado general aún no tiene una entidad configurada.","error");return}if(r==="cancel-power-confirm"){const i=e.target.closest("[data-dialog-card]");if(t.classList.contains("dialog-backdrop")&&i)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(r==="confirm-power"){this._confirmGeneralPower();return}if(r==="clear-scene"){this._clearScene();return}if(r==="media"){this._mediaAction(t.dataset.service);return}if(r==="energy-range"){this._setEnergyRange(t.dataset.range);return}if(r==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(r==="energy-day-next"){this._shiftEnergyDay(1);return}if(r==="energy-day-today"){this._resetEnergyDay();return}r==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(e){console.warn("No se pudo guardar el tema del showroom:",e)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this._requestRender()}_config(){var u;const e=((u=this._panel)==null?void 0:u.config)||{},t=e.panel_kind==="lobby"||e.panel_kind==="general"||e.static_only===!0||e.staticOnly===!0,r=m=>Array.isArray(m)?[...new Set(m.filter(Boolean).map(y=>String(y)))]:[],i=(m,y)=>(Array.isArray(m)&&(m.length||t)?m:y).filter(w=>w==null?void 0:w.entity).map((w,h)=>({entity:String(w.entity),name:w.name||`Dispositivo ${h+1}`,subtitle:w.subtitle||"Iluminación",icon:w.icon||"bulb"})),a=i(e.spots,b.spots),s=i(e.samples||e.muestras,b.samples),o=[...a,...s].map(m=>m.entity),n=r(e.scene_control_entities||e.sceneControlEntities),l=n.length?n:o,c=(m,y,$)=>(Array.isArray(m)&&(m.length||t)?m:y).filter(h=>(h==null?void 0:h.entity)||(h==null?void 0:h.id)||(h==null?void 0:h.key)).map((h,S)=>{const k=h.entity?String(h.entity):"",N=String(h.id||h.key||k||`${$}-${S+1}`),f=y.find(E=>{const R=E.entity?String(E.entity):"",P=String(E.id||E.key||R||"");return k&&R===k||P===N}),M=k||(f!=null&&f.entity?String(f.entity):""),D=String(h.id||h.key||(f==null?void 0:f.id)||M||`${$}-${S+1}`),x=M||`${$}:${D}`,_=Array.isArray(h.on_entities)||Array.isArray(h.onEntities),T=Array.isArray(h.off_entities)||Array.isArray(h.offEntities),C=r(_?h.on_entities||h.onEntities:f==null?void 0:f.onEntities),q=r(T?h.off_entities||h.offEntities:[]),W=T?q.filter(E=>!C.includes(E)):l.filter(E=>!C.includes(E));return{key:x,id:D,entity:M,name:h.name||(f==null?void 0:f.name)||`Escena ${S+1}`,subtitle:h.subtitle||(f==null?void 0:f.subtitle)||"Escena del showroom",icon:h.icon||(f==null?void 0:f.icon)||"presentation",directOnly:h.direct_only??h.directOnly??(f==null?void 0:f.directOnly)??!1,onEntities:C,offEntities:W}}),d=e.reflector||b.reflector,g=Number(e.history_hours??e.historyHours),p=Number(e.chart_hours??e.chartHours);return{title:e.title||b.title,subtitle:e.subtitle||b.subtitle,siteLabel:e.site_label||e.siteLabel||b.siteLabel,logo:e.logo||b.logo,weather:e.weather||b.weather,mediaPlayer:e.media_player||e.mediaPlayer||b.mediaPlayer,lightCountSensor:e.light_count_sensor||e.lightCountSensor||b.lightCountSensor,energySensor:e.energy_sensor||e.energySensor||b.energySensor,batteryLevel:e.battery_level||e.batteryLevel||b.batteryLevel,powerOnScript:e.power_on_script||e.powerOnScript||b.powerOnScript,powerOffScript:e.power_off_script||e.powerOffScript||b.powerOffScript,historyHours:Number.isFinite(g)&&g>0?Math.min(24,g):b.historyHours,chartHours:Number.isFinite(p)&&p>0?Math.min(72,p):b.chartHours,showForecast:e.show_forecast??e.showForecast??b.showForecast,panelKind:e.panel_kind||e.panelKind||"showroom",staticOnly:e.static_only??e.staticOnly??!1,generalOffScript:e.general_off_script||e.generalOffScript||"",spots:a,samples:s,sceneControlEntities:l,reflector:d!=null&&d.entity?{entity:String(d.entity),name:d.name||b.reflector.name,subtitle:d.subtitle||b.reflector.subtitle,icon:d.icon||b.reflector.icon}:null,scenes:c(e.scenes,b.scenes,"scene"),sampleScenes:c(e.sample_scenes||e.sampleScenes,b.sampleScenes,"sample")}}_allDevices(){const e=this._config();return[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]]}_allScenes(e=this._config()){return[...e.scenes,...e.sampleScenes]}_trackedEntities(){const e=this._config();return new Set([...this._allDevices().map(t=>t.entity),...e.sceneControlEntities,...this._allScenes(e).flatMap(t=>[t.entity,...t.onEntities,...t.offEntities]),e.weather,e.mediaPlayer,e.lightCountSensor,e.energySensor,e.batteryLevel,e.powerOnScript,e.powerOffScript].filter(Boolean))}_relevantHassChanged(e,t){var r,i;if(!e||!t)return!0;for(const a of this._trackedEntities())if(((r=e.states)==null?void 0:r[a])!==((i=t.states)==null?void 0:i[a]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var e;if((e=this._hass)!=null&&e.states)for(const t of this._trackedEntities()){const r=this._hass.states[t];r&&this._applyLiveState(t,r)}}_applyLiveState(e,t){if(!t){this._liveStates.delete(e);return}this._liveStates.set(e,t);const r=this._pendingSwitches.get(e);if(r&&t.state===r.desired){this._pendingSwitches.delete(e),this._switchErrors.delete(e);const i=this._switchTimers.get(e);i&&clearTimeout(i),this._switchTimers.delete(e)}}async _fetchCurrentStates(){var e;if((e=this._hass)!=null&&e.callWS)try{const t=await this._hass.callWS({type:"get_states"}),r=this._trackedEntities();for(const i of t||[])r.has(i.entity_id)&&this._applyLiveState(i.entity_id,i);this._requestRender()}catch(t){console.error("No se pudieron sincronizar los estados del showroom:",t)}}async _subscribeStateChanges(){var e;if(!(!((e=this._hass)!=null&&e.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(t=>{var i;const r=(i=t==null?void 0:t.data)==null?void 0:i.entity_id;!r||!this._trackedEntities().has(r)||(this._applyLiveState(r,t.data.new_state),r===this._config().energySensor&&this._scheduleEnergyRefresh(),this._requestRender())},"state_changed")}catch(t){console.error("No se pudo suscribir a state_changed:",t)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var t;const e=this._config();if(!(!((t=this._hass)!=null&&t.connection)||!e.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===e.weather)){this._resetForecastSubscription();try{this._forecastEntity=e.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(r=>{this._forecast=Array.isArray(r==null?void 0:r.forecast)?r.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:e.weather})}catch(r){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",r)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(e=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const t=this._energyRefreshInterval(),r=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,i=e?0:this._energyLastLoadedAt?Math.max(1e3,t-r):t;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),i)}_energyRangeDefinition(e=this._energyRange){const t=new Date;let r,i=t,a,s,o,n=!1;if(e==="year")r=new Date(t.getFullYear(),0,1,0,0,0,0),a="month",s=`Año ${t.getFullYear()}`,o="mes";else if(e==="month")r=new Date(t.getFullYear(),t.getMonth(),1,0,0,0,0),a="day",s=t.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),o="día";else{const l=new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0);r=new Date(l.getFullYear(),l.getMonth(),l.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),n=r.getTime()===l.getTime(),i=n?t:new Date(r.getFullYear(),r.getMonth(),r.getDate()+1,0,0,0,0),a="hour",s=r.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),o="hora"}return{start:r,end:i,period:a,title:s,intervalLabel:o,isToday:n}}_energyValueToKWh(e,t){const r=Number(e);if(!Number.isFinite(r))return null;const i=String(t||"kWh").trim().toLowerCase().replaceAll(" ","");return i==="kwh"?r:i==="wh"?r/1e3:i==="mwh"?r*1e3:null}_deriveEnergyStateDeltas(e,t,r){const i=Number(t instanceof Date?t.getTime():t),a=Number(r instanceof Date?r.getTime():r),s=e.filter(l=>Number.isFinite(Number(l.start))&&Number.isFinite(Number(l.state))).sort((l,c)=>Number(l.start)-Number(c.start)),o=[];let n=null;for(const l of s){const c=Number(l.start),d=Number(l.state);if(c<i){n=d;continue}if(c>=a)break;let g=0;if(Number.isFinite(n)){const p=d-n;g=p>=0?p:Math.max(0,d)}o.push({...l,change:g}),n=d}return o}_aggregateEnergyDayRows(e,t,r){var l;const i=new Date(t.start),a=new Date(t.end),s=t.isToday?new Date(a.getFullYear(),a.getMonth(),a.getDate(),a.getHours(),0,0,0):new Date(i.getFullYear(),i.getMonth(),i.getDate(),23,0,0,0),o=new Map;for(let c=new Date(i);c<=s;c.setHours(c.getHours()+1)){const d=c.getTime();o.set(d,{start:d,end:new Date(c.getFullYear(),c.getMonth(),c.getDate(),c.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:t.isToday&&d===s.getTime(),live:!1})}let n=null;for(const c of e){const d=new Date(Number(c.start));if(!Number.isFinite(d.getTime()))continue;const g=new Date(d.getFullYear(),d.getMonth(),d.getDate(),d.getHours(),0,0,0).getTime(),p=o.get(g);p&&(p.change+=Math.max(0,Number(c.change)||0),p.samples+=1,Number.isFinite(c.state)&&(!n||Number(c.end||c.start)>Number(n.end||n.start))&&(n=c))}if(t.isToday){const c=this._energyValueToKWh(r==null?void 0:r.state,(l=r==null?void 0:r.attributes)==null?void 0:l.unit_of_measurement),d=n==null?void 0:n.state,g=Number((n==null?void 0:n.end)||(n==null?void 0:n.start)),p=o.get(s.getTime());if(p&&Number.isFinite(c)&&Number.isFinite(d)&&Number.isFinite(g)&&g>=s.getTime()&&a.getTime()-g>=0&&a.getTime()-g<=900*1e3){const u=c-d;Number.isFinite(u)&&u>=0&&(p.change+=u,p.live=u>0)}}return[...o.values()].filter(c=>c.samples>0||c.live)}_calculateCurrentMonthEnergy(e,t,r){var g;const i=new Date(t),a=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0).getTime(),s=e.filter(p=>Number.isFinite(Number(p.start))&&Number(p.start)>=a).sort((p,u)=>Number(p.start)-Number(u.start));let o=s.reduce((p,u)=>p+Math.max(0,Number(u.change)||0),0),n=null;for(const p of s)Number.isFinite(Number(p.state))&&(!n||Number(p.end||p.start)>Number(n.end||n.start))&&(n=p);const l=this._energyValueToKWh(r==null?void 0:r.state,(g=r==null?void 0:r.attributes)==null?void 0:g.unit_of_measurement),c=Number(n==null?void 0:n.state),d=Number((n==null?void 0:n.end)||(n==null?void 0:n.start));if(Number.isFinite(l)&&Number.isFinite(c)&&Number.isFinite(d)&&d>=a&&d<=i.getTime()&&i.getTime()-d<=7200*1e3){const p=l-c;Number.isFinite(p)&&p>=0&&(o+=p)}return s.length||Number.isFinite(c)?o:null}_aggregateEnergyCalendarRows(e,t,r,i){var d;if(!["month","year"].includes(i))return[];const a=new Map;let s=null;for(const g of e){const p=Number(g.start);if(!Number.isFinite(p))continue;const u=new Date(p),m=i==="month"?new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0).getTime():new Date(u.getFullYear(),u.getMonth(),1,0,0,0,0).getTime(),y=a.get(m)||{start:m,change:0,samples:0,partial:!1,live:!1};y.change+=Math.max(0,Number(g.change)||0),y.samples+=1,a.set(m,y),Number.isFinite(Number(g.state))&&(!s||p>Number(s.start))&&(s=g)}const o=this._energyValueToKWh(r==null?void 0:r.state,(d=r==null?void 0:r.attributes)==null?void 0:d.unit_of_measurement),n=Number(s==null?void 0:s.state),l=Number(s==null?void 0:s.start),c=new Date(t.end);if(Number.isFinite(o)&&Number.isFinite(n)&&Number.isFinite(l)&&l<=c.getTime()&&c.getTime()-l<=7200*1e3){const g=o-n;if(Number.isFinite(g)&&g>=0){const p=i==="month"?new Date(c.getFullYear(),c.getMonth(),c.getDate(),0,0,0,0).getTime():new Date(c.getFullYear(),c.getMonth(),1,0,0,0,0).getTime(),u=a.get(p)||{start:p,change:0,samples:0,partial:!1,live:!1};u.change+=g,u.live=g>0,a.set(p,u)}}return[...a.values()].sort((g,p)=>Number(g.start)-Number(p.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(e=>[Number(e.start),Number(e.change)||0,Number(e.samples)||0,!!e.partial,!!e.live])])}async _loadEnergyStatistics(){var p;if(!((p=this._hass)!=null&&p.connection))return;clearTimeout(this._energyRefreshTimer);const e=this._config(),t=this._state(e.energySensor),r=Number(t==null?void 0:t.state);if(!t){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${e.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(r)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${e.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const i=++this._energyRequestId,a=this._energyViewSignature(),s=this._energyData.length>0,o=this._energyRangeDefinition(),n=new Date,l=new Date(n.getFullYear(),n.getMonth(),1,0,0,0,0),c=new Date(o.start.getTime()-3600*1e3),d=this._energyRange==="year"?"hour":"5minute",g=new Date(l.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,s||this._requestRender();try{const u=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[e.energySensor]});if(i!==this._energyRequestId)return;const y=(Array.isArray(u)?u:[]).find(x=>(x==null?void 0:x.statistic_id)===e.energySensor)||null;if(!y||!y.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const $=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:c.toISOString(),end_time:o.end.toISOString(),statistic_ids:[e.energySensor],period:d,units:{energy:"kWh"},types:["state"]}),w=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:g.toISOString(),end_time:n.toISOString(),statistic_ids:[e.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[h,S]=await Promise.all([$,w]);if(i!==this._energyRequestId)return;const k=x=>(Array.isArray(x==null?void 0:x[e.energySensor])?x[e.energySensor]:[]).map(_=>({start:Number(_.start),end:Number(_.end),change:_.change===void 0||_.change===null?null:Math.max(0,Number(_.change)||0),state:_.state===void 0||_.state===null?null:Number(_.state)})).filter(_=>Number.isFinite(_.start)&&(Number.isFinite(_.state)||Number.isFinite(_.change))),N=k(h),f=this._deriveEnergyStateDeltas(N,o.start,o.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows(f,o,t):this._aggregateEnergyCalendarRows(f,o,t,this._energyRange);const M=k(S),D=this._deriveEnergyStateDeltas(M,l,n);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(D,n,t),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((x,_)=>x+Math.max(0,Number(_.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const x=new Date(n.getFullYear(),n.getMonth(),1,0,0,0,0).getTime(),_=this._energyData.find(T=>Number(T.start)===x);_&&(_.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(u){if(i!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(u==null?void 0:u.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",u)}finally{if(i===this._energyRequestId){this._energyLoading=!1;const u=a!==this._energyViewSignature();(!s||u)&&this._requestRender(),this._scheduleEnergyRefresh()}}}_setEnergyRange(e){!["day","month","year"].includes(e)||e===this._energyRange||(this._energyRange=e,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(e){if(this._energyRange!=="day")return;const t=Number(e);if(!Number.isFinite(t)||t===0)return;const r=Math.min(0,this._energyDayOffset+t);r!==this._energyDayOffset&&(this._energyDayOffset=r,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(e,t=2){const r=Number(e);return Number.isFinite(r)?r.toLocaleString("es-BO",{minimumFractionDigits:t,maximumFractionDigits:t}):"--"}_energyLabel(e,t=this._energyRange){const r=new Date(Number(e));return Number.isFinite(r.getTime())?t==="year"?r.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):t==="month"?String(r.getDate()):r.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=t<=0||t-e.scrollLeft<=8}_handleEnergyScroll(e){var i,a;const t=(a=(i=e.target)==null?void 0:i.closest)==null?void 0:a.call(i,"[data-energy-scroll]");if(!t)return;const r=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=r<=0||r-t.scrollLeft<=8}_restoreEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){e.scrollLeft=t,this._energyScrollLeft=e.scrollLeft;return}e.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,t))}_energyChart(){const e=this._energyData;if(this._energyLoading&&!e.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!e.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const t=280,r=14,i=16,a=22,s=42,o=this._energyRange==="day"?64:this._energyRange==="month"?36:58,n=Math.max(620,e.length*o+r+i),l=t-a-s,c=n-r-i,d=Math.max(...e.map(h=>h.change),.001),g=this._energyRange==="day"?10:e.length>24?4:7,p=Math.max(6,(c-g*Math.max(0,e.length-1))/e.length),u=this._energyRange==="month"?Math.max(1,Math.ceil(e.length/10)):this._energyRange==="day"?2:1,m=[0,.25,.5,.75,1],y=m.map(h=>{const S=a+l*(1-h);return`<line x1="${r}" y1="${S.toFixed(1)}" x2="${n-i}" y2="${S.toFixed(1)}" class="energy-grid-line"/>`}).join(""),$=m.map(h=>{const S=a+l*(1-h),k=this._formatEnergy(d*h,d<1?2:1);return`<span class="energy-y-tick" style="top:${S.toFixed(1)}px">${this._escape(k)}</span>`}).join(""),w=e.map((h,S)=>{const k=r+S*(p+g),N=h.change>0?Math.max(2,h.change/d*l):1,f=a+l-N,M=S%u===0||S===e.length-1,D=this._energyLabel(h.start),x=h.partial?" · en curso":"",_=h.partial?`<text x="${(k+p/2).toFixed(1)}" y="${(a+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${h.partial?"is-current":""}"><rect x="${k.toFixed(1)}" y="${f.toFixed(1)}" width="${p.toFixed(1)}" height="${N.toFixed(1)}" rx="${Math.min(4,p/2).toFixed(1)}" class="energy-bar ${h.partial?"is-partial":""}"><title>${this._escape(D)} · ${this._formatEnergy(h.change)} kWh${x}</title></rect>${_}${M?`<text x="${(k+p/2).toFixed(1)}" y="${t-14}" text-anchor="middle" class="energy-axis-text">${this._escape(D)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${$}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${n}" height="${t}" viewBox="0 0 ${n} ${t}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${y}${w}</svg></div></div>`}async _loadHistory(){var o;const e=this._config();if(!((o=this._hass)!=null&&o.callApi))return;const t=Math.max(e.historyHours,e.chartHours),r=new Date(Date.now()-t*60*60*1e3).toISOString(),i=new Date().toISOString(),a=[e.lightCountSensor,...e.spots.map(n=>n.entity),...e.samples.map(n=>n.entity)].filter(Boolean).join(",");if(!a)return;const s=`history/period/${encodeURIComponent(r)}?filter_entity_id=${encodeURIComponent(a)}&end_time=${encodeURIComponent(i)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",s),this._historyError=""}catch(n){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",n)}this._requestRender()}_state(e){var t,r;return this._liveStates.get(e)||((r=(t=this._hass)==null?void 0:t.states)==null?void 0:r[e])}_isUnavailable(e){var r;const t=(r=this._state(e))==null?void 0:r.state;return!t||t==="unknown"||t==="unavailable"}_visibleSwitchState(e){var t,r;return((t=this._pendingSwitches.get(e))==null?void 0:t.desired)||((r=this._state(e))==null?void 0:r.state)||"unavailable"}async _toggleSwitch(e){if(!e||!this._hass)return;const t=this._state(e);if(!t||["unknown","unavailable"].includes(t.state)){this._switchErrors.set(e,"No disponible"),this._requestRender();return}const i=this._visibleSwitchState(e)==="on"?"off":"on",a=i==="on"?"turn_on":"turn_off",s=e.split(".")[0]||"switch";this._pendingSwitches.set(e,{desired:i,startedAt:Date.now()}),this._switchErrors.delete(e),this._requestRender();try{await this._hass.callService(s,a,{entity_id:e});const o=this._switchTimers.get(e);o&&clearTimeout(o);const n=setTimeout(()=>this._verifySwitchState(e,i),6e3);this._switchTimers.set(e,n)}catch(o){this._pendingSwitches.delete(e),this._switchErrors.set(e,"La acción falló"),this._requestRender(),console.error(`Error ejecutando ${a} en ${e}:`,o)}}async _verifySwitchState(e,t){var i;await this._fetchCurrentStates();const r=((i=this._state(e))==null?void 0:i.state)===t;this._pendingSwitches.delete(e),this._switchTimers.delete(e),r?this._switchErrors.delete(e):this._switchErrors.set(e,"Sin confirmación"),this._requestRender()}_sceneStatus(e){const t=[...e.onEntities.map(a=>({entityId:a,desired:"on"})),...e.offEntities.map(a=>({entityId:a,desired:"off"}))];if(!t.length)return{active:!1,unavailable:!1,mismatches:[]};const r=t.some(({entityId:a})=>this._isUnavailable(a)),i=t.filter(({entityId:a,desired:s})=>{var o;return((o=this._state(a))==null?void 0:o.state)!==s});return{active:!r&&i.length===0,unavailable:r,mismatches:i}}_sceneExpectations(e){return[...e.offEntities.map(t=>({entityId:t,desired:"off"})),...e.onEntities.map(t=>({entityId:t,desired:"on"}))]}_markExpectedStates(e){const t=Date.now();for(const{entityId:r,desired:i}of e)this._pendingSwitches.set(r,{desired:i,startedAt:t}),this._switchErrors.delete(r)}_clearExpectedStates(e){for(const{entityId:t}of e){this._pendingSwitches.delete(t);const r=this._switchTimers.get(t);r&&clearTimeout(r),this._switchTimers.delete(t)}}async _setEntitiesState(e,t){const r=[...new Set((e||[]).filter(Boolean))];if(!r.length)return;const i=new Map;for(const s of r){const o=s.split(".")[0];o&&(i.has(o)||i.set(o,[]),i.get(o).push(s))}const a=t==="on"?"turn_on":"turn_off";for(const[s,o]of i)await this._hass.callService(s,a,{entity_id:o})}async _waitForExpectedStates(e,t=7e3){const r=Date.now()+t;let i=e;for(;Date.now()<r;){if(await this._fetchCurrentStates(),i=e.filter(({entityId:a,desired:s})=>{var o;return((o=this._state(a))==null?void 0:o.state)!==s}),!i.length)return{ok:!0,mismatches:[]};await new Promise(a=>setTimeout(a,450))}return{ok:!1,mismatches:i}}async _runScene(e,t){var o;if(!e||this._pendingAction)return;const r=this._config(),i=this._allScenes(r).find(n=>n.key===e||n.entity===e);if(!i){this._notify("La escena no está configurada.","error");return}const a=this._sceneExpectations(i);this._pendingAction=i.key,this._markExpectedStates(a),this._requestRender();let s=null;try{if(i.entity&&!i.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:i.entity})}catch(c){s=c,console.warn(`La escena ${i.entity} no respondió; se aplicará el perfil directo.`,c)}await this._setEntitiesState(i.offEntities,"off"),await this._setEntitiesState(i.onEntities,"on");const n=await this._waitForExpectedStates(a);if(!n.ok){const c=n.mismatches.map(d=>d.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${c}`)}const l=s?`${t||"Modo"} aplicado mediante control directo.`:`${t||"Modo"} activo.`;this._notify(l,"success")}catch(n){for(const{entityId:l,desired:c}of a)((o=this._state(l))==null?void 0:o.state)!==c&&this._switchErrors.set(l,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",n)}finally{this._clearExpectedStates(a),this._pendingAction="",await this._fetchCurrentStates(),this._requestRender()}}async _executeGeneralPower(e,t={}){var l;if(!this._hass||this._pendingAction||!["on","off"].includes(e))return!1;const r=this._config(),i=e==="on"?r.powerOnScript:r.powerOffScript,a=r.sceneControlEntities.map(c=>({entityId:c,desired:e})),s=t.successMessage||(e==="on"?"Iluminación general encendida.":"Iluminación general apagada."),o=t.errorMessage||(e==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=i||`direct-power-${e}`,this._markExpectedStates(a),this._requestRender();let n=null;try{if(i)try{await this._hass.callService("script","turn_on",{entity_id:i})}catch(d){n=d,console.warn(`El script ${i} no respondió; se aplicará el control directo.`,d)}await this._setEntitiesState(r.sceneControlEntities,e);const c=await this._waitForExpectedStates(a);if(!c.ok){const d=c.mismatches.map(g=>g.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${d}`)}return this._confirmAction="",this._notify(s,"success"),!0}catch(c){for(const{entityId:d,desired:g}of a)((l=this._state(d))==null?void 0:l.state)!==g&&this._switchErrors.set(d,"Sin confirmación");return this._notify(o,"error"),console.error("Error ejecutando el control general del showroom:",{error:c,scriptError:n,desired:e}),!1}finally{this._clearExpectedStates(a),this._pendingAction="",await this._fetchCurrentStates(),this._requestRender()}}async _confirmGeneralPower(){const e=this._confirmAction;["on","off"].includes(e)&&await this._executeGeneralPower(e,{successMessage:e==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(e){const t=this._config().mediaPlayer;if(!(!t||!e||this._isUnavailable(t)))try{await this._hass.callService("media_player",e,{entity_id:t})}catch(r){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${e}:`,r)}}_notify(e,t="success"){clearTimeout(this._toastTimer),this._toast={message:e,type:t},this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._requestRender()},4200)}_historyMap(){var t;const e=new Map;for(const r of this._history||[]){const i=(t=r==null?void 0:r[0])==null?void 0:t.entity_id;i&&e.set(i,r)}return e}_historySegments(e,t,r){if(!Array.isArray(e)||!e.length)return[];const i=e.map(s=>({state:s.state,time:new Date(s.last_changed||s.last_updated).getTime()})).filter(s=>Number.isFinite(s.time)).sort((s,o)=>s.time-o.time);if(!i.length)return[];const a=[];for(let s=0;s<i.length;s+=1){const o=i[s],n=i[s+1],l=Math.max(t,o.time),c=Math.min(r,(n==null?void 0:n.time)??r);c<=l||a.push({state:o.state,left:(l-t)/(r-t)*100,width:(c-l)/(r-t)*100})}return a}_sparkline(e,t){var u;const r=this._historyMap().get(e)||[],i=Date.now(),a=i-t*60*60*1e3,s=r.map(m=>({value:Number(m.state),time:new Date(m.last_changed||m.last_updated).getTime()})).filter(m=>Number.isFinite(m.value)&&Number.isFinite(m.time)&&m.time>=a).sort((m,y)=>m.time-y.time),o=Number((u=this._state(e))==null?void 0:u.state);if(Number.isFinite(o)&&s.push({value:o,time:i}),!s.length)return{path:"",min:"--",max:"--",avg:"--"};const n=s.map(m=>m.value),l=Math.min(...n),c=Math.max(...n),d=n.reduce((m,y)=>m+y,0)/n.length,g=c-l||1;return{path:s.map((m,y)=>{const $=(m.time-a)/(i-a)*300,w=66-(m.value-l)/g*52;return`${y?"L":"M"}${Math.max(0,Math.min(300,$)).toFixed(1)},${w.toFixed(1)}`}).join(" "),min:this._formatNumber(l),max:this._formatNumber(c),avg:this._formatNumber(d)}}_formatNumber(e){return Number.isFinite(e)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(e):"--"}_updateClock(){var o,n,l;if(!this.shadowRoot)return;const e=new Date,t=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(e),r=((o=t.find(c=>c.type==="hour"))==null?void 0:o.value)||"--",i=((n=t.find(c=>c.type==="minute"))==null?void 0:n.value)||"--",a=(((l=t.find(c=>c.type==="dayPeriod"))==null?void 0:l.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const c of this.shadowRoot.querySelectorAll("[data-clock-time]"))c.textContent=`${r}:${i}`;for(const c of this.shadowRoot.querySelectorAll("[data-clock-period]"))c.textContent=a||"";const s=this.shadowRoot.querySelector("[data-current-time]");s&&(s.dateTime=e.toISOString(),s.setAttribute("aria-label",`${r}:${i} ${a}`.trim()))}_icon(e,t=""){const r=z[e]||z.bulb;return`<svg class="icon ${this._escape(t)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${r}</svg>`}_escape(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(e){const t=this._state(e.entity),r=this._visibleSwitchState(e.entity),i=r==="on",a=!t||["unknown","unavailable"].includes(t.state),s=this._pendingSwitches.has(e.entity),o=this._switchErrors.get(e.entity),n=o||(s?r==="on"?"Encendiendo…":"Apagando…":a?"No disponible":i?"Encendido":"Apagado");return`
      <button
        class="device ${i?"is-on":""} ${s?"is-pending":""} ${o?"is-error":""}"
      ...
      <section class="surface weather-card">
        ...
        ${e.showForecast?`
          <div class="forecast-row">
            ${a.length?a.map(s=>{const o=new Date(s.datetime),n=new Intl.DateTimeFormat("es-BO",{weekday:"short"}).format(o);return`
                <div class="forecast-item">
                  <span>${this._escape(n)}</span>
                  <b>${this._escape(L[s.condition]||"·")}</b>
    ...
    `;return`
      <section class="surface media-card ${r?"is-unavailable":""}">
        <div class="section-heading compact-heading">
          ...
          <span class="media-state ${a?"is-playing":""}">${this._escape(s)}</span>
        </div>
    ...
    `}_renderSceneBlock({eyebrow:e,title:t,scenes:r,className:i}){const a=this._config(),s=r.find(c=>this._sceneStatus(c).active),o=this._pendingAction===a.powerOffScript,n=a.sceneControlEntities.some(c=>{var d;return((d=this._state(c))==null?void 0:d.state)==="on"}),l=!!this._pendingAction||!n;return`
    ...
    `}_renderScenes(){const e=this._config(),t=[...e.scenes,...e.sampleScenes],r=t.find(o=>this._sceneStatus(o).active),i=this._pendingAction===e.powerOffScript,a=e.sceneControlEntities.some(o=>{var n;return((n=this._state(o))==null?void 0:n.state)==="on"}),s=!!this._pendingAction||!a;return`
    ...
    `}render(){var d;if(!this.shadowRoot||!this._hass)return;this._captureEnergyChartScroll(),this.setAttribute("data-theme",this._theme);const e=this._config(),t=this._state(e.weather),r=(t==null?void 0:t.attributes)||{},i=t==null?void 0:t.state,a=this._theme==="dark"?"claro":"oscuro",s=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],o=s.filter(g=>this._visibleSwitchState(g.entity)==="on").length,n=(d=this._state(e.mediaPlayer))==null?void 0:d.state,l=n==="playing"?"Reproduciendo":n==="paused"?"En pausa":"Detenido",c=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`;this.shadowRoot.innerHTML=`
      <style>
        :host {
          --primary: #f26522;
        ...
        <header class="topbar">
          ...
          <div class="status-strip" aria-label="Resumen del showroom">
            <button class="status-pill ${o?"is-active":""}" data-action="set-view" data-view="lights">
              ...
              <span><strong>${o} de ${s.length}</strong><small>Luces</small></span>
            </button>
            <button class="status-pill ${n==="playing"?"is-active":""}" data-action="set-view" data-view="home">
          ...
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(F[i]||i||"Sin datos")}, ${this._escape(r.temperature??"Sin datos")}${this._escape(r.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(L[i]||"·")}</span>
              <strong>${this._escape(r.temperature??"--")}${this._escape(r.temperature_unit??"°")}</strong>
            </div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar a tema ${a}" title="Cambiar a tema ${a}">${j}</button>
          </div>
        </header>

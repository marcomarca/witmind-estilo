...
`;class U extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",e=>this._handleClick(e)),this.shadowRoot.addEventListener("scroll",e=>this._handleEnergyScroll(e),!0)}set hass(e){var o,s,l,c;const t=this._hass,i=this._config(),r=this._relevantHassChanged(t,e),a=!t||((o=t.states)==null?void 0:o[i.energySensor])!==((s=e==null?void 0:e.states)==null?void 0:s[i.energySensor]),n=!((l=t==null?void 0:t.states)!=null&&l[i.energySensor])&&!!((c=e==null?void 0:e.states)!=null&&c[i.energySensor]);this._hass=e,r&&this._syncStatesFromHass(),this._started?a&&(n?this._loadEnergyStatistics():this._scheduleEnergyRefresh()):(this._started=!0,this._start()),r&&this._updateStatePresentation()}get hass(){return this._hass}set panel(e){const t=this._config().weather;this._panel=e;const i=this._config().weather;this._started&&t!==i&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._updateStatePresentation())}get panel(){return this._panel}set narrow(e){this._narrow=!!e,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._updateStatePresentation(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const e of this._switchTimers.values())clearTimeout(e);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_updateStatePresentation(){var o;if(!((o=this.shadowRoot)!=null&&o.querySelector(".app-shell"))){this._requestRender();return}const e=s=>{var h;const l=s.dataset.entity;if(!l)return;const c=this._state(l),d=this._visibleSwitchState(l),g=d==="on",p=!c||["unknown","unavailable"].includes(c.state),u=this._pendingSwitches.has(l),m=this._switchErrors.get(l),y=m||(u?d==="on"?"Encendiendo…":"Apagando…":p?"No disponible":g?"Encendido":"Apagado");s.classList.toggle("is-on",g),s.classList.toggle("is-pending",u),s.classList.toggle("is-error",!!m),s.disabled=p,s.setAttribute("aria-pressed",String(g));const $=((h=s.querySelector(".device-copy strong"))==null?void 0:h.textContent)||l;s.setAttribute("aria-label",`${$}: ${y}`);const x=s.querySelector(".device-copy small");x&&(x.textContent=y)};this.shadowRoot.querySelectorAll('[data-action="toggle-switch"]').forEach(e),this.shadowRoot.querySelectorAll('[data-action="run-scene"]').forEach(s=>{const l=this._allScenes().find(u=>u.key===s.dataset.sceneKey);if(!l)return;const c=this._sceneStatus(l),d=this._pendingAction===l.key,g=d?"Aplicando...":c.active?"Activo":c.unavailable?"Sin datos":"Inactivo";s.classList.toggle("is-pending",d),s.classList.toggle("is-active",c.active),s.classList.toggle("is-unavailable",c.unavailable),s.disabled=!!(this._pendingAction&&!d),s.setAttribute("aria-pressed",String(c.active));const p=s.querySelector(".scene-state");p&&(p.textContent=g)});const t=this._config(),i=[...t.spots,...t.samples,...t.reflector?[t.reflector]:[]],r=i.filter(s=>this._visibleSwitchState(s.entity)==="on").length,a=this.shadowRoot.querySelector("[data-lights-summary]");a&&(a.textContent=`${r} de ${i.length}`);const n=this.shadowRoot.querySelector("[data-energy-summary]");n&&(n.textContent=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`)}_handleClick(e){const t=e.target.closest("[data-action]");if(!t||!this._hass)return;const i=t.dataset.action;if(i==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(i==="toggle-theme"){this._toggleTheme();return}if(i==="set-view"){const r=t.dataset.view;["home","lights","energy","system"].includes(r)&&(this._activeView=r,this._requestRender());return}if(i==="toggle-switch"){this._toggleSwitch(t.dataset.entity);return}if(i==="run-scene"){this._runScene(t.dataset.sceneKey||t.dataset.entity,t.dataset.label);return}if(i==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(i==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(i==="general-off"){const r=this._config().generalOffScript;r?this._hass.callService("script","turn_on",{entity_id:r}).catch(a=>{this._notify("No se pudo ejecutar Apagar todo Witmind.","error"),console.error("Error ejecutando el apagado general de Witmind:",a)}):this._notify("El apagado general aún no tiene una entidad configurada.","error");return}if(i==="cancel-power-confirm"){const r=e.target.closest("[data-dialog-card]");if(t.classList.contains("dialog-backdrop")&&r)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(i==="confirm-power"){this._confirmGeneralPower();return}if(i==="clear-scene"){this._clearScene();return}if(i==="media"){this._mediaAction(t.dataset.service);return}if(i==="energy-range"){this._setEnergyRange(t.dataset.range);return}if(i==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(i==="energy-day-next"){this._shiftEnergyDay(1);return}if(i==="energy-day-today"){this._resetEnergyDay();return}i==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(e){console.warn("No se pudo guardar el tema del showroom:",e)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this._requestRender()}_config(){var u;const e=((u=this._panel)==null?void 0:u.config)||{},t=e.panel_kind==="lobby"||e.panel_kind==="general"||e.static_only===!0||e.staticOnly===!0,i=m=>Array.isArray(m)?[...new Set(m.filter(Boolean).map(y=>String(y)))]:[],r=(m,y)=>(Array.isArray(m)&&(m.length||t)?m:y).filter(x=>x==null?void 0:x.entity).map((x,h)=>({entity:String(x.entity),name:x.name||`Dispositivo ${h+1}`,subtitle:x.subtitle||"Iluminación",icon:x.icon||"bulb"})),a=r(e.spots,b.spots),n=r(e.samples||e.muestras,b.samples),o=[...a,...n].map(m=>m.entity),s=i(e.scene_control_entities||e.sceneControlEntities),l=s.length?s:o,c=(m,y,$)=>(Array.isArray(m)&&(m.length||t)?m:y).filter(h=>(h==null?void 0:h.entity)||(h==null?void 0:h.id)||(h==null?void 0:h.key)).map((h,k)=>{const S=h.entity?String(h.entity):"",T=String(h.id||h.key||S||`${$}-${k+1}`),f=y.find(E=>{const F=E.entity?String(E.entity):"",B=String(E.id||E.key||F||"");return S&&F===S||B===T}),M=S||(f!=null&&f.entity?String(f.entity):""),A=String(h.id||h.key||(f==null?void 0:f.id)||M||`${$}-${k+1}`),w=M||`${$}:${A}`,v=Array.isArray(h.on_entities)||Array.isArray(h.onEntities),N=Array.isArray(h.off_entities)||Array.isArray(h.offEntities),D=i(v?h.on_entities||h.onEntities:f==null?void 0:f.onEntities),H=i(N?h.off_entities||h.offEntities:[]),j=N?H.filter(E=>!D.includes(E)):l.filter(E=>!D.includes(E));return{key:w,id:A,entity:M,name:h.name||(f==null?void 0:f.name)||`Escena ${k+1}`,subtitle:h.subtitle||(f==null?void 0:f.subtitle)||"Escena del showroom",icon:h.icon||(f==null?void 0:f.icon)||"presentation",directOnly:h.direct_only??h.directOnly??(f==null?void 0:f.directOnly)??!1,onEntities:D,offEntities:j}}),d=e.reflector||b.reflector,g=Number(e.history_hours??e.historyHours),p=Number(e.chart_hours??e.chartHours);return{title:e.title||b.title,subtitle:e.subtitle||b.subtitle,siteLabel:e.site_label||e.siteLabel||b.siteLabel,logo:e.logo||b.logo,weather:e.weather||b.weather,mediaPlayer:e.media_player||e.mediaPlayer||b.mediaPlayer,lightCountSensor:e.light_count_sensor||e.lightCountSensor||b.lightCountSensor,energySensor:e.energy_sensor||e.energySensor||b.energySensor,batteryLevel:e.battery_level||e.batteryLevel||b.batteryLevel,powerOnScript:e.power_on_script||e.powerOnScript||b.powerOnScript,powerOffScript:e.power_off_script||e.powerOffScript||b.powerOffScript,historyHours:Number.isFinite(g)&&g>0?Math.min(24,g):b.historyHours,chartHours:Number.isFinite(p)&&p>0?Math.min(72,p):b.chartHours,showForecast:e.show_forecast??e.showForecast??b.showForecast,panelKind:e.panel_kind||e.panelKind||"showroom",staticOnly:e.static_only??e.staticOnly??!1,generalOffScript:e.general_off_script||e.generalOffScript||"",spots:a,samples:n,sceneControlEntities:l,reflector:d!=null&&d.entity?{entity:String(d.entity),name:d.name||b.reflector.name,subtitle:d.subtitle||b.reflector.subtitle,icon:d.icon||b.reflector.icon}:null,scenes:c(e.scenes,b.scenes,"scene"),sampleScenes:c(e.sample_scenes||e.sampleScenes,b.sampleScenes,"sample")}}_allDevices(){const e=this._config();return[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]]}_allScenes(e=this._config()){return[...e.scenes,...e.sampleScenes]}_trackedEntities(){const e=this._config();return new Set([...this._allDevices().map(t=>t.entity),...e.sceneControlEntities,...this._allScenes(e).flatMap(t=>[t.entity,...t.onEntities,...t.offEntities]),e.weather,e.mediaPlayer,e.lightCountSensor,e.energySensor,e.batteryLevel,e.powerOnScript,e.powerOffScript].filter(Boolean))}_relevantHassChanged(e,t){var i,r;if(!e||!t)return!0;for(const a of this._trackedEntities())if(((i=e.states)==null?void 0:i[a])!==((r=t.states)==null?void 0:r[a]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var e;if((e=this._hass)!=null&&e.states)for(const t of this._trackedEntities()){const i=this._hass.states[t];i&&this._applyLiveState(t,i)}}_applyLiveState(e,t){if(!t){this._liveStates.delete(e);return}this._liveStates.set(e,t);const i=this._pendingSwitches.get(e);if(i&&t.state===i.desired){this._pendingSwitches.delete(e),this._switchErrors.delete(e);const r=this._switchTimers.get(e);r&&clearTimeout(r),this._switchTimers.delete(e)}}async _fetchCurrentStates(){var e;if((e=this._hass)!=null&&e.callWS)try{const t=await this._hass.callWS({type:"get_states"}),i=this._trackedEntities();for(const r of t||[])i.has(r.entity_id)&&this._applyLiveState(r.entity_id,r);this._updateStatePresentation()}catch(t){console.error("No se pudieron sincronizar los estados del showroom:",t)}}async _subscribeStateChanges(){var e;if(!(!((e=this._hass)!=null&&e.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(t=>{var r;const i=(r=t==null?void 0:t.data)==null?void 0:r.entity_id;!i||!this._trackedEntities().has(i)||(this._applyLiveState(i,t.data.new_state),i===this._config().energySensor&&this._scheduleEnergyRefresh(),this._updateStatePresentation())},"state_changed")}catch(t){console.error("No se pudo suscribir a state_changed:",t)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var t;const e=this._config();if(!(!((t=this._hass)!=null&&t.connection)||!e.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===e.weather)){this._resetForecastSubscription();try{this._forecastEntity=e.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(i=>{this._forecast=Array.isArray(i==null?void 0:i.forecast)?i.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:e.weather})}catch(i){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",i)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(e=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const t=this._energyRefreshInterval(),i=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,r=e?0:this._energyLastLoadedAt?Math.max(1e3,t-i):t;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),r)}_energyRangeDefinition(e=this._energyRange){const t=new Date;let i,r=t,a,n,o,s=!1;if(e==="year")i=new Date(t.getFullYear(),0,1,0,0,0,0),a="month",n=`Año ${t.getFullYear()}`,o="mes";else if(e==="month")i=new Date(t.getFullYear(),t.getMonth(),1,0,0,0,0),a="day",n=t.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),o="día";else{const l=new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0);i=new Date(l.getFullYear(),l.getMonth(),l.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),s=i.getTime()===l.getTime(),r=s?t:new Date(i.getFullYear(),i.getMonth(),i.getDate()+1,0,0,0,0),a="hour",n=i.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),o="hora"}return{start:i,end:r,period:a,title:n,intervalLabel:o,isToday:s}}_energyValueToKWh(e,t){const i=Number(e);if(!Number.isFinite(i))return null;const r=String(t||"kWh").trim().toLowerCase().replaceAll(" ","");return r==="kwh"?i:r==="wh"?i/1e3:r==="mwh"?i*1e3:null}_deriveEnergyStateDeltas(e,t,i){const r=Number(t instanceof Date?t.getTime():t),a=Number(i instanceof Date?i.getTime():i),n=e.filter(l=>Number.isFinite(Number(l.start))&&Number.isFinite(Number(l.state))).sort((l,c)=>Number(l.start)-Number(c.start)),o=[];let s=null;for(const l of n){const c=Number(l.start),d=Number(l.state);if(c<r){s=d;continue}if(c>=a)break;let g=0;if(Number.isFinite(s)){const p=d-s;g=p>=0?p:Math.max(0,d)}o.push({...l,change:g}),s=d}return o}_aggregateEnergyDayRows(e,t,i){var l;const r=new Date(t.start),a=new Date(t.end),n=t.isToday?new Date(a.getFullYear(),a.getMonth(),a.getDate(),a.getHours(),0,0,0):new Date(r.getFullYear(),r.getMonth(),r.getDate(),23,0,0,0),o=new Map;for(let c=new Date(r);c<=n;c.setHours(c.getHours()+1)){const d=c.getTime();o.set(d,{start:d,end:new Date(c.getFullYear(),c.getMonth(),c.getDate(),c.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:t.isToday&&d===n.getTime(),live:!1})}let s=null;for(const c of e){const d=new Date(Number(c.start));if(!Number.isFinite(d.getTime()))continue;const g=new Date(d.getFullYear(),d.getMonth(),d.getDate(),d.getHours(),0,0,0).getTime(),p=o.get(g);p&&(p.change+=Math.max(0,Number(c.change)||0),p.samples+=1,Number.isFinite(c.state)&&(!s||Number(c.end||c.start)>Number(s.end||s.start))&&(s=c))}if(t.isToday){const c=this._energyValueToKWh(i==null?void 0:i.state,(l=i==null?void 0:i.attributes)==null?void 0:l.unit_of_measurement),d=s==null?void 0:s.state,g=Number((s==null?void 0:s.end)||(s==null?void 0:s.start)),p=o.get(n.getTime());if(p&&Number.isFinite(c)&&Number.isFinite(d)&&Number.isFinite(g)&&g>=n.getTime()&&a.getTime()-g>=0&&a.getTime()-g<=900*1e3){const u=c-d;Number.isFinite(u)&&u>=0&&(p.change+=u,p.live=u>0)}}return[...o.values()].filter(c=>c.samples>0||c.live)}_calculateCurrentMonthEnergy(e,t,i){var g;const r=new Date(t),a=new Date(r.getFullYear(),r.getMonth(),1,0,0,0,0).getTime(),n=e.filter(p=>Number.isFinite(Number(p.start))&&Number(p.start)>=a).sort((p,u)=>Number(p.start)-Number(u.start));let o=n.reduce((p,u)=>p+Math.max(0,Number(u.change)||0),0),s=null;for(const p of n)Number.isFinite(Number(p.state))&&(!s||Number(p.end||p.start)>Number(s.end||s.start))&&(s=p);const l=this._energyValueToKWh(i==null?void 0:i.state,(g=i==null?void 0:i.attributes)==null?void 0:g.unit_of_measurement),c=Number(s==null?void 0:s.state),d=Number((s==null?void 0:s.end)||(s==null?void 0:s.start));if(Number.isFinite(l)&&Number.isFinite(c)&&Number.isFinite(d)&&d>=a&&d<=r.getTime()&&r.getTime()-d<=7200*1e3){const p=l-c;Number.isFinite(p)&&p>=0&&(o+=p)}return n.length||Number.isFinite(c)?o:null}_aggregateEnergyCalendarRows(e,t,i,r){var d;if(!["month","year"].includes(r))return[];const a=new Map;let n=null;for(const g of e){const p=Number(g.start);if(!Number.isFinite(p))continue;const u=new Date(p),m=r==="month"?new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0).getTime():new Date(u.getFullYear(),u.getMonth(),1,0,0,0,0).getTime(),y=a.get(m)||{start:m,change:0,samples:0,partial:!1,live:!1};y.change+=Math.max(0,Number(g.change)||0),y.samples+=1,a.set(m,y),Number.isFinite(Number(g.state))&&(!n||p>Number(n.start))&&(n=g)}const o=this._energyValueToKWh(i==null?void 0:i.state,(d=i==null?void 0:i.attributes)==null?void 0:d.unit_of_measurement),s=Number(n==null?void 0:n.state),l=Number(n==null?void 0:n.start),c=new Date(t.end);if(Number.isFinite(o)&&Number.isFinite(s)&&Number.isFinite(l)&&l<=c.getTime()&&c.getTime()-l<=7200*1e3){const g=o-s;if(Number.isFinite(g)&&g>=0){const p=r==="month"?new Date(c.getFullYear(),c.getMonth(),c.getDate(),0,0,0,0).getTime():new Date(c.getFullYear(),c.getMonth(),1,0,0,0,0).getTime(),u=a.get(p)||{start:p,change:0,samples:0,partial:!1,live:!1};u.change+=g,u.live=g>0,a.set(p,u)}}return[...a.values()].sort((g,p)=>Number(g.start)-Number(p.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(e=>[Number(e.start),Number(e.change)||0,Number(e.samples)||0,!!e.partial,!!e.live])])}async _loadEnergyStatistics(){var p;if(!((p=this._hass)!=null&&p.connection))return;clearTimeout(this._energyRefreshTimer);const e=this._config(),t=this._state(e.energySensor),i=Number(t==null?void 0:t.state);if(!t){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${e.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(i)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${e.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const r=++this._energyRequestId,a=this._energyViewSignature(),n=this._energyData.length>0,o=this._energyRangeDefinition(),s=new Date,l=new Date(s.getFullYear(),s.getMonth(),1,0,0,0,0),c=new Date(o.start.getTime()-3600*1e3),d=this._energyRange==="year"?"hour":"5minute",g=new Date(l.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,n||this._requestRender();try{const u=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[e.energySensor]});if(r!==this._energyRequestId)return;const y=(Array.isArray(u)?u:[]).find(w=>(w==null?void 0:w.statistic_id)===e.energySensor)||null;if(!y||!y.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const $=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:c.toISOString(),end_time:o.end.toISOString(),statistic_ids:[e.energySensor],period:d,units:{energy:"kWh"},types:["state"]}),x=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:g.toISOString(),end_time:s.toISOString(),statistic_ids:[e.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[h,k]=await Promise.all([$,x]);if(r!==this._energyRequestId)return;const S=w=>(Array.isArray(w==null?void 0:w[e.energySensor])?w[e.energySensor]:[]).map(v=>({start:Number(v.start),end:Number(v.end),change:v.change===void 0||v.change===null?null:Math.max(0,Number(v.change)||0),state:v.state===void 0||v.state===null?null:Number(v.state)})).filter(v=>Number.isFinite(v.start)&&(Number.isFinite(v.state)||Number.isFinite(v.change))),T=S(h),f=this._deriveEnergyStateDeltas(T,o.start,o.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows(f,o,t):this._aggregateEnergyCalendarRows(f,o,t,this._energyRange);const M=S(k),A=this._deriveEnergyStateDeltas(M,l,s);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(A,s,t),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((w,v)=>w+Math.max(0,Number(v.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const w=new Date(s.getFullYear(),s.getMonth(),1,0,0,0,0).getTime(),v=this._energyData.find(N=>Number(N.start)===w);v&&(v.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(u){if(r!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(u==null?void 0:u.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",u)}finally{if(r===this._energyRequestId){this._energyLoading=!1;const u=a!==this._energyViewSignature();!n||u&&this._activeView==="energy"?this._requestRender():this._updateStatePresentation(),this._scheduleEnergyRefresh()}}}_setEnergyRange(e){!["day","month","year"].includes(e)||e===this._energyRange||(this._energyRange=e,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(e){if(this._energyRange!=="day")return;const t=Number(e);if(!Number.isFinite(t)||t===0)return;const i=Math.min(0,this._energyDayOffset+t);i!==this._energyDayOffset&&(this._energyDayOffset=i,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(e,t=2){const i=Number(e);return Number.isFinite(i)?i.toLocaleString("es-BO",{minimumFractionDigits:t,maximumFractionDigits:t}):"--"}_energyLabel(e,t=this._energyRange){const i=new Date(Number(e));return Number.isFinite(i.getTime())?t==="year"?i.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):t==="month"?String(i.getDate()):i.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var i;const e=(i=this.shadowRoot)==null?void 0:i.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=t<=0||t-e.scrollLeft<=8}_handleEnergyScroll(e){var r,a;const t=(a=(r=e.target)==null?void 0:r.closest)==null?void 0:a.call(r,"[data-energy-scroll]");if(!t)return;const i=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=i<=0||i-t.scrollLeft<=8}_restoreEnergyChartScroll(){var i;const e=(i=this.shadowRoot)==null?void 0:i.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){e.scrollLeft=t,this._energyScrollLeft=e.scrollLeft;return}e.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,t))}_energyChart(){const e=this._energyData;if(this._energyLoading&&!e.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!e.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const t=280,i=14,r=16,a=22,n=42,o=this._energyRange==="day"?64:this._energyRange==="month"?36:58,s=Math.max(620,e.length*o+i+r),l=t-a-n,c=s-i-r,d=Math.max(...e.map(h=>h.change),.001),g=this._energyRange==="day"?10:e.length>24?4:7,p=Math.max(6,(c-g*Math.max(0,e.length-1))/e.length),u=this._energyRange==="month"?Math.max(1,Math.ceil(e.length/10)):this._energyRange==="day"?2:1,m=[0,.25,.5,.75,1],y=m.map(h=>{const k=a+l*(1-h);return`<line x1="${i}" y1="${k.toFixed(1)}" x2="${s-r}" y2="${k.toFixed(1)}" class="energy-grid-line"/>`}).join(""),$=m.map(h=>{const k=a+l*(1-h),S=this._formatEnergy(d*h,d<1?2:1);return`<span class="energy-y-tick" style="top:${k.toFixed(1)}px">${this._escape(S)}</span>`}).join(""),x=e.map((h,k)=>{const S=i+k*(p+g),T=h.change>0?Math.max(2,h.change/d*l):1,f=a+l-T,M=k%u===0||k===e.length-1,A=this._energyLabel(h.start),w=h.partial?" · en curso":"",v=h.partial?`<text x="${(S+p/2).toFixed(1)}" y="${(a+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${h.partial?"is-current":""}"><rect x="${S.toFixed(1)}" y="${f.toFixed(1)}" width="${p.toFixed(1)}" height="${T.toFixed(1)}" rx="${Math.min(4,p/2).toFixed(1)}" class="energy-bar ${h.partial?"is-partial":""}"><title>${this._escape(A)} · ${this._formatEnergy(h.change)} kWh${w}</title></rect>${v}${M?`<text x="${(S+p/2).toFixed(1)}" y="${t-14}" text-anchor="middle" class="energy-axis-text">${this._escape(A)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${$}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${s}" height="${t}" viewBox="0 0 ${s} ${t}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${y}${x}</svg></div></div>`}async _loadHistory(){var o;const e=this._config();if(!((o=this._hass)!=null&&o.callApi))return;const t=Math.max(e.historyHours,e.chartHours),i=new Date(Date.now()-t*60*60*1e3).toISOString(),r=new Date().toISOString(),a=[e.lightCountSensor,...e.spots.map(s=>s.entity),...e.samples.map(s=>s.entity)].filter(Boolean).join(",");if(!a)return;const n=`history/period/${encodeURIComponent(i)}?filter_entity_id=${encodeURIComponent(a)}&end_time=${encodeURIComponent(r)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",n),this._historyError=""}catch(s){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",s)}this._requestRender()}_state(e){var t,i;return this._liveStates.get(e)||((i=(t=this._hass)==null?void 0:t.states)==null?void 0:i[e])}_isUnavailable(e){var i;const t=(i=this._state(e))==null?void 0:i.state;return!t||t==="unknown"||t==="unavailable"}_visibleSwitchState(e){var t,i;return((t=this._pendingSwitches.get(e))==null?void 0:t.desired)||((i=this._state(e))==null?void 0:i.state)||"unavailable"}async _toggleSwitch(e){if(!e||!this._hass)return;const t=this._state(e);if(!t||["unknown","unavailable"].includes(t.state)){this._switchErrors.set(e,"No disponible"),this._updateStatePresentation();return}const r=this._visibleSwitchState(e)==="on"?"off":"on",a=r==="on"?"turn_on":"turn_off",n=e.split(".")[0]||"switch";this._pendingSwitches.set(e,{desired:r,startedAt:Date.now()}),this._switchErrors.delete(e),this._updateStatePresentation();try{await this._hass.callService(n,a,{entity_id:e});const o=this._switchTimers.get(e);o&&clearTimeout(o);const s=setTimeout(()=>this._verifySwitchState(e,r),6e3);this._switchTimers.set(e,s)}catch(o){this._pendingSwitches.delete(e),this._switchErrors.set(e,"La acción falló"),this._updateStatePresentation(),console.error(`Error ejecutando ${a} en ${e}:`,o)}}async _verifySwitchState(e,t){var r;await this._fetchCurrentStates();const i=((r=this._state(e))==null?void 0:r.state)===t;this._pendingSwitches.delete(e),this._switchTimers.delete(e),i?this._switchErrors.delete(e):this._switchErrors.set(e,"Sin confirmación"),this._updateStatePresentation()}_sceneStatus(e){const t=[...e.onEntities.map(a=>({entityId:a,desired:"on"})),...e.offEntities.map(a=>({entityId:a,desired:"off"}))];if(!t.length)return{active:!1,unavailable:!1,mismatches:[]};const i=t.some(({entityId:a})=>this._isUnavailable(a)),r=t.filter(({entityId:a,desired:n})=>{var o;return((o=this._state(a))==null?void 0:o.state)!==n});return{active:!i&&r.length===0,unavailable:i,mismatches:r}}_sceneExpectations(e){return[...e.offEntities.map(t=>({entityId:t,desired:"off"})),...e.onEntities.map(t=>({entityId:t,desired:"on"}))]}_markExpectedStates(e){const t=Date.now();for(const{entityId:i,desired:r}of e)this._pendingSwitches.set(i,{desired:r,startedAt:t}),this._switchErrors.delete(i)}_clearExpectedStates(e){for(const{entityId:t}of e){this._pendingSwitches.delete(t);const i=this._switchTimers.get(t);i&&clearTimeout(i),this._switchTimers.delete(t)}}async _setEntitiesState(e,t){const i=[...new Set((e||[]).filter(Boolean))];if(!i.length)return;const r=new Map;for(const n of i){const o=n.split(".")[0];o&&(r.has(o)||r.set(o,[]),r.get(o).push(n))}const a=t==="on"?"turn_on":"turn_off";for(const[n,o]of r)await this._hass.callService(n,a,{entity_id:o})}async _waitForExpectedStates(e,t=7e3){const i=Date.now()+t;let r=e;for(;Date.now()<i;){if(await this._fetchCurrentStates(),r=e.filter(({entityId:a,desired:n})=>{var o;return((o=this._state(a))==null?void 0:o.state)!==n}),!r.length)return{ok:!0,mismatches:[]};await new Promise(a=>setTimeout(a,450))}return{ok:!1,mismatches:r}}async _runScene(e,t){var o;if(!e||this._pendingAction)return;const i=this._config(),r=this._allScenes(i).find(s=>s.key===e||s.entity===e);if(!r){this._notify("La escena no está configurada.","error");return}const a=this._sceneExpectations(r);this._pendingAction=r.key,this._markExpectedStates(a),this._updateStatePresentation();let n=null;try{if(r.entity&&!r.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:r.entity})}catch(c){n=c,console.warn(`La escena ${r.entity} no respondió; se aplicará el perfil directo.`,c)}await this._setEntitiesState(r.offEntities,"off"),await this._setEntitiesState(r.onEntities,"on");const s=await this._waitForExpectedStates(a);if(!s.ok){const c=s.mismatches.map(d=>d.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${c}`)}const l=n?`${t||"Modo"} aplicado mediante control directo.`:`${t||"Modo"} activo.`;this._notify(l,"success")}catch(s){for(const{entityId:l,desired:c}of a)((o=this._state(l))==null?void 0:o.state)!==c&&this._switchErrors.set(l,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",s)}finally{this._clearExpectedStates(a),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _executeGeneralPower(e,t={}){var l;if(!this._hass||this._pendingAction||!["on","off"].includes(e))return!1;const i=this._config(),r=e==="on"?i.powerOnScript:i.powerOffScript,a=i.sceneControlEntities.map(c=>({entityId:c,desired:e})),n=t.successMessage||(e==="on"?"Iluminación general encendida.":"Iluminación general apagada."),o=t.errorMessage||(e==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=r||`direct-power-${e}`,this._markExpectedStates(a),this._updateStatePresentation();let s=null;try{if(r)try{await this._hass.callService("script","turn_on",{entity_id:r})}catch(d){s=d,console.warn(`El script ${r} no respondió; se aplicará el control directo.`,d)}await this._setEntitiesState(i.sceneControlEntities,e);const c=await this._waitForExpectedStates(a);if(!c.ok){const d=c.mismatches.map(g=>g.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${d}`)}return this._confirmAction="",this._notify(n,"success"),!0}catch(c){for(const{entityId:d,desired:g}of a)((l=this._state(d))==null?void 0:l.state)!==g&&this._switchErrors.set(d,"Sin confirmación");return this._notify(o,"error"),console.error("Error ejecutando el control general del showroom:",{error:c,scriptError:s,desired:e}),!1}finally{this._clearExpectedStates(a),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _confirmGeneralPower(){const e=this._confirmAction;["on","off"].includes(e)&&await this._executeGeneralPower(e,{successMessage:e==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(e){const t=this._config().mediaPlayer;if(!(!t||!e||this._isUnavailable(t)))try{await this._hass.callService("media_player",e,{entity_id:t})}catch(i){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${e}:`,i)}}_updateToastPresentation(){if(!this.shadowRoot)return;const e=this.shadowRoot.querySelector("[data-toast]");if(!this._toast){e==null||e.remove();return}if(e){e.className=`toast ${this._escape(this._toast.type)}`,e.textContent=this._toast.message;return}const t=document.createElement("div");t.dataset.toast="",t.className=`toast ${this._escape(this._toast.type)}`,t.setAttribute("role","status"),t.textContent=this._toast.message,this.shadowRoot.append(t)}_notify(e,t="success"){var i;clearTimeout(this._toastTimer),this._toast={message:e,type:t},(i=this.shadowRoot)!=null&&i.querySelector(".app-shell")?this._updateToastPresentation():this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._updateToastPresentation()},4200)}_historyMap(){var t;const e=new Map;for(const i of this._history||[]){const r=(t=i==null?void 0:i[0])==null?void 0:t.entity_id;r&&e.set(r,i)}return e}_historySegments(e,t,i){if(!Array.isArray(e)||!e.length)return[];const r=e.map(n=>({state:n.state,time:new Date(n.last_changed||n.last_updated).getTime()})).filter(n=>Number.isFinite(n.time)).sort((n,o)=>n.time-o.time);if(!r.length)return[];const a=[];for(let n=0;n<r.length;n+=1){const o=r[n],s=r[n+1],l=Math.max(t,o.time),c=Math.min(i,(s==null?void 0:s.time)??i);c<=l||a.push({state:o.state,left:(l-t)/(i-t)*100,width:(c-l)/(i-t)*100})}return a}_sparkline(e,t){var u;const i=this._historyMap().get(e)||[],r=Date.now(),a=r-t*60*60*1e3,n=i.map(m=>({value:Number(m.state),time:new Date(m.last_changed||m.last_updated).getTime()})).filter(m=>Number.isFinite(m.value)&&Number.isFinite(m.time)&&m.time>=a).sort((m,y)=>m.time-y.time),o=Number((u=this._state(e))==null?void 0:u.state);if(Number.isFinite(o)&&n.push({value:o,time:r}),!n.length)return{path:"",min:"--",max:"--",avg:"--"};const s=n.map(m=>m.value),l=Math.min(...s),c=Math.max(...s),d=s.reduce((m,y)=>m+y,0)/s.length,g=c-l||1;return{path:n.map((m,y)=>{const $=(m.time-a)/(r-a)*300,x=66-(m.value-l)/g*52;return`${y?"L":"M"}${Math.max(0,Math.min(300,$)).toFixed(1)},${x.toFixed(1)}`}).join(" "),min:this._formatNumber(l),max:this._formatNumber(c),avg:this._formatNumber(d)}}_formatNumber(e){return Number.isFinite(e)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(e):"--"}_updateClock(){var o,s,l;if(!this.shadowRoot)return;const e=new Date,t=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(e),i=((o=t.find(c=>c.type==="hour"))==null?void 0:o.value)||"--",r=((s=t.find(c=>c.type==="minute"))==null?void 0:s.value)||"--",a=(((l=t.find(c=>c.type==="dayPeriod"))==null?void 0:l.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const c of this.shadowRoot.querySelectorAll("[data-clock-time]"))c.textContent=`${i}:${r}`;for(const c of this.shadowRoot.querySelectorAll("[data-clock-period]"))c.textContent=a||"";const n=this.shadowRoot.querySelector("[data-current-time]");n&&(n.dateTime=e.toISOString(),n.setAttribute("aria-label",`${i}:${r} ${a}`.trim()))}_icon(e,t=""){const i=P[e]||P.bulb;return`<svg class="icon ${this._escape(t)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${i}</svg>`}_escape(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(e){const t=this._state(e.entity),i=this._visibleSwitchState(e.entity),r=i==="on",a=!t||["unknown","unavailable"].includes(t.state),n=this._pendingSwitches.has(e.entity),o=this._switchErrors.get(e.entity),s=o||(n?i==="on"?"Encendiendo…":"Apagando…":a?"No disponible":r?"Encendido":"Apagado");return`
      <button
        class="device ${r?"is-on":""} ${n?"is-pending":""} ${o?"is-error":""}"
    ...
    `}_renderDeviceGroup(e,t,i){return`
      <section class="surface control-section">
        ...
        <div class="device-grid">${i.map(r=>this._renderDevice(r)).join("")}</div>
      </section>
    `}_renderWeather(){const e=this._config(),t=this._state(e.weather);if(!t)return`
      ...
      `;const i=t.attributes||{},r=t.state,a=this._forecast.slice(0,3);return`
      <section class="surface weather-card">
        <div class="weather-main">
          <div class="weather-symbol">${this._escape(L[r]||"·")}</div>
          <div class="weather-copy">
            <span class="eyebrow">Clima · Casa</span>
            <h2>${this._escape(O[r]||r)}</h2>
    ...
    `}_renderMedia(){const e=this._config(),t=this._state(e.mediaPlayer),i=!t||["unknown","unavailable"].includes(t.state),r=(t==null?void 0:t.attributes)||{},a=(t==null?void 0:t.state)==="playing",n=i?"No disponible":a?"Reproduciendo":(t==null?void 0:t.state)==="paused"?"En pausa":(t==null?void 0:t.state)==="idle"?"En espera":(t==null?void 0:t.state)||"Detenido",o=r.media_title||r.friendly_name||"Showroom 1",s=r.media_artist||r.source||"Música del showroom",l=Number(r.volume_level),c=(d,g,p,u=!1)=>`
      <button
    ...
    `}_renderSceneButton(e){const t=this._pendingAction===e.key,i=this._sceneStatus(e),r=t?"Aplicando...":i.active?"Activo":i.unavailable?"Sin datos":"Inactivo";return`
      <button
        ...
        aria-label="${this._escape(`${e.name}: ${r}`)}"
      ...
      >
        ...
        <span class="scene-state" aria-hidden="true">${this._escape(r)}</span>
      </button>
    `}_renderSceneBlock({eyebrow:e,title:t,scenes:i,className:r}){const a=this._config(),n=i.find(c=>this._sceneStatus(c).active),o=this._pendingAction===a.powerOffScript,s=a.sceneControlEntities.some(c=>{var d;return((d=this._state(c))==null?void 0:d.state)==="on"}),l=!!this._pendingAction||!s;return`
      <section class="surface scenes-card ${this._escape(r)}">
    ...
    `}_renderScenes(){const e=this._config(),t=[...e.scenes,...e.sampleScenes],i=t.find(o=>this._sceneStatus(o).active),r=this._pendingAction===e.powerOffScript,a=e.sceneControlEntities.some(o=>{var s;return((s=this._state(o))==null?void 0:s.state)==="on"}),n=!!this._pendingAction||!a;return`
      <section class="surface scenes-card quick-scenes-card">
        <div class="section-heading compact-heading scenes-heading">
          ...
          <div class="scene-heading-actions">
            <span class="scene-summary ${i?"is-active":""}">${this._escape(r?"Apagando...":i?i.name:"Manual")}</span>
            <button
              class="clear-scene-button"
              data-action="clear-scene"
              aria-label="Apagar toda la iluminación de escenas"
            ...
            >
              <span>${this._icon("power")}</span>
              <strong>${r?"Apagando...":"Apagar"}</strong>
            </button>
          </div>
        </div>
        <div class="scene-grid quick-scene-grid">${t.map(o=>this._renderSceneButton(o)).join("")}</div>
      </section>
    `}_renderGeneralControl(e=!0){const t=this._config(),i=this._pendingAction===t.powerOnScript,r=this._pendingAction===t.powerOffScript;return`
      <section class="surface general-card">
        ...
        <div class="general-actions">
          ...
          <button class="general-action power-off" data-action="open-power-off" ${this._pendingAction?"disabled":""}>
            <span>${this._icon("power")}</span>
            <strong>${r?"Apagando…":"Apagar todo"}</strong>
          </button>
        </div>
        ${e&&t.reflector?`
    ...
    `}_renderActivity(){const e=this._energyRangeDefinition(),t=this._energyData.map(s=>Math.max(0,Number(s.change)||0)),i=t.reduce((s,l)=>s+l,0),r=t.length?i/t.length:0,a=t.length?Math.max(...t):0,n=new Date().toLocaleDateString("es-BO",{month:"short",year:"numeric"}).replace(".",""),o=this._energyRange==="day"?`${t.length} ${t.length===1?"hora":"horas"}`:this._energyRange==="month"?`${t.length} ${t.length===1?"día":"días"}`:`${t.length} ${t.length===1?"mes":"meses"}`;return`
      <section class="surface activity-card energy-shell">
        ...
        <div class="energy-summary">
          ...
          <div class="energy-stat"><small>Promedio por ${this._escape(e.intervalLabel)}</small><strong>${this._energyLoading&&!this._energyData.length?"…":`${this._formatEnergy(r)} kWh`}</strong><span>${this._escape(o)}</span></div>
    ...
    `}_renderConfirmDialog(){if(!["on","off"].includes(this._confirmAction))return"";const e=this._confirmAction==="on",t=this._config(),i=e?t.powerOnScript:t.powerOffScript,r=this._pendingAction===i,a=e?"¿Encender toda la iluminación?":"¿Apagar todo el showroom?",n=e?"Se encenderán las luminarias generales del showroom. Después podrás elegir una escena o ajustar cada zona de forma individual.":"Se apagarán las luminarias generales del showroom y cualquier escena activa.",o=e?"Sí, encender":"Sí, apagar";return`
      <div class="dialog-backdrop" data-action="cancel-power-confirm">
        <section class="dialog-card" role="dialog" aria-modal="true" aria-labelledby="power-dialog-title" data-dialog-card>
          ...
          <div class="dialog-actions">
            <button class="secondary-button" data-action="cancel-power-confirm" ${r?"disabled":""}>Cancelar</button>
            <button class="primary-button ${e?"confirm-on":""}" data-action="confirm-power" ${r?"disabled":""}>${r?"Ejecutando…":this._escape(o)}</button>
          </div>
        </section>
      </div>
    `}_renderNavigation(){return`
      <nav class="view-navigation" aria-label="Secciones del showroom">
        ${[["home","bulb","Inicio"],["lights","spot","Iluminación"],["energy","energy","Energía"],["system","health","Sistema"]].map(([t,i,r])=>`
          <button
    ...
    `}render(){var g;if(!this.shadowRoot||!this._hass)return;this._captureEnergyChartScroll(),this.setAttribute("data-theme",this._theme);const e=this._config(),t=e.staticOnly||e.panelKind==="general",i=this._state(e.weather),r=(i==null?void 0:i.attributes)||{},a=i==null?void 0:i.state,n=this._theme==="dark"?"claro":"oscuro",o=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],s=o.filter(p=>this._visibleSwitchState(p.entity)==="on").length,l=(g=this._state(e.mediaPlayer))==null?void 0:g.state,c=l==="playing"?"Reproduciendo":l==="paused"?"En pausa":"Detenido",d=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`;this.shadowRoot.innerHTML=`
      <style>
        :host {
          --primary: #f26522;
      ...
      <div class="app-shell">
        <header class="topbar">
          ...
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(O[a]||a||"Sin datos")}, ${this._escape(r.temperature??"Sin datos")}${this._escape(r.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(L[a]||"·")}</span>
              <strong>${this._escape(r.temperature??"--")}${this._escape(r.temperature_unit??"°")}</strong>
            </div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar a tema ${n}" title="Cambiar a tema ${n}">${Y}</button>
          </div>
        </header>
    ...
    `;const e=this.shadowRoot.querySelector("[data-track]");this._track=e,e.addEventListener("pointerdown",i=>this._onPointerDown(i)),e.addEventListener("pointermove",i=>this._onPointerMove(i)),e.addEventListener("pointerup",i=>this._onPointerUp(i)),e.addEventListener("pointercancel",i=>this._onPointerUp(i)),(t=this.shadowRoot.querySelector("[data-nav]"))==null||t.addEventListener("click",i=>this._onNavClick(i))}_panelConfigs(){const e=this._config.workspace_panels||this._config.workspacePanels,t=Array.isArray(e)?e:[],i=new Map(t.map(a=>{const n=a&&typeof a=="object"?a:{};return[C(n.id||n.panel_id||n.panelId||n.panel_kind),n]})),r=C(this._config.panel_id||this._config.panelId||this._config.panel_kind);return R.map(a=>{const n=a.id==="general"?K:a.id==="lobby"?G:{},o=i.get(a.id)||{},s=a.id===r?this._config:{},l={...I(n),...I(o),...I(s)};return a.id==="lobby"&&Array.isArray(s.devices)&&!Array.isArray(s.spots)&&(l.spots=s.devices),a.id==="general"?(l.panel_kind="general",l.static_only=!0):a.id==="lobby"?l.panel_kind="lobby":(delete l.panel_kind,delete l.static_only),{...a,config:l}})}_mountPages(){this._track&&(this._track.innerHTML="",this._pages=[],this._panelConfigs().forEach((e,t)=>{const i=document.createElement("section");i.className="page",i.dataset.panelId=e.id,i.setAttribute("aria-label",e.label),i.setAttribute("aria-hidden",String(e.id!==this._activeId)),i.inert=e.id!==this._activeId;const r=document.createElement("showroom-panel");r.panel={config:e.config},r.narrow=this._narrow,this._hass&&(r.hass=this._hass),i.append(r),this._track.append(i),this._pages.push(i),t===0&&(i.dataset.first="true")}),this._renderNav(),this._snap(!1))}_activeIndex(){const e=this._pages.findIndex(t=>t.dataset.panelId===this._activeId);return e>=0?e:0}_renderNav(){var i;const e=(i=this.shadowRoot)==null?void 0:i.querySelector("[data-nav]");if(!e)return;const t=this._activeIndex();e.innerHTML=`
      <button type="button" data-direction="prev" aria-label="Panel anterior" ${t===0?"disabled":""}>‹</button>
      ${R.map(r=>`<button type="button" class="dot" data-panel="${r.id}" aria-label="Abrir ${r.label}" aria-current="${r.id===this._activeId?"page":"false"}"></button>`).join("")}
      <button type="button" data-direction="next" aria-label="Panel siguiente" ${t===this._pages.length-1?"disabled":""}>›</button>
    `}_onNavClick(e){const t=e.target.closest("button");t&&(t.dataset.panel?this._goTo(t.dataset.panel):this._goTo(this._activeIndex()+(t.dataset.direction==="next"?1:-1)))}_onPointerDown(e){var r,a;const t=e.target,i=e.composedPath().some(n=>n instanceof HTMLElement&&!!n.closest("button,a,input,textarea,select,[data-no-swipe]"))||!!t.closest("button,a,input,textarea,select,[data-no-swipe]");this._drag={pointerId:e.pointerId,x:e.clientX,y:e.clientY,time:performance.now(),ignored:i},this._dragging=!1,i||(a=(r=e.currentTarget).setPointerCapture)==null||a.call(r,e.pointerId)}_onPointerMove(e){if(!this._drag||this._drag.ignored||this._drag.pointerId!==e.pointerId||!this._track)return;const t=e.clientX-this._drag.x,i=e.clientY-this._drag.y;!this._dragging&&(Math.abs(t)<8||Math.abs(t)<=Math.abs(i)*1.15)||(this._dragging=!0,this._track.classList.add("is-dragging"),this._track.style.transform=`translate3d(calc(${this._activeIndex()*-100}% + ${t}px), 0, 0)`,e.preventDefault())}_onPointerUp(e){var n;if(!this._drag||this._drag.pointerId!==e.pointerId)return;const t=e.clientX-this._drag.x,i=Math.max(1,performance.now()-this._drag.time),r=this._dragging&&(Math.abs(t)>56||Math.abs(t/i)>.45),a=t<0?1:-1;(n=this._track)==null||n.classList.remove("is-dragging"),this._drag=null,this._dragging=!1,r?this._goTo(this._activeIndex()+a):this._snap(!0)}_goTo(e){const t=typeof e=="string"?this._pages.findIndex(i=>i.dataset.panelId===e):e;if(t<0||t>=this._pages.length){this._snap(!0);return}this._activeId=this._pages[t].dataset.panelId||this._activeId,this._pages.forEach(i=>{const r=i.dataset.panelId===this._activeId;i.setAttribute("aria-hidden",String(!r)),i.inert=!r}),this._renderNav(),this._snap(!0),this.dispatchEvent(new CustomEvent("witmind-panel-change",{detail:{panelId:this._activeId},bubbles:!0,composed:!0}))}_snap(e=!0){this._track&&(this._track.classList.toggle("is-dragging",!e),this._track.style.transform=`translate3d(${this._activeIndex()*-100}%, 0, 0)`)}}customElements.get("witmind-workspace")||customElements.define("witmind-workspace",X);const W=()=>{var _,e;return((e=(_=globalThis.crypto)==null?void 0:_.randomUUID)==null?void 0:e.call(_))||`${Date.now()}-${Math.random().toString(16).slice(2)}`};class Q{constructor(e=window.parent){this.target=e,this.states=new Map,this.listeners=new Set,this.pending=new Map,this.subscriptions=new Map,window.addEventListener("message",t=>this.onMessage(t)),this.post({type:"WITMIND_READY"})}subscribeEntities(e,t){return this.listeners.add(t),this.post({type:"WITMIND_SUBSCRIBE_ENTITIES",entityIds:[...new Set(e)]}),t(Object.fromEntries(this.states)),()=>this.listeners.delete(t)}getEntity(e){return this.states.get(e)}callService(e,t={},i){return this.request("WITMIND_CALL_SERVICE","WITMIND_SERVICE_RESULT",{service:e,serviceData:t,target:i})}toggleMenu(){this.post({type:"WITMIND_TOGGLE_MENU"})}dbRequest(e,t={}){return this.request("WITMIND_DB_REQUEST","WITMIND_DB_RESULT",{command:e,payload:t})}haRequest(e,t={}){return this.request("WITMIND_HA_COMMAND","WITMIND_HA_RESULT",{command:e,payload:t})}haSubscribe(e,t,i){const r=W(),a=()=>{this.subscriptions.delete(r),this.post({type:"WITMIND_HA_UNSUBSCRIBE",requestId:r})};return new Promise((n,o)=>{const s=window.setTimeout(()=>{this.pending.delete(r),o(new Error("Timeout esperando suscripción HA"))},1e4);this.pending.set(r,{resolve:()=>{window.clearTimeout(s),n(a)},reject:o,timer:s}),this.subscriptions.set(r,i),this.post({type:"WITMIND_HA_SUBSCRIBE",requestId:r,command:e,payload:t})})}post(e){this.target.postMessage({protocol:1,source:"witmind-ui",...e},"*")}request(e,t,i){const r=W();return new Promise((a,n)=>{const o=window.setTimeout(()=>{this.pending.delete(r),n(new Error(`Timeout esperando ${t}`))},1e4);this.pending.set(r,{resolve:a,reject:n,timer:o}),this.post({type:e,requestId:r,...i})})}onMessage(e){var r,a,n;if(e.source!==this.target||((r=e.data)==null?void 0:r.protocol)!==1||((a=e.data)==null?void 0:a.source)!=="witmind-ha")return;const t=e.data;if(t.type==="WITMIND_HA_EVENT"){(n=this.subscriptions.get(t.requestId))==null||n(t.result);return}if(t.type==="WITMIND_ENTITY_UPDATE"){Object.entries(t.states||{}).forEach(([s,l])=>this.states.set(s,l)),(t.removed||[]).forEach(s=>this.states.delete(s));const o=Object.fromEntries(this.states);this.listeners.forEach(s=>s(o));return}const i=this.pending.get(t.requestId);i&&(this.pending.delete(t.requestId),window.clearTimeout(i.timer),t.ok?i.resolve(t.result):i.reject(new Error(String(t.error||"Witmind request failed"))))}}const q=["weather.forecast_casa","media_player.showroom_1","sensor.showroom_luminarias_encendidas","sensor.showroom_energia_estimada","sensor.21051182g_battery_level","switch.interruptor_inteligente_switch_1","switch.interruptor_inteligente_switch_2","switch.interruptor_inteligente_switch_3","switch.interruptor_inteligente_switch_4","switch.interruptor_inteligente_2_switch_1","switch.interruptor_inteligente_2_switch_2","switch.interruptor_inteligente_2_switch_3","switch.interruptor_inteligente_2_switch_4","switch.smart_relay_switch_3_switch","switch.smart_relay_switch_4_switch","switch.interruptor_inteligente_3_switch_1","switch.interruptor_inteligente_3_switch_2","switch.interruptor_inteligente_3_switch_3","switch.interruptor_inteligente_3_switch_4","scene.presentacion","scene.reunion","scene.visita","scene.regular","script.showroom_encendido_general","script.showroom_apagado_general"],z=(_,e=new Set)=>(typeof _=="string"&&/^[a-z_]+\.[a-z0-9_]+$/i.test(_)?e.add(_):Array.isArray(_)?_.forEach(t=>z(t,e)):_&&typeof _=="object"&&Object.values(_).forEach(t=>z(t,e)),[...e]);class Z extends HTMLElement{constructor(){super(...arguments),this.states={},this.previousStates={},this.eventListeners=new Set,this.panelConfig={},this.messageHandler=e=>{var i,r;if(e.source!==window.parent||((i=e.data)==null?void 0:i.protocol)!==1||((r=e.data)==null?void 0:r.source)!=="witmind-ha")return;e.data.type==="WITMIND_INIT"&&e.data.panelConfig&&(this.panelConfig=e.data.panelConfig,this.applyPanelConfig(),this.resubscribeWithConfig());const t=e.data.theme;(t==="light"||t==="dark")&&this.panel&&this.panel.setAttribute("theme",t)}}connectedCallback(){this.attachShadow({mode:"open"}),window.addEventListener("message",this.messageHandler),this.shadowRoot.innerHTML="<style>:host{display:block;min-height:100dvh;background:var(--wit-surface,#071118)} witmind-workspace{display:block;min-height:100dvh}</style><witmind-workspace></witmind-workspace>",this.panel=this.shadowRoot.querySelector("witmind-workspace"),this.panel.addEventListener("hass-toggle-menu",()=>{var e;return(e=this.client)==null?void 0:e.toggleMenu()}),this.client=new Q(window.parent),this.applyPanelConfig(),this.subscribe(q),this.panel.hass=this.createHassAdapter()}subscribe(e){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=this.client.subscribeEntities(e,i=>{this.previousStates=this.states,this.states=i,this.panel.hass=this.createHassAdapter(),this.emitStateChanges()})}resubscribeWithConfig(){const e=[...new Set([...q,...z(this.panelConfig)])];this.subscribe(e)}applyPanelConfig(){this.panel&&(this.panel.config=this.panelConfig)}disconnectedCallback(){var e;(e=this.unsubscribe)==null||e.call(this),window.removeEventListener("message",this.messageHandler)}createHassAdapter(){const e=this.client;return{states:this.states,language:"es",selectedTheme:null,callService:(i,r,a={},n)=>e.callService(`${i}.${r}`,a,n),callWS:async i=>i.type==="get_states"?Object.values(this.states):e.haRequest(String(i.type||""),i),callApi:async(i,r)=>[],connection:{subscribeEvents:async(i,r)=>{const a=n=>{(!r||r==="state_changed")&&i(n)};return this.eventListeners.add(a),()=>this.eventListeners.delete(a)},sendMessagePromise:i=>e.haRequest(String(i.type||""),i),subscribeMessage:(i,r)=>e.haSubscribe(String(r.type||""),r,i)}}}emitStateChanges(){Object.entries(this.states).forEach(([e,t])=>{this.previousStates[e]!==t&&this.eventListeners.forEach(i=>i({event_type:"state_changed",data:{entity_id:e,new_state:t,old_state:this.previousStates[e]||null}}))})}}customElements.define("witmind-ui-app",Z);

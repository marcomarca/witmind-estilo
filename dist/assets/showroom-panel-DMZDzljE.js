...
`;class j extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",e=>this._handleClick(e)),this.shadowRoot.addEventListener("scroll",e=>this._handleEnergyScroll(e),!0)}set hass(e){var c,i,l,o;const t=this._hass,r=this._config(),a=this._relevantHassChanged(t,e),s=!t||((c=t.states)==null?void 0:c[r.energySensor])!==((i=e==null?void 0:e.states)==null?void 0:i[r.energySensor]),n=!((l=t==null?void 0:t.states)!=null&&l[r.energySensor])&&!!((o=e==null?void 0:e.states)!=null&&o[r.energySensor]);this._hass=e,a&&this._syncStatesFromHass(),this.isConnected&&(this._started?s&&(n?this._loadEnergyStatistics():this._scheduleEnergyRefresh()):(this._started=!0,this._start()),a&&this._updateStatePresentation())}get hass(){return this._hass}set panel(e){const t=this._config().weather;this._panel=e;const r=this._config().weather;this._config().panelKind==="general"&&this._activeView==="home"&&(this._activeView="lights"),this._started&&t!==r&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._updateStatePresentation())}get panel(){return this._panel}set narrow(e){this._narrow=!!e,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._started||(this._started=!0,this._start()),this._updateStatePresentation(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const e of this._switchTimers.values())clearTimeout(e);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_updateStatePresentation(){var c;if(!((c=this.shadowRoot)!=null&&c.querySelector(".app-shell"))){this._requestRender();return}const e=i=>{var h;const l=i.dataset.entity;if(!l)return;const o=this._state(l),d=this._visibleSwitchState(l),g=d==="on",p=!o||["unknown","unavailable"].includes(o.state),u=this._pendingSwitches.has(l),m=this._switchErrors.get(l),f=m||(u?d==="on"?"Encendiendo…":"Apagando…":p?"No disponible":g?"Encendido":"Apagado");i.classList.toggle("is-on",g),i.classList.toggle("is-pending",u),i.classList.toggle("is-error",!!m),i.disabled=p,i.setAttribute("aria-pressed",String(g));const S=((h=i.querySelector(".device-copy strong"))==null?void 0:h.textContent)||l;i.setAttribute("aria-label",`${S}: ${f}`);const v=i.querySelector(".device-copy small");v&&(v.textContent=f)};this.shadowRoot.querySelectorAll('[data-action="toggle-switch"]').forEach(e),this.shadowRoot.querySelectorAll('[data-action="run-scene"]').forEach(i=>{const l=this._allScenes().find(u=>u.key===i.dataset.sceneKey);if(!l)return;const o=this._sceneStatus(l),d=this._pendingAction===l.key,g=d?"Aplicando...":o.active?"Activo":o.unavailable?"Sin datos":"Inactivo";i.classList.toggle("is-pending",d),i.classList.toggle("is-active",o.active),i.classList.toggle("is-unavailable",o.unavailable),i.disabled=!!(this._pendingAction&&!d),i.setAttribute("aria-pressed",String(o.active));const p=i.querySelector(".scene-state");p&&(p.textContent=g)});const t=this._config(),r=[...t.spots,...t.samples,...t.reflector?[t.reflector]:[]],a=r.filter(i=>this._visibleSwitchState(i.entity)==="on").length,s=this.shadowRoot.querySelector("[data-lights-summary]");s&&(s.textContent=`${a} de ${r.length}`);const n=this.shadowRoot.querySelector("[data-energy-summary]");n&&(n.textContent=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`)}_handleClick(e){const t=e.target.closest("[data-action]");if(!t||!this._hass)return;const r=t.dataset.action;if(r==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(r==="toggle-theme"){this._toggleTheme();return}if(r==="set-view"){const a=t.dataset.view;["home","lights","energy","system"].includes(a)&&(this._activeView=a,this._requestRender());return}if(r==="toggle-switch"){this._toggleSwitch(t.dataset.entity);return}if(r==="run-scene"){this._runScene(t.dataset.sceneKey||t.dataset.entity,t.dataset.label);return}if(r==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(r==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(r==="general-off"){const a=this._config().generalOffScript;a?this._hass.callService("script","turn_on",{entity_id:a}).catch(s=>{this._notify("No se pudo ejecutar Apagar todo Witmind.","error"),console.error("Error ejecutando el apagado general de Witmind:",s)}):this._notify("El apagado general aún no tiene una entidad configurada.","error");return}if(r==="cancel-power-confirm"){const a=e.target.closest("[data-dialog-card]");if(t.classList.contains("dialog-backdrop")&&a)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(r==="confirm-power"){this._confirmGeneralPower();return}if(r==="clear-scene"){this._clearScene();return}if(r==="media"){this._mediaAction(t.dataset.service);return}if(r==="energy-range"){this._setEnergyRange(t.dataset.range);return}if(r==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(r==="energy-day-next"){this._shiftEnergyDay(1);return}if(r==="energy-day-today"){this._resetEnergyDay();return}r==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(e){console.warn("No se pudo guardar el tema del showroom:",e)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this.dispatchEvent(new CustomEvent("witmind-theme-change",{detail:{theme:this._theme},bubbles:!0,composed:!0})),this._requestRender()}set theme(e){e!=="dark"&&e!=="light"||(this._theme=e,this.setAttribute("data-theme",e),this._saveTheme())}get theme(){return this._theme}_config(){var u;const e=((u=this._panel)==null?void 0:u.config)||{},t=e.panel_kind==="lobby"||e.panel_kind==="general"||e.static_only===!0||e.staticOnly===!0,r=m=>Array.isArray(m)?[...new Set(m.filter(Boolean).map(f=>String(f)))]:[],a=(m,f)=>(Array.isArray(m)&&(m.length||t)?m:f).filter(v=>v==null?void 0:v.entity).map((v,h)=>({entity:String(v.entity),name:v.name||`Dispositivo ${h+1}`,subtitle:v.subtitle||"Iluminación",icon:v.icon||"bulb"})),s=a(e.spots,b.spots),n=a(e.samples||e.muestras,b.samples),c=[...s,...n].map(m=>m.entity),i=r(e.scene_control_entities||e.sceneControlEntities),l=i.length?i:c,o=(m,f,S)=>(Array.isArray(m)&&(m.length||t)?m:f).filter(h=>(h==null?void 0:h.entity)||(h==null?void 0:h.id)||(h==null?void 0:h.key)).map((h,w)=>{const k=h.entity?String(h.entity):"",A=String(h.id||h.key||k||`${S}-${w+1}`),y=f.find($=>{const z=$.entity?String($.entity):"",O=String($.id||$.key||z||"");return k&&z===k||O===A}),E=k||(y!=null&&y.entity?String(y.entity):""),M=String(h.id||h.key||(y==null?void 0:y.id)||E||`${S}-${w+1}`),_=E||`${S}:${M}`,x=Array.isArray(h.on_entities)||Array.isArray(h.onEntities),D=Array.isArray(h.off_entities)||Array.isArray(h.offEntities),N=r(x?h.on_entities||h.onEntities:y==null?void 0:y.onEntities),C=r(D?h.off_entities||h.offEntities:[]),R=D?C.filter($=>!N.includes($)):l.filter($=>!N.includes($));return{key:_,id:M,entity:E,name:h.name||(y==null?void 0:y.name)||`Escena ${w+1}`,subtitle:h.subtitle||(y==null?void 0:y.subtitle)||"Escena del showroom",icon:h.icon||(y==null?void 0:y.icon)||"presentation",directOnly:h.direct_only??h.directOnly??(y==null?void 0:y.directOnly)??!1,onEntities:N,offEntities:R}}),d=e.reflector||b.reflector,g=Number(e.history_hours??e.historyHours),p=Number(e.chart_hours??e.chartHours);return{title:e.title||b.title,subtitle:e.subtitle||b.subtitle,siteLabel:e.site_label||e.siteLabel||b.siteLabel,logo:e.logo||b.logo,weather:e.weather||b.weather,mediaPlayer:e.media_player||e.mediaPlayer||b.mediaPlayer,lightCountSensor:e.light_count_sensor||e.lightCountSensor||b.lightCountSensor,energySensor:e.energy_sensor||e.energySensor||b.energySensor,batteryLevel:e.battery_level||e.batteryLevel||b.batteryLevel,powerOnScript:e.power_on_script||e.powerOnScript||b.powerOnScript,powerOffScript:e.power_off_script||e.powerOffScript||b.powerOffScript,historyHours:Number.isFinite(g)&&g>0?Math.min(24,g):b.historyHours,chartHours:Number.isFinite(p)&&p>0?Math.min(72,p):b.chartHours,showForecast:e.show_forecast??e.showForecast??b.showForecast,panelKind:e.panel_kind||e.panelKind||"showroom",staticOnly:e.static_only??e.staticOnly??!1,generalOffScript:e.general_off_script||e.generalOffScript||"",spots:s,samples:n,sceneControlEntities:l,reflector:d!=null&&d.entity?{entity:String(d.entity),name:d.name||b.reflector.name,subtitle:d.subtitle||b.reflector.subtitle,icon:d.icon||b.reflector.icon}:null,scenes:o(e.scenes,b.scenes,"scene"),sampleScenes:o(e.sample_scenes||e.sampleScenes,b.sampleScenes,"sample")}}_allDevices(){const e=this._config();return[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]]}_allScenes(e=this._config()){return[...e.scenes,...e.sampleScenes]}_trackedEntities(){const e=this._config();return new Set([...this._allDevices().map(t=>t.entity),...e.sceneControlEntities,...this._allScenes(e).flatMap(t=>[t.entity,...t.onEntities,...t.offEntities]),e.weather,e.mediaPlayer,e.lightCountSensor,e.energySensor,e.batteryLevel,e.powerOnScript,e.powerOffScript].filter(Boolean))}_relevantHassChanged(e,t){var r,a;if(!e||!t)return!0;for(const s of this._trackedEntities())if(((r=e.states)==null?void 0:r[s])!==((a=t.states)==null?void 0:a[s]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var e;if((e=this._hass)!=null&&e.states)for(const t of this._trackedEntities()){const r=this._hass.states[t];r&&this._applyLiveState(t,r)}}_applyLiveState(e,t){if(!t){this._liveStates.delete(e);return}this._liveStates.set(e,t);const r=this._pendingSwitches.get(e);if(r&&t.state===r.desired){this._pendingSwitches.delete(e),this._switchErrors.delete(e);const a=this._switchTimers.get(e);a&&clearTimeout(a),this._switchTimers.delete(e)}}async _fetchCurrentStates(){var e;if((e=this._hass)!=null&&e.callWS)try{const t=await this._hass.callWS({type:"get_states"}),r=this._trackedEntities();for(const a of t||[])r.has(a.entity_id)&&this._applyLiveState(a.entity_id,a);this._updateStatePresentation()}catch(t){console.error("No se pudieron sincronizar los estados del showroom:",t)}}async _subscribeStateChanges(){var e;if(!(!((e=this._hass)!=null&&e.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(t=>{var a;const r=(a=t==null?void 0:t.data)==null?void 0:a.entity_id;!r||!this._trackedEntities().has(r)||(this._applyLiveState(r,t.data.new_state),r===this._config().energySensor&&this._scheduleEnergyRefresh(),this._updateStatePresentation())},"state_changed")}catch(t){console.error("No se pudo suscribir a state_changed:",t)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var t;const e=this._config();if(!(!((t=this._hass)!=null&&t.connection)||!e.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===e.weather)){this._resetForecastSubscription();try{this._forecastEntity=e.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(r=>{this._forecast=Array.isArray(r==null?void 0:r.forecast)?r.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:e.weather})}catch(r){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",r)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(e=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const t=this._energyRefreshInterval(),r=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,a=e?0:this._energyLastLoadedAt?Math.max(1e3,t-r):t;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),a)}_energyRangeDefinition(e=this._energyRange){const t=new Date;let r,a=t,s,n,c,i=!1;if(e==="year")r=new Date(t.getFullYear(),0,1,0,0,0,0),s="month",n=`Año ${t.getFullYear()}`,c="mes";else if(e==="month")r=new Date(t.getFullYear(),t.getMonth(),1,0,0,0,0),s="day",n=t.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),c="día";else{const l=new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0);r=new Date(l.getFullYear(),l.getMonth(),l.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),i=r.getTime()===l.getTime(),a=i?t:new Date(r.getFullYear(),r.getMonth(),r.getDate()+1,0,0,0,0),s="hour",n=r.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),c="hora"}return{start:r,end:a,period:s,title:n,intervalLabel:c,isToday:i}}_energyValueToKWh(e,t){const r=Number(e);if(!Number.isFinite(r))return null;const a=String(t||"kWh").trim().toLowerCase().replaceAll(" ","");return a==="kwh"?r:a==="wh"?r/1e3:a==="mwh"?r*1e3:null}_deriveEnergyStateDeltas(e,t,r){const a=Number(t instanceof Date?t.getTime():t),s=Number(r instanceof Date?r.getTime():r),n=e.filter(l=>Number.isFinite(Number(l.start))&&Number.isFinite(Number(l.state))).sort((l,o)=>Number(l.start)-Number(o.start)),c=[];let i=null;for(const l of n){const o=Number(l.start),d=Number(l.state);if(o<a){i=d;continue}if(o>=s)break;let g=0;if(Number.isFinite(i)){const p=d-i;g=p>=0?p:Math.max(0,d)}c.push({...l,change:g}),i=d}return c}_aggregateEnergyDayRows(e,t,r){var l;const a=new Date(t.start),s=new Date(t.end),n=t.isToday?new Date(s.getFullYear(),s.getMonth(),s.getDate(),s.getHours(),0,0,0):new Date(a.getFullYear(),a.getMonth(),a.getDate(),23,0,0,0),c=new Map;for(let o=new Date(a);o<=n;o.setHours(o.getHours()+1)){const d=o.getTime();c.set(d,{start:d,end:new Date(o.getFullYear(),o.getMonth(),o.getDate(),o.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:t.isToday&&d===n.getTime(),live:!1})}let i=null;for(const o of e){const d=new Date(Number(o.start));if(!Number.isFinite(d.getTime()))continue;const g=new Date(d.getFullYear(),d.getMonth(),d.getDate(),d.getHours(),0,0,0).getTime(),p=c.get(g);p&&(p.change+=Math.max(0,Number(o.change)||0),p.samples+=1,Number.isFinite(o.state)&&(!i||Number(o.end||o.start)>Number(i.end||i.start))&&(i=o))}if(t.isToday){const o=this._energyValueToKWh(r==null?void 0:r.state,(l=r==null?void 0:r.attributes)==null?void 0:l.unit_of_measurement),d=i==null?void 0:i.state,g=Number((i==null?void 0:i.end)||(i==null?void 0:i.start)),p=c.get(n.getTime());if(p&&Number.isFinite(o)&&Number.isFinite(d)&&Number.isFinite(g)&&g>=n.getTime()&&s.getTime()-g>=0&&s.getTime()-g<=900*1e3){const u=o-d;Number.isFinite(u)&&u>=0&&(p.change+=u,p.live=u>0)}}return[...c.values()].filter(o=>o.samples>0||o.live)}_calculateCurrentMonthEnergy(e,t,r){var g;const a=new Date(t),s=new Date(a.getFullYear(),a.getMonth(),1,0,0,0,0).getTime(),n=e.filter(p=>Number.isFinite(Number(p.start))&&Number(p.start)>=s).sort((p,u)=>Number(p.start)-Number(u.start));let c=n.reduce((p,u)=>p+Math.max(0,Number(u.change)||0),0),i=null;for(const p of n)Number.isFinite(Number(p.state))&&(!i||Number(p.end||p.start)>Number(i.end||i.start))&&(i=p);const l=this._energyValueToKWh(r==null?void 0:r.state,(g=r==null?void 0:r.attributes)==null?void 0:g.unit_of_measurement),o=Number(i==null?void 0:i.state),d=Number((i==null?void 0:i.end)||(i==null?void 0:i.start));if(Number.isFinite(l)&&Number.isFinite(o)&&Number.isFinite(d)&&d>=s&&d<=a.getTime()&&a.getTime()-d<=7200*1e3){const p=l-o;Number.isFinite(p)&&p>=0&&(c+=p)}return n.length||Number.isFinite(o)?c:null}_aggregateEnergyCalendarRows(e,t,r,a){var d;if(!["month","year"].includes(a))return[];const s=new Map;let n=null;for(const g of e){const p=Number(g.start);if(!Number.isFinite(p))continue;const u=new Date(p),m=a==="month"?new Date(u.getFullYear(),u.getMonth(),u.getDate(),0,0,0,0).getTime():new Date(u.getFullYear(),u.getMonth(),1,0,0,0,0).getTime(),f=s.get(m)||{start:m,change:0,samples:0,partial:!1,live:!1};f.change+=Math.max(0,Number(g.change)||0),f.samples+=1,s.set(m,f),Number.isFinite(Number(g.state))&&(!n||p>Number(n.start))&&(n=g)}const c=this._energyValueToKWh(r==null?void 0:r.state,(d=r==null?void 0:r.attributes)==null?void 0:d.unit_of_measurement),i=Number(n==null?void 0:n.state),l=Number(n==null?void 0:n.start),o=new Date(t.end);if(Number.isFinite(c)&&Number.isFinite(i)&&Number.isFinite(l)&&l<=o.getTime()&&o.getTime()-l<=7200*1e3){const g=c-i;if(Number.isFinite(g)&&g>=0){const p=a==="month"?new Date(o.getFullYear(),o.getMonth(),o.getDate(),0,0,0,0).getTime():new Date(o.getFullYear(),o.getMonth(),1,0,0,0,0).getTime(),u=s.get(p)||{start:p,change:0,samples:0,partial:!1,live:!1};u.change+=g,u.live=g>0,s.set(p,u)}}return[...s.values()].sort((g,p)=>Number(g.start)-Number(p.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(e=>[Number(e.start),Number(e.change)||0,Number(e.samples)||0,!!e.partial,!!e.live])])}async _loadEnergyStatistics(){var p;if(!((p=this._hass)!=null&&p.connection))return;clearTimeout(this._energyRefreshTimer);const e=this._config(),t=this._state(e.energySensor),r=Number(t==null?void 0:t.state);if(!t){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${e.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(r)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${e.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const a=++this._energyRequestId,s=this._energyViewSignature(),n=this._energyData.length>0,c=this._energyRangeDefinition(),i=new Date,l=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0),o=new Date(c.start.getTime()-3600*1e3),d=this._energyRange==="year"?"hour":"5minute",g=new Date(l.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,n||this._requestRender();try{const u=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[e.energySensor]});if(a!==this._energyRequestId)return;const f=(Array.isArray(u)?u:[]).find(_=>(_==null?void 0:_.statistic_id)===e.energySensor)||null;if(!f||!f.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const S=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:o.toISOString(),end_time:c.end.toISOString(),statistic_ids:[e.energySensor],period:d,units:{energy:"kWh"},types:["state"]}),v=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:g.toISOString(),end_time:i.toISOString(),statistic_ids:[e.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[h,w]=await Promise.all([S,v]);if(a!==this._energyRequestId)return;const k=_=>(Array.isArray(_==null?void 0:_[e.energySensor])?_[e.energySensor]:[]).map(x=>({start:Number(x.start),end:Number(x.end),change:x.change===void 0||x.change===null?null:Math.max(0,Number(x.change)||0),state:x.state===void 0||x.state===null?null:Number(x.state)})).filter(x=>Number.isFinite(x.start)&&(Number.isFinite(x.state)||Number.isFinite(x.change))),A=k(h),y=this._deriveEnergyStateDeltas(A,c.start,c.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows(y,c,t):this._aggregateEnergyCalendarRows(y,c,t,this._energyRange);const E=k(w),M=this._deriveEnergyStateDeltas(E,l,i);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(M,i,t),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((_,x)=>_+Math.max(0,Number(x.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const _=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0).getTime(),x=this._energyData.find(D=>Number(D.start)===_);x&&(x.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(u){if(a!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(u==null?void 0:u.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",u)}finally{if(a===this._energyRequestId){this._energyLoading=!1;const u=s!==this._energyViewSignature();!n||u&&this._activeView==="energy"?this._requestRender():this._updateStatePresentation(),this._scheduleEnergyRefresh()}}}_setEnergyRange(e){!["day","month","year"].includes(e)||e===this._energyRange||(this._energyRange=e,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(e){if(this._energyRange!=="day")return;const t=Number(e);if(!Number.isFinite(t)||t===0)return;const r=Math.min(0,this._energyDayOffset+t);r!==this._energyDayOffset&&(this._energyDayOffset=r,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(e,t=2){const r=Number(e);return Number.isFinite(r)?r.toLocaleString("es-BO",{minimumFractionDigits:t,maximumFractionDigits:t}):"--"}_energyLabel(e,t=this._energyRange){const r=new Date(Number(e));return Number.isFinite(r.getTime())?t==="year"?r.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):t==="month"?String(r.getDate()):r.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=t<=0||t-e.scrollLeft<=8}_handleEnergyScroll(e){var a,s;const t=(s=(a=e.target)==null?void 0:a.closest)==null?void 0:s.call(a,"[data-energy-scroll]");if(!t)return;const r=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=r<=0||r-t.scrollLeft<=8}_restoreEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){e.scrollLeft=t,this._energyScrollLeft=e.scrollLeft;return}e.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,t))}_energyChart(){const e=this._energyData;if(this._energyLoading&&!e.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!e.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const t=280,r=14,a=16,s=22,n=42,c=this._energyRange==="day"?64:this._energyRange==="month"?36:58,i=Math.max(620,e.length*c+r+a),l=t-s-n,o=i-r-a,d=Math.max(...e.map(h=>h.change),.001),g=this._energyRange==="day"?10:e.length>24?4:7,p=Math.max(6,(o-g*Math.max(0,e.length-1))/e.length),u=this._energyRange==="month"?Math.max(1,Math.ceil(e.length/10)):this._energyRange==="day"?2:1,m=[0,.25,.5,.75,1],f=m.map(h=>{const w=s+l*(1-h);return`<line x1="${r}" y1="${w.toFixed(1)}" x2="${i-a}" y2="${w.toFixed(1)}" class="energy-grid-line"/>`}).join(""),S=m.map(h=>{const w=s+l*(1-h),k=this._formatEnergy(d*h,d<1?2:1);return`<span class="energy-y-tick" style="top:${w.toFixed(1)}px">${this._escape(k)}</span>`}).join(""),v=e.map((h,w)=>{const k=r+w*(p+g),A=h.change>0?Math.max(2,h.change/d*l):1,y=s+l-A,E=w%u===0||w===e.length-1,M=this._energyLabel(h.start),_=h.partial?" · en curso":"",x=h.partial?`<text x="${(k+p/2).toFixed(1)}" y="${(s+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${h.partial?"is-current":""}"><rect x="${k.toFixed(1)}" y="${y.toFixed(1)}" width="${p.toFixed(1)}" height="${A.toFixed(1)}" rx="${Math.min(4,p/2).toFixed(1)}" class="energy-bar ${h.partial?"is-partial":""}"><title>${this._escape(M)} · ${this._formatEnergy(h.change)} kWh${_}</title></rect>${x}${E?`<text x="${(k+p/2).toFixed(1)}" y="${t-14}" text-anchor="middle" class="energy-axis-text">${this._escape(M)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${S}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${i}" height="${t}" viewBox="0 0 ${i} ${t}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${f}${v}</svg></div></div>`}async _loadHistory(){var c;const e=this._config();if(!((c=this._hass)!=null&&c.callApi))return;const t=Math.max(e.historyHours,e.chartHours),r=new Date(Date.now()-t*60*60*1e3).toISOString(),a=new Date().toISOString(),s=[e.lightCountSensor,...e.spots.map(i=>i.entity),...e.samples.map(i=>i.entity)].filter(Boolean).join(",");if(!s)return;const n=`history/period/${encodeURIComponent(r)}?filter_entity_id=${encodeURIComponent(s)}&end_time=${encodeURIComponent(a)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",n),this._historyError=""}catch(i){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",i)}this._requestRender()}_state(e){var t,r;return this._liveStates.get(e)||((r=(t=this._hass)==null?void 0:t.states)==null?void 0:r[e])}_isUnavailable(e){var r;const t=(r=this._state(e))==null?void 0:r.state;return!t||t==="unknown"||t==="unavailable"}_visibleSwitchState(e){var t,r;return((t=this._pendingSwitches.get(e))==null?void 0:t.desired)||((r=this._state(e))==null?void 0:r.state)||"unavailable"}async _toggleSwitch(e){if(!e||!this._hass)return;const t=this._state(e);if(!t||["unknown","unavailable"].includes(t.state)){this._switchErrors.set(e,"No disponible"),this._updateStatePresentation();return}const a=this._visibleSwitchState(e)==="on"?"off":"on",s=a==="on"?"turn_on":"turn_off",n=e.split(".")[0]||"switch";this._pendingSwitches.set(e,{desired:a,startedAt:Date.now()}),this._switchErrors.delete(e),this._updateStatePresentation();try{await this._hass.callService(n,s,{entity_id:e});const c=this._switchTimers.get(e);c&&clearTimeout(c);const i=setTimeout(()=>this._verifySwitchState(e,a),6e3);this._switchTimers.set(e,i)}catch(c){this._pendingSwitches.delete(e),this._switchErrors.set(e,"La acción falló"),this._updateStatePresentation(),console.error(`Error ejecutando ${s} en ${e}:`,c)}}async _verifySwitchState(e,t){var a;await this._fetchCurrentStates();const r=((a=this._state(e))==null?void 0:a.state)===t;this._pendingSwitches.delete(e),this._switchTimers.delete(e),r?this._switchErrors.delete(e):this._switchErrors.set(e,"Sin confirmación"),this._updateStatePresentation()}_sceneStatus(e){const t=[...e.onEntities.map(s=>({entityId:s,desired:"on"})),...e.offEntities.map(s=>({entityId:s,desired:"off"}))];if(!t.length)return{active:!1,unavailable:!1,mismatches:[]};const r=t.some(({entityId:s})=>this._isUnavailable(s)),a=t.filter(({entityId:s,desired:n})=>{var c;return((c=this._state(s))==null?void 0:c.state)!==n});return{active:!r&&a.length===0,unavailable:r,mismatches:a}}_sceneExpectations(e){return[...e.offEntities.map(t=>({entityId:t,desired:"off"})),...e.onEntities.map(t=>({entityId:t,desired:"on"}))]}_markExpectedStates(e){const t=Date.now();for(const{entityId:r,desired:a}of e)this._pendingSwitches.set(r,{desired:a,startedAt:t}),this._switchErrors.delete(r)}_clearExpectedStates(e){for(const{entityId:t}of e){this._pendingSwitches.delete(t);const r=this._switchTimers.get(t);r&&clearTimeout(r),this._switchTimers.delete(t)}}async _setEntitiesState(e,t){const r=[...new Set((e||[]).filter(Boolean))];if(!r.length)return;const a=new Map;for(const n of r){const c=n.split(".")[0];c&&(a.has(c)||a.set(c,[]),a.get(c).push(n))}const s=t==="on"?"turn_on":"turn_off";for(const[n,c]of a)await this._hass.callService(n,s,{entity_id:c})}async _waitForExpectedStates(e,t=7e3){const r=Date.now()+t;let a=e;for(;Date.now()<r;){if(await this._fetchCurrentStates(),a=e.filter(({entityId:s,desired:n})=>{var c;return((c=this._state(s))==null?void 0:c.state)!==n}),!a.length)return{ok:!0,mismatches:[]};await new Promise(s=>setTimeout(s,450))}return{ok:!1,mismatches:a}}async _runScene(e,t){var c;if(!e||this._pendingAction)return;const r=this._config(),a=this._allScenes(r).find(i=>i.key===e||i.entity===e);if(!a){this._notify("La escena no está configurada.","error");return}const s=this._sceneExpectations(a);this._pendingAction=a.key,this._markExpectedStates(s),this._updateStatePresentation();let n=null;try{if(a.entity&&!a.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:a.entity})}catch(o){n=o,console.warn(`La escena ${a.entity} no respondió; se aplicará el perfil directo.`,o)}await this._setEntitiesState(a.offEntities,"off"),await this._setEntitiesState(a.onEntities,"on");const i=await this._waitForExpectedStates(s);if(!i.ok){const o=i.mismatches.map(d=>d.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${o}`)}const l=n?`${t||"Modo"} aplicado mediante control directo.`:`${t||"Modo"} activo.`;this._notify(l,"success")}catch(i){for(const{entityId:l,desired:o}of s)((c=this._state(l))==null?void 0:c.state)!==o&&this._switchErrors.set(l,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",i)}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _executeGeneralPower(e,t={}){var l;if(!this._hass||this._pendingAction||!["on","off"].includes(e))return!1;const r=this._config(),a=e==="on"?r.powerOnScript:r.powerOffScript,s=r.sceneControlEntities.map(o=>({entityId:o,desired:e})),n=t.successMessage||(e==="on"?"Iluminación general encendida.":"Iluminación general apagada."),c=t.errorMessage||(e==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=a||`direct-power-${e}`,this._markExpectedStates(s),this._updateStatePresentation();let i=null;try{if(a)try{await this._hass.callService("script","turn_on",{entity_id:a})}catch(d){i=d,console.warn(`El script ${a} no respondió; se aplicará el control directo.`,d)}await this._setEntitiesState(r.sceneControlEntities,e);const o=await this._waitForExpectedStates(s);if(!o.ok){const d=o.mismatches.map(g=>g.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${d}`)}return this._confirmAction="",this._notify(n,"success"),!0}catch(o){for(const{entityId:d,desired:g}of s)((l=this._state(d))==null?void 0:l.state)!==g&&this._switchErrors.set(d,"Sin confirmación");return this._notify(c,"error"),console.error("Error ejecutando el control general del showroom:",{error:o,scriptError:i,desired:e}),!1}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._updateStatePresentation()}}async _confirmGeneralPower(){const e=this._confirmAction;["on","off"].includes(e)&&await this._executeGeneralPower(e,{successMessage:e==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(e){const t=this._config().mediaPlayer;if(!(!t||!e||this._isUnavailable(t)))try{await this._hass.callService("media_player",e,{entity_id:t})}catch(r){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${e}:`,r)}}_updateToastPresentation(){if(!this.shadowRoot)return;const e=this.shadowRoot.querySelector("[data-toast]");if(!this._toast){e==null||e.remove();return}if(e){e.className=`toast ${this._escape(this._toast.type)}`,e.textContent=this._toast.message;return}const t=document.createElement("div");t.dataset.toast="",t.className=`toast ${this._escape(this._toast.type)}`,t.setAttribute("role","status"),t.textContent=this._toast.message,this.shadowRoot.append(t)}_notify(e,t="success"){var r;clearTimeout(this._toastTimer),this._toast={message:e,type:t},(r=this.shadowRoot)!=null&&r.querySelector(".app-shell")?this._updateToastPresentation():this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._updateToastPresentation()},4200)}_historyMap(){var t;const e=new Map;for(const r of this._history||[]){const a=(t=r==null?void 0:r[0])==null?void 0:t.entity_id;a&&e.set(a,r)}return e}_historySegments(e,t,r){if(!Array.isArray(e)||!e.length)return[];const a=e.map(n=>({state:n.state,time:new Date(n.last_changed||n.last_updated).getTime()})).filter(n=>Number.isFinite(n.time)).sort((n,c)=>n.time-c.time);if(!a.length)return[];const s=[];for(let n=0;n<a.length;n+=1){const c=a[n],i=a[n+1],l=Math.max(t,c.time),o=Math.min(r,(i==null?void 0:i.time)??r);o<=l||s.push({state:c.state,left:(l-t)/(r-t)*100,width:(o-l)/(r-t)*100})}return s}_sparkline(e,t){var u;const r=this._historyMap().get(e)||[],a=Date.now(),s=a-t*60*60*1e3,n=r.map(m=>({value:Number(m.state),time:new Date(m.last_changed||m.last_updated).getTime()})).filter(m=>Number.isFinite(m.value)&&Number.isFinite(m.time)&&m.time>=s).sort((m,f)=>m.time-f.time),c=Number((u=this._state(e))==null?void 0:u.state);if(Number.isFinite(c)&&n.push({value:c,time:a}),!n.length)return{path:"",min:"--",max:"--",avg:"--"};const i=n.map(m=>m.value),l=Math.min(...i),o=Math.max(...i),d=i.reduce((m,f)=>m+f,0)/i.length,g=o-l||1;return{path:n.map((m,f)=>{const S=(m.time-s)/(a-s)*300,v=66-(m.value-l)/g*52;return`${f?"L":"M"}${Math.max(0,Math.min(300,S)).toFixed(1)},${v.toFixed(1)}`}).join(" "),min:this._formatNumber(l),max:this._formatNumber(o),avg:this._formatNumber(d)}}_formatNumber(e){return Number.isFinite(e)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(e):"--"}_updateClock(){var c,i,l;if(!this.shadowRoot)return;const e=new Date,t=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(e),r=((c=t.find(o=>o.type==="hour"))==null?void 0:c.value)||"--",a=((i=t.find(o=>o.type==="minute"))==null?void 0:i.value)||"--",s=(((l=t.find(o=>o.type==="dayPeriod"))==null?void 0:l.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const o of this.shadowRoot.querySelectorAll("[data-clock-time]"))o.textContent=`${r}:${a}`;for(const o of this.shadowRoot.querySelectorAll("[data-clock-period]"))o.textContent=s||"";const n=this.shadowRoot.querySelector("[data-current-time]");n&&(n.dateTime=e.toISOString(),n.setAttribute("aria-label",`${r}:${a} ${s}`.trim()))}_icon(e,t=""){const r=L[e]||L.bulb;return`<svg class="icon ${this._escape(t)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${r}</svg>`}_escape(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(e){const t=this._state(e.entity),r=this._visibleSwitchState(e.entity),a=r==="on",s=!t||["unknown","unavailable"].includes(t.state),n=this._pendingSwitches.has(e.entity),c=this._switchErrors.get(e.entity),i=c||(n?r==="on"?"Encendiendo…":"Apagando…":s?"No disponible":a?"Encendido":"Apagado");return`
      <button
        class="device ${a?"is-on":""} ${n?"is-pending":""} ${c?"is-error":""}"
    ...
    `}_renderDeviceGroup(e,t,r){return`
      <section class="surface control-section">
        ...
        <div class="device-grid">${r.map(a=>this._renderDevice(a)).join("")}</div>
      </section>
    `}_renderWeather(){const e=this._config(),t=this._state(e.weather);if(!t)return`
      ...
      `;const r=t.attributes||{},a=t.state,s=this._forecast.slice(0,3);return`
      <section class="surface weather-card">
        <div class="weather-main">
          <div class="weather-symbol">${this._escape(T[a]||"·")}</div>
          <div class="weather-copy">
            <span class="eyebrow">Clima · Casa</span>
            <h2>${this._escape(F[a]||a)}</h2>
        ...
        ${e.showForecast?`
          <div class="forecast-row">
            ${s.length?s.map(n=>{const c=new Date(n.datetime),i=new Intl.DateTimeFormat("es-BO",{weekday:"short"}).format(c);return`
                <div class="forecast-item">
                  <span>${this._escape(i)}</span>
                  <b>${this._escape(T[n.condition]||"·")}</b>
    ...
    `}_renderMedia(){const e=this._config(),t=this._state(e.mediaPlayer),r=!t||["unknown","unavailable"].includes(t.state),a=(t==null?void 0:t.attributes)||{},s=(t==null?void 0:t.state)==="playing",n=r?"No disponible":s?"Reproduciendo":(t==null?void 0:t.state)==="paused"?"En pausa":(t==null?void 0:t.state)==="idle"?"En espera":(t==null?void 0:t.state)||"Detenido",c=a.media_title||a.friendly_name||"Showroom 1",i=a.media_artist||a.source||"Música del showroom",l=Number(a.volume_level),o=(d,g,p,u=!1)=>`
      <button
    ...
    `;return`
      <section class="surface media-card ${r?"is-unavailable":""}">
        <div class="section-heading compact-heading">
          ...
          <span class="media-state ${s?"is-playing":""}">${this._escape(n)}</span>
        </div>
        <div class="media-body">
          <div class="media-art">${this._icon("music")}</div>
          <div class="media-copy">
            <strong>${this._escape(c)}</strong>
            <span>${this._escape(i)}</span>
            <small>${Number.isFinite(l)?`Volumen ${Math.round(l*100)}%`:"Volumen no informado"}</small>
          </div>
        </div>
        <div class="media-controls">
          ${o("volume_down","volumeDown","Bajar volumen")}
          ${o("media_previous_track","previous","Pista anterior")}
          ${o("media_play_pause",s?"pause":"play",s?"Pausar":"Reproducir",!0)}
          ${o("media_next_track","next","Pista siguiente")}
          ${o("volume_up","volumeUp","Subir volumen")}
        </div>
      </section>
    `}_renderSceneButton(e){const t=this._pendingAction===e.key,r=this._sceneStatus(e),a=t?"Aplicando...":r.active?"Activo":r.unavailable?"Sin datos":"Inactivo";return`
      <button
    ...
    `}_renderSceneBlock({eyebrow:e,title:t,scenes:r,className:a}){const s=this._config(),n=r.find(o=>this._sceneStatus(o).active),c=this._pendingAction===s.powerOffScript,i=s.sceneControlEntities.some(o=>{var d;return((d=this._state(o))==null?void 0:d.state)==="on"}),l=!!this._pendingAction||!i;return`
        ...
        <div class="section-heading compact-heading scenes-heading">
          ...
          <div class="scene-heading-actions">
            <span class="scene-summary ${n?"is-active":""}">
              ${this._escape(c?"Apagando…":n?n.name:"Selección manual")}
            </span>
            <button
            ...
            >
              ...
              <strong>${c?"Apagando…":"Apagar escena"}</strong>
            </button>
          </div>
        </div>
        <div class="scene-grid">${r.map(o=>this._renderSceneButton(o)).join("")}</div>
      </section>
    `}_renderScenes(){const e=this._config(),t=[...e.scenes,...e.sampleScenes],r=t.find(c=>this._sceneStatus(c).active),a=this._pendingAction===e.powerOffScript,s=e.sceneControlEntities.some(c=>{var i;return((i=this._state(c))==null?void 0:i.state)==="on"}),n=!!this._pendingAction||!s;return`
      <section class="surface scenes-card quick-scenes-card">
        <div class="section-heading compact-heading scenes-heading">
          ...
          <div class="scene-heading-actions">
            <span class="scene-summary ${r?"is-active":""}">${this._escape(a?"Apagando...":r?r.name:"Manual")}</span>
            <button
              class="clear-scene-button"
              data-action="clear-scene"
              aria-label="Apagar toda la iluminación de escenas"
        ...
        <div class="scene-grid quick-scene-grid">${t.map(c=>this._renderSceneButton(c)).join("")}</div>
      </section>
    `}_renderGeneralControl(e=!0){const t=this._config(),r=this._pendingAction===t.powerOnScript,a=this._pendingAction===t.powerOffScript;return`
    ...
    `}_renderActivity(){const e=this._energyRangeDefinition(),t=this._energyData.map(i=>Math.max(0,Number(i.change)||0)),r=t.reduce((i,l)=>i+l,0),a=t.length?r/t.length:0,s=t.length?Math.max(...t):0,n=new Date().toLocaleDateString("es-BO",{month:"short",year:"numeric"}).replace(".",""),c=this._energyRange==="day"?`${t.length} ${t.length===1?"hora":"horas"}`:this._energyRange==="month"?`${t.length} ${t.length===1?"día":"días"}`:`${t.length} ${t.length===1?"mes":"meses"}`;return`
      <section class="surface activity-card energy-shell">
        ...
        <div class="energy-toolbar">
          <div class="energy-tabs" role="tablist" aria-label="Período de consumo energético">
            ${[["day","Día"],["month","Mes"],["year","Año"]].map(([i,l])=>`<button class="energy-tab ${this._energyRange===i?"is-active":""}" data-action="energy-range" data-range="${i}" role="tab" aria-selected="${this._energyRange===i}">${l}</button>`).join("")}
          </div>
        ...
        <div class="energy-summary">
          ...
          <div class="energy-stat"><small>Promedio por ${this._escape(e.intervalLabel)}</small><strong>${this._energyLoading&&!this._energyData.length?"…":`${this._formatEnergy(a)} kWh`}</strong><span>${this._escape(c)}</span></div>
          <div class="energy-stat"><small>Mayor intervalo</small><strong>${this._energyLoading&&!this._energyData.length?"…":`${this._formatEnergy(s)} kWh`}</strong><span>Pico estimado del período</span></div>
        </div>
    ...
    `}_renderConfirmDialog(){if(!["on","off"].includes(this._confirmAction))return"";const e=this._confirmAction==="on",t=this._config(),r=e?t.powerOnScript:t.powerOffScript,a=this._pendingAction===r,s=e?"¿Encender toda la iluminación?":"¿Apagar todo el showroom?",n=e?"Se encenderán las luminarias generales del showroom. Después podrás elegir una escena o ajustar cada zona de forma individual.":"Se apagarán las luminarias generales del showroom y cualquier escena activa.",c=e?"Sí, encender":"Sí, apagar";return`
      <div class="dialog-backdrop" data-action="cancel-power-confirm">
        <section class="dialog-card" role="dialog" aria-modal="true" aria-labelledby="power-dialog-title" data-dialog-card>
          ...
          <h2 id="power-dialog-title">${this._escape(s)}</h2>
          <p>${this._escape(n)}</p>
          <div class="dialog-actions">
            <button class="secondary-button" data-action="cancel-power-confirm" ${a?"disabled":""}>Cancelar</button>
            <button class="primary-button ${e?"confirm-on":""}" data-action="confirm-power" ${a?"disabled":""}>${a?"Ejecutando…":this._escape(c)}</button>
          </div>
        </section>
      </div>
        ...
        <section class="view-panel view-lights" aria-labelledby="lights-view-title">
          ...
          <div class="lighting-layout">
            <section class="surface control-section spots-section">
              ...
              <div class="device-grid">${e.spots.map(r=>this._renderDevice(r)).join("")}</div>
            </section>
            ${e.samples.length?`<section class="surface control-section samples-section">
              ...
              <div class="device-grid">${e.samples.map(r=>this._renderDevice(r)).join("")}</div>
            </section>`:""}
    ...
    `}render(){var g;if(!this.shadowRoot||!this._hass)return;this._captureEnergyChartScroll(),this.setAttribute("data-theme",this._theme);const e=this._config();this.setAttribute("data-panel-kind",e.panelKind);const t=e.staticOnly,r=this._state(e.weather),a=(r==null?void 0:r.attributes)||{},s=r==null?void 0:r.state,n=this._theme==="dark"?"claro":"oscuro",c=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],i=c.filter(p=>this._visibleSwitchState(p.entity)==="on").length,l=(g=this._state(e.mediaPlayer))==null?void 0:g.state,o=l==="playing"?"Reproduciendo":l==="paused"?"En pausa":"Detenido",d=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`;this.shadowRoot.innerHTML=`
      <style>
        :host {
          --primary: #f26522;
      ...
      <div class="app-shell">
        <header class="topbar">
          ...
          ${t?"":`<div class="status-strip" aria-label="Resumen de ${this._escape(e.title)}">
            <button class="status-pill ${i?"is-active":""}" data-action="set-view" data-view="lights">
              <span class="status-pill-icon">${this._icon("bulb")}</span>
              <span><strong data-lights-summary>${i} de ${c.length}</strong><small>Luces</small></span>
            </button>
            ${e.panelKind==="general"?"":`<button class="status-pill ${l==="playing"?"is-active":""}" data-action="set-view" data-view="home">
              ...
              <span><strong>${this._escape(o)}</strong><small>Multimedia</small></span>
            </button>`}
          ...
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(F[s]||s||"Sin datos")}, ${this._escape(a.temperature??"Sin datos")}${this._escape(a.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(T[s]||"·")}</span>
              <strong>${this._escape(a.temperature??"--")}${this._escape(a.temperature_unit??"°")}</strong>
            </div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar a tema ${n}" title="Cambiar a tema ${n}">${q}</button>
          </div>
        </header>
    ...
    `,this._updateClock(),requestAnimationFrame(()=>this._restoreEnergyChartScroll())}}customElements.get("showroom-panel")||customElements.define("showroom-panel",j);

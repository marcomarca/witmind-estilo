...
`;class P extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._panel=null,this._narrow=!1,this._started=!1,this._renderQueued=!1,this._forecast=[],this._history=[],this._historyError="",this._liveStates=new Map,this._pendingSwitches=new Map,this._switchErrors=new Map,this._switchTimers=new Map,this._pendingAction="",this._confirmAction="",this._toast=null,this._toastTimer=null,this._clockTimer=null,this._historyTimer=null,this._unsubscribeStates=null,this._unsubscribeForecast=null,this._forecastEntity="",this._energyRange="day",this._energyDayOffset=0,this._energyData=[],this._energyLoading=!1,this._energyError=null,this._energyMonthTotal=null,this._energyRequestId=0,this._energyRefreshTimer=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._themeStorageKey="witmind-showroom-panel-theme",this._theme=this._loadTheme(),this._activeView="home",this.shadowRoot.addEventListener("click",e=>this._handleClick(e)),this.shadowRoot.addEventListener("scroll",e=>this._handleEnergyScroll(e),!0)}set hass(e){var n,o;const t=this._hass,r=this._config(),a=this._relevantHassChanged(t,e),s=!t||((n=t.states)==null?void 0:n[r.energySensor])!==((o=e==null?void 0:e.states)==null?void 0:o[r.energySensor]);this._hass=e,a&&this._syncStatesFromHass(),this._started?s&&this._scheduleEnergyRefresh():(this._started=!0,this._start()),a&&this._requestRender()}get hass(){return this._hass}set panel(e){const t=this._config().weather;this._panel=e;const r=this._config().weather;this._started&&t!==r&&(this._resetForecastSubscription(),this._subscribeWeather()),this._hass&&(this._syncStatesFromHass(),this._loadEnergyStatistics(),this._requestRender())}get panel(){return this._panel}set narrow(e){this._narrow=!!e,this.toggleAttribute("narrow",this._narrow)}get narrow(){return this._narrow}connectedCallback(){this._hass&&(this._requestRender(),this._scheduleEnergyRefresh(!0))}disconnectedCallback(){clearInterval(this._clockTimer),clearInterval(this._historyTimer),clearTimeout(this._toastTimer),clearTimeout(this._energyRefreshTimer),this._energyRequestId+=1;for(const e of this._switchTimers.values())clearTimeout(e);this._switchTimers.clear(),this._resetForecastSubscription(),this._unsubscribeStates&&(this._unsubscribeStates(),this._unsubscribeStates=null),this._started=!1}_requestRender(){this._renderQueued||!this._hass||!this.shadowRoot||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.render()}))}_handleClick(e){const t=e.target.closest("[data-action]");if(!t||!this._hass)return;const r=t.dataset.action;if(r==="toggle-menu"){this._toggleHomeAssistantMenu();return}if(r==="toggle-theme"){this._toggleTheme();return}if(r==="set-view"){const a=t.dataset.view;["home","lights","energy","system"].includes(a)&&(this._activeView=a,this._requestRender());return}if(r==="toggle-switch"){this._toggleSwitch(t.dataset.entity);return}if(r==="run-scene"){this._runScene(t.dataset.sceneKey||t.dataset.entity,t.dataset.label);return}if(r==="open-power-on"){this._pendingAction||(this._confirmAction="on",this._requestRender());return}if(r==="open-power-off"){this._pendingAction||(this._confirmAction="off",this._requestRender());return}if(r==="cancel-power-confirm"){const a=e.target.closest("[data-dialog-card]");if(t.classList.contains("dialog-backdrop")&&a)return;this._pendingAction||(this._confirmAction="",this._requestRender());return}if(r==="confirm-power"){this._confirmGeneralPower();return}if(r==="clear-scene"){this._clearScene();return}if(r==="media"){this._mediaAction(t.dataset.service);return}if(r==="energy-range"){this._setEnergyRange(t.dataset.range);return}if(r==="energy-day-prev"){this._shiftEnergyDay(-1);return}if(r==="energy-day-next"){this._shiftEnergyDay(1);return}if(r==="energy-day-today"){this._resetEnergyDay();return}r==="refresh-energy"&&(this._energyLastLoadedAt=0,this._loadEnergyStatistics())}_toggleHomeAssistantMenu(){this.dispatchEvent(new Event("hass-toggle-menu",{bubbles:!0,composed:!0}))}_loadTheme(){try{return localStorage.getItem(this._themeStorageKey)==="light"?"light":"dark"}catch{return"dark"}}_saveTheme(){try{localStorage.setItem(this._themeStorageKey,this._theme)}catch(e){console.warn("No se pudo guardar el tema del showroom:",e)}}_toggleTheme(){this._theme=this._theme==="dark"?"light":"dark",this.setAttribute("data-theme",this._theme),this._saveTheme(),this._requestRender()}_config(){var p;const e=((p=this._panel)==null?void 0:p.config)||{},t=h=>Array.isArray(h)?[...new Set(h.filter(Boolean).map(m=>String(m)))]:[],r=(h,m)=>(Array.isArray(h)&&h.length?h:m).filter(w=>w==null?void 0:w.entity).map((w,u)=>({entity:String(w.entity),name:w.name||`Dispositivo ${u+1}`,subtitle:w.subtitle||"Iluminación",icon:w.icon||"bulb"})),a=r(e.spots,b.spots),s=r(e.samples||e.muestras,b.samples),n=[...a,...s].map(h=>h.entity),o=t(e.scene_control_entities||e.sceneControlEntities),i=o.length?o:n,l=(h,m,v)=>(Array.isArray(h)&&h.length?h:m).filter(u=>(u==null?void 0:u.entity)||(u==null?void 0:u.id)||(u==null?void 0:u.key)).map((u,x)=>{const k=u.entity?String(u.entity):"",S=String(u.id||u.key||k||`${v}-${x+1}`),y=m.find(M=>{const T=M.entity?String(M.entity):"",C=String(M.id||M.key||T||"");return k&&T===k||C===S}),$=k||(y!=null&&y.entity?String(y.entity):""),E=String(u.id||u.key||(y==null?void 0:y.id)||$||`${v}-${x+1}`),D=$||`${v}:${E}`,_=Array.isArray(u.on_entities)||Array.isArray(u.onEntities),f=Array.isArray(u.off_entities)||Array.isArray(u.offEntities),N=t(_?u.on_entities||u.onEntities:y==null?void 0:y.onEntities),R=t(f?u.off_entities||u.offEntities:[]),z=f?R.filter(M=>!N.includes(M)):i.filter(M=>!N.includes(M));return{key:D,id:E,entity:$,name:u.name||(y==null?void 0:y.name)||`Escena ${x+1}`,subtitle:u.subtitle||(y==null?void 0:y.subtitle)||"Escena del showroom",icon:u.icon||(y==null?void 0:y.icon)||"presentation",directOnly:u.direct_only??u.directOnly??(y==null?void 0:y.directOnly)??!1,onEntities:N,offEntities:z}}),c=e.reflector||b.reflector,d=Number(e.history_hours??e.historyHours),g=Number(e.chart_hours??e.chartHours);return{title:e.title||b.title,subtitle:e.subtitle||b.subtitle,siteLabel:e.site_label||e.siteLabel||b.siteLabel,logo:e.logo||b.logo,weather:e.weather||b.weather,mediaPlayer:e.media_player||e.mediaPlayer||b.mediaPlayer,lightCountSensor:e.light_count_sensor||e.lightCountSensor||b.lightCountSensor,energySensor:e.energy_sensor||e.energySensor||b.energySensor,batteryLevel:e.battery_level||e.batteryLevel||b.batteryLevel,powerOnScript:e.power_on_script||e.powerOnScript||b.powerOnScript,powerOffScript:e.power_off_script||e.powerOffScript||b.powerOffScript,historyHours:Number.isFinite(d)&&d>0?Math.min(24,d):b.historyHours,chartHours:Number.isFinite(g)&&g>0?Math.min(72,g):b.chartHours,showForecast:e.show_forecast??e.showForecast??b.showForecast,spots:a,samples:s,sceneControlEntities:i,reflector:c!=null&&c.entity?{entity:String(c.entity),name:c.name||b.reflector.name,subtitle:c.subtitle||b.reflector.subtitle,icon:c.icon||b.reflector.icon}:null,scenes:l(e.scenes,b.scenes,"scene"),sampleScenes:l(e.sample_scenes||e.sampleScenes,b.sampleScenes,"sample")}}_allDevices(){const e=this._config();return[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]]}_allScenes(e=this._config()){return[...e.scenes,...e.sampleScenes]}_trackedEntities(){const e=this._config();return new Set([...this._allDevices().map(t=>t.entity),...e.sceneControlEntities,...this._allScenes(e).flatMap(t=>[t.entity,...t.onEntities,...t.offEntities]),e.weather,e.mediaPlayer,e.lightCountSensor,e.energySensor,e.batteryLevel,e.powerOnScript,e.powerOffScript].filter(Boolean))}_relevantHassChanged(e,t){var r,a;if(!e||!t)return!0;for(const s of this._trackedEntities())if(((r=e.states)==null?void 0:r[s])!==((a=t.states)==null?void 0:a[s]))return!0;return!1}async _start(){this._clockTimer=setInterval(()=>this._updateClock(),3e4),await Promise.allSettled([this._fetchCurrentStates(),this._subscribeStateChanges(),this._subscribeWeather(),this._loadEnergyStatistics()])}_syncStatesFromHass(){var e;if((e=this._hass)!=null&&e.states)for(const t of this._trackedEntities()){const r=this._hass.states[t];r&&this._applyLiveState(t,r)}}_applyLiveState(e,t){if(!t){this._liveStates.delete(e);return}this._liveStates.set(e,t);const r=this._pendingSwitches.get(e);if(r&&t.state===r.desired){this._pendingSwitches.delete(e),this._switchErrors.delete(e);const a=this._switchTimers.get(e);a&&clearTimeout(a),this._switchTimers.delete(e)}}async _fetchCurrentStates(){var e;if((e=this._hass)!=null&&e.callWS)try{const t=await this._hass.callWS({type:"get_states"}),r=this._trackedEntities();for(const a of t||[])r.has(a.entity_id)&&this._applyLiveState(a.entity_id,a);this._requestRender()}catch(t){console.error("No se pudieron sincronizar los estados del showroom:",t)}}async _subscribeStateChanges(){var e;if(!(!((e=this._hass)!=null&&e.connection)||this._unsubscribeStates))try{this._unsubscribeStates=await this._hass.connection.subscribeEvents(t=>{var a;const r=(a=t==null?void 0:t.data)==null?void 0:a.entity_id;!r||!this._trackedEntities().has(r)||(this._applyLiveState(r,t.data.new_state),r===this._config().energySensor&&this._scheduleEnergyRefresh(),this._requestRender())},"state_changed")}catch(t){console.error("No se pudo suscribir a state_changed:",t)}}_resetForecastSubscription(){this._unsubscribeForecast&&(this._unsubscribeForecast(),this._unsubscribeForecast=null),this._forecastEntity=""}async _subscribeWeather(){var t;const e=this._config();if(!(!((t=this._hass)!=null&&t.connection)||!e.weather)&&!(this._unsubscribeForecast&&this._forecastEntity===e.weather)){this._resetForecastSubscription();try{this._forecastEntity=e.weather,this._unsubscribeForecast=await this._hass.connection.subscribeMessage(r=>{this._forecast=Array.isArray(r==null?void 0:r.forecast)?r.forecast:[],this._requestRender()},{type:"weather/subscribe_forecast",forecast_type:"daily",entity_id:e.weather})}catch(r){this._forecastEntity="",console.error("No se pudo cargar el pronóstico:",r)}}}_energyRefreshInterval(){return this._energyRange==="day"?3e4:this._energyRange==="month"?12e4:3e5}_scheduleEnergyRefresh(e=!1){if(!this._hass||!this.isConnected||(clearTimeout(this._energyRefreshTimer),this._energyRange==="day"&&this._energyDayOffset!==0))return;const t=this._energyRefreshInterval(),r=this._energyLastLoadedAt?Date.now()-this._energyLastLoadedAt:0,a=e?0:this._energyLastLoadedAt?Math.max(1e3,t-r):t;this._energyRefreshTimer=setTimeout(()=>this._loadEnergyStatistics(),a)}_energyRangeDefinition(e=this._energyRange){const t=new Date;let r,a=t,s,n,o,i=!1;if(e==="year")r=new Date(t.getFullYear(),0,1,0,0,0,0),s="month",n=`Año ${t.getFullYear()}`,o="mes";else if(e==="month")r=new Date(t.getFullYear(),t.getMonth(),1,0,0,0,0),s="day",n=t.toLocaleDateString("es-BO",{month:"long",year:"numeric"}),o="día";else{const l=new Date(t.getFullYear(),t.getMonth(),t.getDate(),0,0,0,0);r=new Date(l.getFullYear(),l.getMonth(),l.getDate()+Math.min(0,Number(this._energyDayOffset)||0),0,0,0,0),i=r.getTime()===l.getTime(),a=i?t:new Date(r.getFullYear(),r.getMonth(),r.getDate()+1,0,0,0,0),s="hour",n=r.toLocaleDateString("es-BO",{weekday:"long",day:"numeric",month:"long"}),o="hora"}return{start:r,end:a,period:s,title:n,intervalLabel:o,isToday:i}}_energyValueToKWh(e,t){const r=Number(e);if(!Number.isFinite(r))return null;const a=String(t||"kWh").trim().toLowerCase().replaceAll(" ","");return a==="kwh"?r:a==="wh"?r/1e3:a==="mwh"?r*1e3:null}_deriveEnergyStateDeltas(e,t,r){const a=Number(t instanceof Date?t.getTime():t),s=Number(r instanceof Date?r.getTime():r),n=e.filter(l=>Number.isFinite(Number(l.start))&&Number.isFinite(Number(l.state))).sort((l,c)=>Number(l.start)-Number(c.start)),o=[];let i=null;for(const l of n){const c=Number(l.start),d=Number(l.state);if(c<a){i=d;continue}if(c>=s)break;let g=0;if(Number.isFinite(i)){const p=d-i;g=p>=0?p:Math.max(0,d)}o.push({...l,change:g}),i=d}return o}_aggregateEnergyDayRows(e,t,r){var l;const a=new Date(t.start),s=new Date(t.end),n=t.isToday?new Date(s.getFullYear(),s.getMonth(),s.getDate(),s.getHours(),0,0,0):new Date(a.getFullYear(),a.getMonth(),a.getDate(),23,0,0,0),o=new Map;for(let c=new Date(a);c<=n;c.setHours(c.getHours()+1)){const d=c.getTime();o.set(d,{start:d,end:new Date(c.getFullYear(),c.getMonth(),c.getDate(),c.getHours()+1,0,0,0).getTime(),change:0,samples:0,partial:t.isToday&&d===n.getTime(),live:!1})}let i=null;for(const c of e){const d=new Date(Number(c.start));if(!Number.isFinite(d.getTime()))continue;const g=new Date(d.getFullYear(),d.getMonth(),d.getDate(),d.getHours(),0,0,0).getTime(),p=o.get(g);p&&(p.change+=Math.max(0,Number(c.change)||0),p.samples+=1,Number.isFinite(c.state)&&(!i||Number(c.end||c.start)>Number(i.end||i.start))&&(i=c))}if(t.isToday){const c=this._energyValueToKWh(r==null?void 0:r.state,(l=r==null?void 0:r.attributes)==null?void 0:l.unit_of_measurement),d=i==null?void 0:i.state,g=Number((i==null?void 0:i.end)||(i==null?void 0:i.start)),p=o.get(n.getTime());if(p&&Number.isFinite(c)&&Number.isFinite(d)&&Number.isFinite(g)&&g>=n.getTime()&&s.getTime()-g>=0&&s.getTime()-g<=900*1e3){const h=c-d;Number.isFinite(h)&&h>=0&&(p.change+=h,p.live=h>0)}}return[...o.values()].filter(c=>c.samples>0||c.live)}_calculateCurrentMonthEnergy(e,t,r){var g;const a=new Date(t),s=new Date(a.getFullYear(),a.getMonth(),1,0,0,0,0).getTime(),n=e.filter(p=>Number.isFinite(Number(p.start))&&Number(p.start)>=s).sort((p,h)=>Number(p.start)-Number(h.start));let o=n.reduce((p,h)=>p+Math.max(0,Number(h.change)||0),0),i=null;for(const p of n)Number.isFinite(Number(p.state))&&(!i||Number(p.end||p.start)>Number(i.end||i.start))&&(i=p);const l=this._energyValueToKWh(r==null?void 0:r.state,(g=r==null?void 0:r.attributes)==null?void 0:g.unit_of_measurement),c=Number(i==null?void 0:i.state),d=Number((i==null?void 0:i.end)||(i==null?void 0:i.start));if(Number.isFinite(l)&&Number.isFinite(c)&&Number.isFinite(d)&&d>=s&&d<=a.getTime()&&a.getTime()-d<=7200*1e3){const p=l-c;Number.isFinite(p)&&p>=0&&(o+=p)}return n.length||Number.isFinite(c)?o:null}_aggregateEnergyCalendarRows(e,t,r,a){var d;if(!["month","year"].includes(a))return[];const s=new Map;let n=null;for(const g of e){const p=Number(g.start);if(!Number.isFinite(p))continue;const h=new Date(p),m=a==="month"?new Date(h.getFullYear(),h.getMonth(),h.getDate(),0,0,0,0).getTime():new Date(h.getFullYear(),h.getMonth(),1,0,0,0,0).getTime(),v=s.get(m)||{start:m,change:0,samples:0,partial:!1,live:!1};v.change+=Math.max(0,Number(g.change)||0),v.samples+=1,s.set(m,v),Number.isFinite(Number(g.state))&&(!n||p>Number(n.start))&&(n=g)}const o=this._energyValueToKWh(r==null?void 0:r.state,(d=r==null?void 0:r.attributes)==null?void 0:d.unit_of_measurement),i=Number(n==null?void 0:n.state),l=Number(n==null?void 0:n.start),c=new Date(t.end);if(Number.isFinite(o)&&Number.isFinite(i)&&Number.isFinite(l)&&l<=c.getTime()&&c.getTime()-l<=7200*1e3){const g=o-i;if(Number.isFinite(g)&&g>=0){const p=a==="month"?new Date(c.getFullYear(),c.getMonth(),c.getDate(),0,0,0,0).getTime():new Date(c.getFullYear(),c.getMonth(),1,0,0,0,0).getTime(),h=s.get(p)||{start:p,change:0,samples:0,partial:!1,live:!1};h.change+=g,h.live=g>0,s.set(p,h)}}return[...s.values()].sort((g,p)=>Number(g.start)-Number(p.start))}_energyViewSignature(){return JSON.stringify([this._energyRange,this._energyDayOffset,this._energyMonthTotal,this._energyError||"",this._energyData.map(e=>[Number(e.start),Number(e.change)||0,Number(e.samples)||0,!!e.partial,!!e.live])])}async _loadEnergyStatistics(){var p;if(!((p=this._hass)!=null&&p.connection))return;clearTimeout(this._energyRefreshTimer);const e=this._config(),t=this._state(e.energySensor),r=Number(t==null?void 0:t.state);if(!t){this._energyData=[],this._energyMonthTotal=null,this._energyError=`No existe ${e.energySensor} en Home Assistant.`,this._energyLoading=!1,this._requestRender();return}if(!Number.isFinite(r)){this._energyData=[],this._energyMonthTotal=null,this._energyError=`${e.energySensor} no entrega un valor numérico.`,this._energyLoading=!1,this._requestRender();return}const a=++this._energyRequestId,s=this._energyViewSignature(),n=this._energyData.length>0,o=this._energyRangeDefinition(),i=new Date,l=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0),c=new Date(o.start.getTime()-3600*1e3),d=this._energyRange==="year"?"hour":"5minute",g=new Date(l.getTime()-3600*1e3);this._energyLoading=!0,this._energyError=null,n||this._requestRender();try{const h=await this._hass.connection.sendMessagePromise({type:"recorder/get_statistics_metadata",statistic_ids:[e.energySensor]});if(a!==this._energyRequestId)return;const v=(Array.isArray(h)?h:[]).find(_=>(_==null?void 0:_.statistic_id)===e.energySensor)||null;if(!v||!v.has_sum)throw new Error("La entidad no dispone de estadísticas acumulables. Verifica device_class: energy y state_class total/total_increasing.");const w=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:c.toISOString(),end_time:o.end.toISOString(),statistic_ids:[e.energySensor],period:d,units:{energy:"kWh"},types:["state"]}),u=this._hass.connection.sendMessagePromise({type:"recorder/statistics_during_period",start_time:g.toISOString(),end_time:i.toISOString(),statistic_ids:[e.energySensor],period:"hour",units:{energy:"kWh"},types:["state"]}),[x,k]=await Promise.all([w,u]);if(a!==this._energyRequestId)return;const S=_=>(Array.isArray(_==null?void 0:_[e.energySensor])?_[e.energySensor]:[]).map(f=>({start:Number(f.start),end:Number(f.end),change:f.change===void 0||f.change===null?null:Math.max(0,Number(f.change)||0),state:f.state===void 0||f.state===null?null:Number(f.state)})).filter(f=>Number.isFinite(f.start)&&(Number.isFinite(f.state)||Number.isFinite(f.change))),y=S(x),$=this._deriveEnergyStateDeltas(y,o.start,o.end);this._energyData=this._energyRange==="day"?this._aggregateEnergyDayRows($,o,t):this._aggregateEnergyCalendarRows($,o,t,this._energyRange);const E=S(k),D=this._deriveEnergyStateDeltas(E,l,i);if(this._energyMonthTotal=this._calculateCurrentMonthEnergy(D,i,t),this._energyRange==="month"&&this._energyData.length)this._energyMonthTotal=this._energyData.reduce((_,f)=>_+Math.max(0,Number(f.change)||0),0);else if(this._energyRange==="year"&&Number.isFinite(this._energyMonthTotal)){const _=new Date(i.getFullYear(),i.getMonth(),1,0,0,0,0).getTime(),f=this._energyData.find(N=>Number(N.start)===_);f&&(f.change=this._energyMonthTotal)}this._energyLastLoadedAt=Date.now(),this._energyError=null}catch(h){if(a!==this._energyRequestId)return;this._energyData=[],this._energyMonthTotal=null,this._energyError=(h==null?void 0:h.message)||"No se pudieron consultar las estadísticas energéticas.",console.error("Error cargando estadísticas de energía del showroom:",h)}finally{if(a===this._energyRequestId){this._energyLoading=!1;const h=s!==this._energyViewSignature();(!n||h)&&this._requestRender(),this._scheduleEnergyRefresh()}}}_setEnergyRange(e){!["day","month","year"].includes(e)||e===this._energyRange||(this._energyRange=e,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_shiftEnergyDay(e){if(this._energyRange!=="day")return;const t=Number(e);if(!Number.isFinite(t)||t===0)return;const r=Math.min(0,this._energyDayOffset+t);r!==this._energyDayOffset&&(this._energyDayOffset=r,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_resetEnergyDay(){this._energyRange!=="day"||this._energyDayOffset===0||(this._energyDayOffset=0,this._energyData=[],this._energyError=null,this._energyLastLoadedAt=0,this._energyAutoFollow=!0,this._energyScrollLeft=null,this._loadEnergyStatistics())}_formatEnergy(e,t=2){const r=Number(e);return Number.isFinite(r)?r.toLocaleString("es-BO",{minimumFractionDigits:t,maximumFractionDigits:t}):"--"}_energyLabel(e,t=this._energyRange){const r=new Date(Number(e));return Number.isFinite(r.getTime())?t==="year"?r.toLocaleDateString("es-BO",{month:"short"}).replace(".",""):t==="month"?String(r.getDate()):r.toLocaleTimeString("es-BO",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--"}_captureEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);this._energyScrollLeft=e.scrollLeft,this._energyAutoFollow=t<=0||t-e.scrollLeft<=8}_handleEnergyScroll(e){var a,s;const t=(s=(a=e.target)==null?void 0:a.closest)==null?void 0:s.call(a,"[data-energy-scroll]");if(!t)return;const r=Math.max(0,t.scrollWidth-t.clientWidth);this._energyScrollLeft=t.scrollLeft,this._energyAutoFollow=r<=0||r-t.scrollLeft<=8}_restoreEnergyChartScroll(){var r;const e=(r=this.shadowRoot)==null?void 0:r.querySelector("[data-energy-scroll]");if(!e)return;const t=Math.max(0,e.scrollWidth-e.clientWidth);if(this._energyAutoFollow||this._energyScrollLeft===null){e.scrollLeft=t,this._energyScrollLeft=e.scrollLeft;return}e.scrollLeft=Math.max(0,Math.min(this._energyScrollLeft,t))}_energyChart(){const e=this._energyData;if(this._energyLoading&&!e.length)return'<div class="energy-empty"><span class="energy-spinner"></span>Consultando estadísticas de Home Assistant…</div>';if(this._energyError)return`<div class="energy-empty error">${this._icon("status")}<span>${this._escape(this._energyError)}</span></div>`;if(!e.length)return'<div class="energy-empty">No hay estadísticas de consumo disponibles para este período.</div>';const t=280,r=14,a=16,s=22,n=42,o=this._energyRange==="day"?64:this._energyRange==="month"?36:58,i=Math.max(620,e.length*o+r+a),l=t-s-n,c=i-r-a,d=Math.max(...e.map(x=>x.change),.001),g=this._energyRange==="day"?10:e.length>24?4:7,p=Math.max(6,(c-g*Math.max(0,e.length-1))/e.length),h=this._energyRange==="month"?Math.max(1,Math.ceil(e.length/10)):this._energyRange==="day"?2:1,m=[0,.25,.5,.75,1],v=m.map(x=>{const k=s+l*(1-x);return`<line x1="${r}" y1="${k.toFixed(1)}" x2="${i-a}" y2="${k.toFixed(1)}" class="energy-grid-line"/>`}).join(""),w=m.map(x=>{const k=s+l*(1-x),S=this._formatEnergy(d*x,d<1?2:1);return`<span class="energy-y-tick" style="top:${k.toFixed(1)}px">${this._escape(S)}</span>`}).join(""),u=e.map((x,k)=>{const S=r+k*(p+g),y=x.change>0?Math.max(2,x.change/d*l):1,$=s+l-y,E=k%h===0||k===e.length-1,D=this._energyLabel(x.start),_=x.partial?" · en curso":"",f=x.partial?`<text x="${(S+p/2).toFixed(1)}" y="${(s+10).toFixed(1)}" text-anchor="middle" class="energy-current-label">ahora</text>`:"";return`<g class="energy-bar-group ${x.partial?"is-current":""}"><rect x="${S.toFixed(1)}" y="${$.toFixed(1)}" width="${p.toFixed(1)}" height="${y.toFixed(1)}" rx="${Math.min(4,p/2).toFixed(1)}" class="energy-bar ${x.partial?"is-partial":""}"><title>${this._escape(D)} · ${this._formatEnergy(x.change)} kWh${_}</title></rect>${f}${E?`<text x="${(S+p/2).toFixed(1)}" y="${t-14}" text-anchor="middle" class="energy-axis-text">${this._escape(D)}</text>`:""}</g>`}).join("");return`<div class="energy-chart-layout"><div class="energy-y-axis" aria-hidden="true"><span class="energy-y-unit">kWh</span>${w}</div><div class="energy-chart-wrap" data-energy-scroll><svg class="energy-chart" width="${i}" height="${t}" viewBox="0 0 ${i} ${t}" role="img" aria-label="Gráfica de consumo energético estimado en kWh">${v}${u}</svg></div></div>`}async _loadHistory(){var o;const e=this._config();if(!((o=this._hass)!=null&&o.callApi))return;const t=Math.max(e.historyHours,e.chartHours),r=new Date(Date.now()-t*60*60*1e3).toISOString(),a=new Date().toISOString(),s=[e.lightCountSensor,...e.spots.map(i=>i.entity),...e.samples.map(i=>i.entity)].filter(Boolean).join(",");if(!s)return;const n=`history/period/${encodeURIComponent(r)}?filter_entity_id=${encodeURIComponent(s)}&end_time=${encodeURIComponent(a)}&minimal_response&no_attributes`;try{this._history=await this._hass.callApi("GET",n),this._historyError=""}catch(i){this._history=[],this._historyError="No se pudo cargar el historial.",console.error("No se pudo cargar el historial del showroom:",i)}this._requestRender()}_state(e){var t,r;return this._liveStates.get(e)||((r=(t=this._hass)==null?void 0:t.states)==null?void 0:r[e])}_isUnavailable(e){var r;const t=(r=this._state(e))==null?void 0:r.state;return!t||t==="unknown"||t==="unavailable"}_visibleSwitchState(e){var t,r;return((t=this._pendingSwitches.get(e))==null?void 0:t.desired)||((r=this._state(e))==null?void 0:r.state)||"unavailable"}async _toggleSwitch(e){if(!e||!this._hass)return;const t=this._state(e);if(!t||["unknown","unavailable"].includes(t.state)){this._switchErrors.set(e,"No disponible"),this._requestRender();return}const a=this._visibleSwitchState(e)==="on"?"off":"on",s=a==="on"?"turn_on":"turn_off",n=e.split(".")[0]||"switch";this._pendingSwitches.set(e,{desired:a,startedAt:Date.now()}),this._switchErrors.delete(e),this._requestRender();try{await this._hass.callService(n,s,{entity_id:e});const o=this._switchTimers.get(e);o&&clearTimeout(o);const i=setTimeout(()=>this._verifySwitchState(e,a),6e3);this._switchTimers.set(e,i)}catch(o){this._pendingSwitches.delete(e),this._switchErrors.set(e,"La acción falló"),this._requestRender(),console.error(`Error ejecutando ${s} en ${e}:`,o)}}async _verifySwitchState(e,t){var a;await this._fetchCurrentStates();const r=((a=this._state(e))==null?void 0:a.state)===t;this._pendingSwitches.delete(e),this._switchTimers.delete(e),r?this._switchErrors.delete(e):this._switchErrors.set(e,"Sin confirmación"),this._requestRender()}_sceneStatus(e){const t=[...e.onEntities.map(s=>({entityId:s,desired:"on"})),...e.offEntities.map(s=>({entityId:s,desired:"off"}))];if(!t.length)return{active:!1,unavailable:!1,mismatches:[]};const r=t.some(({entityId:s})=>this._isUnavailable(s)),a=t.filter(({entityId:s,desired:n})=>{var o;return((o=this._state(s))==null?void 0:o.state)!==n});return{active:!r&&a.length===0,unavailable:r,mismatches:a}}_sceneExpectations(e){return[...e.offEntities.map(t=>({entityId:t,desired:"off"})),...e.onEntities.map(t=>({entityId:t,desired:"on"}))]}_markExpectedStates(e){const t=Date.now();for(const{entityId:r,desired:a}of e)this._pendingSwitches.set(r,{desired:a,startedAt:t}),this._switchErrors.delete(r)}_clearExpectedStates(e){for(const{entityId:t}of e){this._pendingSwitches.delete(t);const r=this._switchTimers.get(t);r&&clearTimeout(r),this._switchTimers.delete(t)}}async _setEntitiesState(e,t){const r=[...new Set((e||[]).filter(Boolean))];if(!r.length)return;const a=new Map;for(const n of r){const o=n.split(".")[0];o&&(a.has(o)||a.set(o,[]),a.get(o).push(n))}const s=t==="on"?"turn_on":"turn_off";for(const[n,o]of a)await this._hass.callService(n,s,{entity_id:o})}async _waitForExpectedStates(e,t=7e3){const r=Date.now()+t;let a=e;for(;Date.now()<r;){if(await this._fetchCurrentStates(),a=e.filter(({entityId:s,desired:n})=>{var o;return((o=this._state(s))==null?void 0:o.state)!==n}),!a.length)return{ok:!0,mismatches:[]};await new Promise(s=>setTimeout(s,450))}return{ok:!1,mismatches:a}}async _runScene(e,t){var o;if(!e||this._pendingAction)return;const r=this._config(),a=this._allScenes(r).find(i=>i.key===e||i.entity===e);if(!a){this._notify("La escena no está configurada.","error");return}const s=this._sceneExpectations(a);this._pendingAction=a.key,this._markExpectedStates(s),this._requestRender();let n=null;try{if(a.entity&&!a.directOnly)try{await this._hass.callService("scene","turn_on",{entity_id:a.entity})}catch(c){n=c,console.warn(`La escena ${a.entity} no respondió; se aplicará el perfil directo.`,c)}await this._setEntitiesState(a.offEntities,"off"),await this._setEntitiesState(a.onEntities,"on");const i=await this._waitForExpectedStates(s);if(!i.ok){const c=i.mismatches.map(d=>d.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${c}`)}const l=n?`${t||"Modo"} aplicado mediante control directo.`:`${t||"Modo"} activo.`;this._notify(l,"success")}catch(i){for(const{entityId:l,desired:c}of s)((o=this._state(l))==null?void 0:o.state)!==c&&this._switchErrors.set(l,"No confirmó el modo");this._notify("No se pudo aplicar completamente el modo seleccionado.","error"),console.error("Error aplicando modo del showroom:",i)}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._requestRender()}}async _executeGeneralPower(e,t={}){var l;if(!this._hass||this._pendingAction||!["on","off"].includes(e))return!1;const r=this._config(),a=e==="on"?r.powerOnScript:r.powerOffScript;if(!a)return this._notify("El control general no está configurado.","error"),!1;const s=r.sceneControlEntities.map(c=>({entityId:c,desired:e})),n=t.successMessage||(e==="on"?"Iluminación general encendida.":"Iluminación general apagada."),o=t.errorMessage||(e==="on"?"No se pudo encender toda la iluminación.":"No se pudo apagar toda la iluminación.");this._pendingAction=a,this._markExpectedStates(s),this._requestRender();let i=null;try{try{await this._hass.callService("script","turn_on",{entity_id:a})}catch(d){i=d,console.warn(`El script ${a} no respondió; se aplicará el control directo.`,d)}await this._setEntitiesState(r.sceneControlEntities,e);const c=await this._waitForExpectedStates(s);if(!c.ok){const d=c.mismatches.map(g=>g.entityId).join(", ");throw new Error(`No se confirmaron los estados de: ${d}`)}return this._confirmAction="",this._notify(n,"success"),!0}catch(c){for(const{entityId:d,desired:g}of s)((l=this._state(d))==null?void 0:l.state)!==g&&this._switchErrors.set(d,"Sin confirmación");return this._notify(o,"error"),console.error("Error ejecutando el control general del showroom:",{error:c,scriptError:i,desired:e}),!1}finally{this._clearExpectedStates(s),this._pendingAction="",await this._fetchCurrentStates(),this._requestRender()}}async _confirmGeneralPower(){const e=this._confirmAction;["on","off"].includes(e)&&await this._executeGeneralPower(e,{successMessage:e==="on"?"Toda la iluminación del showroom está encendida.":"Toda la iluminación del showroom está apagada."})}async _clearScene(){this._pendingAction||await this._executeGeneralPower("off",{successMessage:"Escena apagada. La iluminación del showroom quedó apagada.",errorMessage:"No se pudo apagar completamente la escena."})}async _mediaAction(e){const t=this._config().mediaPlayer;if(!(!t||!e||this._isUnavailable(t)))try{await this._hass.callService("media_player",e,{entity_id:t})}catch(r){this._notify("No se pudo controlar el reproductor.","error"),console.error(`Error ejecutando media_player.${e}:`,r)}}_notify(e,t="success"){clearTimeout(this._toastTimer),this._toast={message:e,type:t},this._requestRender(),this._toastTimer=setTimeout(()=>{this._toast=null,this._requestRender()},4200)}_historyMap(){var t;const e=new Map;for(const r of this._history||[]){const a=(t=r==null?void 0:r[0])==null?void 0:t.entity_id;a&&e.set(a,r)}return e}_historySegments(e,t,r){if(!Array.isArray(e)||!e.length)return[];const a=e.map(n=>({state:n.state,time:new Date(n.last_changed||n.last_updated).getTime()})).filter(n=>Number.isFinite(n.time)).sort((n,o)=>n.time-o.time);if(!a.length)return[];const s=[];for(let n=0;n<a.length;n+=1){const o=a[n],i=a[n+1],l=Math.max(t,o.time),c=Math.min(r,(i==null?void 0:i.time)??r);c<=l||s.push({state:o.state,left:(l-t)/(r-t)*100,width:(c-l)/(r-t)*100})}return s}_sparkline(e,t){var h;const r=this._historyMap().get(e)||[],a=Date.now(),s=a-t*60*60*1e3,n=r.map(m=>({value:Number(m.state),time:new Date(m.last_changed||m.last_updated).getTime()})).filter(m=>Number.isFinite(m.value)&&Number.isFinite(m.time)&&m.time>=s).sort((m,v)=>m.time-v.time),o=Number((h=this._state(e))==null?void 0:h.state);if(Number.isFinite(o)&&n.push({value:o,time:a}),!n.length)return{path:"",min:"--",max:"--",avg:"--"};const i=n.map(m=>m.value),l=Math.min(...i),c=Math.max(...i),d=i.reduce((m,v)=>m+v,0)/i.length,g=c-l||1;return{path:n.map((m,v)=>{const w=(m.time-s)/(a-s)*300,u=66-(m.value-l)/g*52;return`${v?"L":"M"}${Math.max(0,Math.min(300,w)).toFixed(1)},${u.toFixed(1)}`}).join(" "),min:this._formatNumber(l),max:this._formatNumber(c),avg:this._formatNumber(d)}}_formatNumber(e){return Number.isFinite(e)?new Intl.NumberFormat("es-BO",{maximumFractionDigits:1}).format(e):"--"}_updateClock(){var o,i,l;if(!this.shadowRoot)return;const e=new Date,t=new Intl.DateTimeFormat("es-BO",{hour:"numeric",minute:"2-digit",hour12:!0}).formatToParts(e),r=((o=t.find(c=>c.type==="hour"))==null?void 0:o.value)||"--",a=((i=t.find(c=>c.type==="minute"))==null?void 0:i.value)||"--",s=(((l=t.find(c=>c.type==="dayPeriod"))==null?void 0:l.value)||"").replaceAll(".","").replaceAll(" ","").toUpperCase();for(const c of this.shadowRoot.querySelectorAll("[data-clock-time]"))c.textContent=`${r}:${a}`;for(const c of this.shadowRoot.querySelectorAll("[data-clock-period]"))c.textContent=s||"";const n=this.shadowRoot.querySelector("[data-current-time]");n&&(n.dateTime=e.toISOString(),n.setAttribute("aria-label",`${r}:${a} ${s}`.trim()))}_icon(e,t=""){const r=L[e]||L.bulb;return`<svg class="icon ${this._escape(t)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${r}</svg>`}_escape(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}_renderDevice(e){const t=this._state(e.entity),r=this._visibleSwitchState(e.entity),a=r==="on",s=!t||["unknown","unavailable"].includes(t.state),n=this._pendingSwitches.has(e.entity),o=this._switchErrors.get(e.entity),i=o||(n?r==="on"?"Encendiendo…":"Apagando…":s?"No disponible":a?"Encendido":"Apagado");return`
      <button
    ...
    `}_renderWeather(){const e=this._config(),t=this._state(e.weather);if(!t)return`
      ...
      <section class="surface weather-card">
        <div class="weather-main">
          <div class="weather-symbol">${this._escape(A[a]||"·")}</div>
        ...
        ${e.showForecast?`
          <div class="forecast-row">
            ${s.length?s.map(n=>{const o=new Date(n.datetime),i=new Intl.DateTimeFormat("es-BO",{weekday:"short"}).format(o);return`
                <div class="forecast-item">
                  <span>${this._escape(i)}</span>
                  <b>${this._escape(A[n.condition]||"·")}</b>
    ...
    `;return`
      <section class="surface media-card ${r?"is-unavailable":""}">
        ...
        <div class="media-controls">
          ...
          ${c("media_play_pause",s?"pause":"play",s?"Pausar":"Reproducir",!0)}
    ...
    `}_renderSceneBlock({eyebrow:e,title:t,scenes:r,className:a}){const s=this._config(),n=r.find(c=>this._sceneStatus(c).active),o=this._pendingAction===s.powerOffScript,i=s.sceneControlEntities.some(c=>{var d;return((d=this._state(c))==null?void 0:d.state)==="on"}),l=!!this._pendingAction||!i;return`
    ...
    `}_renderScenes(){const e=this._config(),t=[...e.scenes,...e.sampleScenes],r=t.find(o=>this._sceneStatus(o).active),a=this._pendingAction===e.powerOffScript,s=e.sceneControlEntities.some(o=>{var i;return((i=this._state(o))==null?void 0:i.state)==="on"}),n=!!this._pendingAction||!s;return`
    ...
    `}_renderNavigation(){return`
      <nav class="view-navigation" aria-label="Secciones del showroom">
        ${[["home","bulb","Inicio"],["lights","spot","Iluminación"],["energy","energy","Energía"],["system","health","Sistema"]].map(([t,r,a])=>`
          <button
    ...
    `}_renderActiveView(e){return this._activeView==="lights"?`
    ...
    `}render(){var d;if(!this.shadowRoot||!this._hass)return;this._captureEnergyChartScroll(),this.setAttribute("data-theme",this._theme);const e=this._config(),t=this._state(e.weather),r=(t==null?void 0:t.attributes)||{},a=t==null?void 0:t.state,s=this._theme==="dark"?"claro":"oscuro",n=[...e.spots,...e.samples,...e.reflector?[e.reflector]:[]],o=n.filter(g=>this._visibleSwitchState(g.entity)==="on").length,i=(d=this._state(e.mediaPlayer))==null?void 0:d.state,l=i==="playing"?"Reproduciendo":i==="paused"?"En pausa":"Detenido",c=this._energyMonthTotal===null?"Sin datos":`${this._formatEnergy(this._energyMonthTotal)} kWh`;this.shadowRoot.innerHTML=`
      <style>
        :host {
          --primary: #f26522;
      ...
      <div class="app-shell">
        <header class="topbar">
          ...
          <div class="status-strip" aria-label="Resumen del showroom">
            <button class="status-pill ${o?"is-active":""}" data-action="set-view" data-view="lights">
              <span class="status-pill-icon">${this._icon("bulb")}</span>
              <span><strong>${o} de ${n.length}</strong><small>Luces</small></span>
            </button>
            <button class="status-pill ${i==="playing"?"is-active":""}" data-action="set-view" data-view="home">
            ...
            <button class="status-pill" data-action="set-view" data-view="energy">
              ...
              <span><strong>${this._escape(c)}</strong><small>Este mes</small></span>
            </button>
          </div>
          <div class="topbar-meta">
            ...
            <div class="header-weather" aria-label="Clima actual: ${this._escape(F[a]||a||"Sin datos")}, ${this._escape(r.temperature??"Sin datos")}${this._escape(r.temperature_unit??"°")}">
              <span aria-hidden="true">${this._escape(A[a]||"·")}</span>
              <strong>${this._escape(r.temperature??"--")}${this._escape(r.temperature_unit??"°")}</strong>
            </div>
            <button class="theme-button" data-action="toggle-theme" aria-label="Cambiar a tema ${s}" title="Cambiar a tema ${s}">${q}</button>
          </div>
        </header>
    ...
    `,this._updateClock(),requestAnimationFrame(()=>this._restoreEnergyChartScroll())}}customElements.get("showroom-panel")||customElements.define("showroom-panel",P);

...
class WitmindAdminPanel extends HTMLElement {
  ...
  private _render() {
    ...
    </style><div class="admin-shell"><header class="topbar"><button class="menu" data-action="toggle-menu" aria-label="Abrir menú de Home Assistant"><span></span></button><div><span class="eyebrow">WTX · MDTC</span><h1>${esc(this._title())}</h1></div></header><main class="dashboard"><section class="hero"><div><span class="eyebrow">${esc(this._subtitle())}</span><h2>${esc(this._title())}</h2><p>${esc(this._panel.description || "Información conectada a Home Assistant")}</p></div><button class="refresh" data-action="refresh">Actualizar</button></section>${this._error ? `<div class="error">${esc(this._error)}</div>` : this._loading ? `<section class="surface empty">Cargando datos de Home Assistant…</section>` : kind === "notifications" ? this._renderNotifications() : this._renderCalendar()}</main></div>`;
  }

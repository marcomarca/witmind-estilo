...
| `0.7.2` | `0.7.3` | [`src/building-control-panel.ts`](file:///c:/dev/automatizacion-estilo/src/building-control-panel.ts) | En resoluciones intermedias (849x978) y móviles (366x794), el plano se recortaba en los laterales y se comía habitaciones. | Ajuste dinámico de `.floor-stage` con ResizeObserver y relación de aspecto exacta calculada en px con `object-fit: fill`, desacoplando media-query de padding y promoviendo release estable a HA. |
| `0.7.3` | `0.7.4` | [`src/building-config.ts`](file:///c:/dev/automatizacion-estilo/src/building-config.ts), [`src/building-control-panel.ts`](file:///c:/dev/automatizacion-estilo/src/building-control-panel.ts), [`home-assistant/`](file:///c:/dev/automatizacion-estilo/home-assistant/) | Discrepancias de potencia (Watts), conteo ambiguo de circuitos y acciones inconsistentes por zona. | Potencias documentadas completas (3.048 W total en 25 circuitos), desglose explícito de activos/inactivos (`X enc. · Y apag.`), acciones por escena/zona y sensores template nativos en HA. |
